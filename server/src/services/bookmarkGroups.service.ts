...
export async function ensureDefaultBookmarkGroup(
  ...
  const found = await BookmarkGroupModel.findOne({
    userId: userOid,
    isDefault: true,
  })
    .select("_id")
...
export async function listGroupsForUser(userOid: mongoose.Types.ObjectId) {
  ...
  return BookmarkGroupModel.find({ userId: userOid })
...
> {
  if (!mongoose.Types.ObjectId.isValid(groupIdHex))
    return { ok: false, message: "Invalid group" };
  const gid = new mongoose.Types.ObjectId(groupIdHex);
  const existing = await BookmarkGroupModel.findOne({
    _id: gid,
    userId: userOid,
  }).lean();
  ...
  try {
    await BookmarkGroupModel.updateOne(
      { _id: gid, userId: userOid },
      { $set: updates },
    );
...
export async function setDefaultGroupForUser(
  ...
  if (!mongoose.Types.ObjectId.isValid(groupIdHex))
    return { ok: false, message: "Invalid group" };
  const gid = new mongoose.Types.ObjectId(groupIdHex);
  const g = await BookmarkGroupModel.findOne({ _id: gid, userId: userOid })
    .select("_id")
    .lean();
  if (!g) return { ok: false, message: "Group not found" };
  await BookmarkGroupModel.updateMany(
    { userId: userOid },
    { $set: { isDefault: false } },
  );
  await BookmarkGroupModel.updateOne(
...
export async function deleteGroupForUser(
  ...
  if (!mongoose.Types.ObjectId.isValid(groupIdHex))
    return { ok: false, message: "Invalid group" };
  const gid = new mongoose.Types.ObjectId(groupIdHex);
  const g = await BookmarkGroupModel.findOne({
    _id: gid,
    userId: userOid,
  }).lean();
  if (!g) return { ok: false, message: "Group not found" };
  if (g.isDefault)
    return { ok: false, message: "Cannot delete the default group" };
  const defaultId = await ensureDefaultBookmarkGroup(userOid);
  await BlogBookmarkModel.updateMany(
