...
export async function respondWithSessionAfterEmailAuth(
  ...
  if (user.twoFactorEnabled && !skipTwoFactor) {
    try {
      const { challengeToken, expiresIn } = await createAuthChallenge(
        String(user._id),
      );
      if (isNewUser) {
      ...
      res.status(503).json({
        success: false,
        message:
          "Two-factor authentication temporarily unavailable. Try again later.",
      });
      return;
    }
  }
  ...
  if (staffRole) {
    const deviceCheck = await assertNewStaffDeviceAllowed(
      String(user._id),
      req,
    );
    if (!deviceCheck.ok) {
