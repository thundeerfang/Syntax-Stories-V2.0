...
function ttlForPurpose(purpose: EmailOtpPurpose): number {
  const sec =
    purpose === "login"
      ? authConfig.OTP_LOGIN_TTL_SECONDS
      : authConfig.OTP_SIGNUP_TTL_SECONDS;
...
function otpPepper(): string {
  ...
  if (!p && process.env.NODE_ENV === "production") {
    console.error(
      "[emailOtp] Set OTP_PEPPER or JWT_SECRET for email OTP hashing.",
    );
  }
...
export async function storeEmailOtpLogin(
  emailNorm: string,
  code: string,
): Promise<number> {
...
export async function storeEmailOtpSignup(
  ...
  const payload: StoredSignupOtp = {
    h: hashEmailOtp(emailNorm, code),
    fullName,
    v,
  };
  await redis.setEx(
...
export async function getStoredLoginOtp(
  emailNorm: string,
): Promise<StoredLoginOtp | null> {
  const redis = getRedis();
...
export async function getStoredSignupOtp(
  emailNorm: string,
): Promise<StoredSignupOtp | null> {
  const redis = getRedis();
  if (!redis) return null;
  ...
  try {
    ...
    return o?.h && typeof o.h === "string" && typeof o.fullName === "string"
      ? o
      : null;
  } catch {
    return null;
...
export async function markOtpResendGate(
  ...
  await redis.setEx(
    redisKeys.otp.resendGate(purpose, normalizedEmail),
    sec,
    "1",
  );
}
export async function registerEmailOtpSendOrReject(
  ...
  const coolIdx = Math.min(
    Math.max(strike - 1, 0),
    OTP_SEND_COOLDOWN_SEC.length - 1,
  );
