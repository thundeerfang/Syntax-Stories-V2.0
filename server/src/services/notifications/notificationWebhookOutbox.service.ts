...
/** Enqueue outbound platform webhook delivery for the dedicated worker. */
...
function parseOutboxMessage(raw: string): NotificationWebhookOutboxMessage | null {
  try {
    ...
    if (!parsed.userId || !parsed.notificationId || !parsed.payload?.id) return null;
...
/** Process one outbox message (used by the dedicated webhook worker). */
export async function processNotificationWebhookOutboxMessage(raw: string): Promise<void> {
