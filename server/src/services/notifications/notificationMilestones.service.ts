...
async function markMilestoneSent(postId: string, metric: string, threshold: number): Promise<void> {
  const redis = getRedis();
  if (!redis) return;
  const key = redisKeys.notifications.milestoneSent(postId, metric, threshold);
...
/** Check engagement milestones after stats update; notify post author once per threshold. */
export async function evaluatePostEngagementMilestones(postId: mongoose.Types.ObjectId): Promise<void> {
  ...
  const authorId =
    authorRaw && typeof authorRaw === 'object' && '_id' in (authorRaw as object)
      ? String((authorRaw as { _id: mongoose.Types.ObjectId })._id)
      : String(authorRaw);
  const username =
    authorRaw && typeof authorRaw === 'object' && 'username' in (authorRaw as object)
      ? String((authorRaw as { username?: string }).username ?? '')
      : '';
  const slug = typeof post.slug === 'string' ? post.slug : '';
  const title = typeof post.title === 'string' ? post.title : 'Your post';
  const href = username && slug ? postHref(username, slug) : '/blogs';

  const repostCount = Math.max(0, (post as { repostCount?: number }).repostCount ?? 0);
  const viewCount = Math.max(0, (post as { viewCount?: number }).viewCount ?? 0);
  const respectCount = Math.max(0, (post as { respectCount?: number }).respectCount ?? 0);
  ...
  for (const c of checks) {
    for (const threshold of c.thresholds) {
      ...
      const sent = await milestoneAlreadySent(String(postId), c.metric, threshold);
      if (sent) continue;
      await markMilestoneSent(String(postId), c.metric, threshold);
      const countLabel = threshold >= 1000 ? `${threshold / 1000}K` : String(threshold);
  ...
  const squadId = (post as { squadId?: mongoose.Types.ObjectId }).squadId;
  if (squadId) {
...
async function notifySquadRespectMilestones(params: {
  ...
  for (const threshold of RESPECT_MILESTONES) {
    ...
    const squad = await SquadModel.findById(params.squadId).select('slug name').lean();
    if (!squad) continue;
...
async function evaluateTrendingNotification(params: {
  ...
  if (params.viewCount < TRENDING_VIEW_MIN || engagement < TRENDING_ENGAGEMENT_MIN) return;
