...
/** Persist notification, fan-out realtime + optional webhook. */
...
export async function listNotificationsForUser(
  ...
  const rows = await NotificationModel.find({ userId: new mongoose.Types.ObjectId(userId) })
    .sort({ createdAt: -1 })
...
export async function updateNotificationPreferences(
  ...
  if (typeof patch.inAppEnabled === 'boolean') prefs.inAppEnabled = patch.inAppEnabled;
  if (typeof patch.milestonesEnabled === 'boolean') prefs.milestonesEnabled = patch.milestonesEnabled;
  if (typeof patch.followingEnabled === 'boolean') prefs.followingEnabled = patch.followingEnabled;
  if (typeof patch.trendingEnabled === 'boolean') prefs.trendingEnabled = patch.trendingEnabled;
  if (typeof patch.settingsEnabled === 'boolean') prefs.settingsEnabled = patch.settingsEnabled;
  if (typeof patch.referralsEnabled === 'boolean') prefs.referralsEnabled = patch.referralsEnabled;
  if (typeof patch.squadsEnabled === 'boolean') prefs.squadsEnabled = patch.squadsEnabled;
  if (typeof patch.categoriesEnabled === 'boolean') prefs.categoriesEnabled = patch.categoriesEnabled;
  if (typeof patch.tagsEnabled === 'boolean') prefs.tagsEnabled = patch.tagsEnabled;
  if (typeof patch.achievementsEnabled === 'boolean') prefs.achievementsEnabled = patch.achievementsEnabled;
  await prefs.save();
  return prefs;
}

export function serializeNotificationPreferences(prefs: IUserNotificationPreferences) {
  return {
