...
export type PlatformNotificationConfigAdmin = PlatformNotificationConfigPublic & {
  webhookSecret: string;
};

async function ensureConfig(): Promise<IPlatformNotificationConfig> {
  let doc = await PlatformNotificationConfigModel.findOne({ singleton: 'default' })
...
/** Active webhook target for outbound delivery (internal). */
export async function getActivePlatformWebhook(): Promise<{
  enabled: boolean;
  url: string;
  secret?: string;
} | null> {
  const doc = await PlatformNotificationConfigModel.findOne({ singleton: 'default' })
...
export async function patchPlatformNotificationConfig(patch: {
  ...
  const doc = await ensureConfig();
  if (typeof patch.webhookEnabled === 'boolean') doc.webhookEnabled = patch.webhookEnabled;
  ...
  await doc.save();
  return serializePlatformNotificationConfig(doc, true) as PlatformNotificationConfigAdmin;
}
