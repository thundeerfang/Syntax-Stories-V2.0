...
/** Resolve up to 10 stack names to catalog rows with iconSlug + iconUrl. */
export async function resolveTechStackNames(names: string[]): Promise<TechStackItemDto[]> {
  const list = names
...
/** Profile read helper — maps stored `stackAndTools` strings to display DTOs. */
export async function enrichStackAndToolsDisplay(raw: unknown): Promise<TechStackItemDto[]> {
