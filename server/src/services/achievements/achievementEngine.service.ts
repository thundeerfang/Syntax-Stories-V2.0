...
function metricValue(
  metric: AchievementProgressItemDto["metric"],
  snapshot: MetricSnapshot,
): number {
...
async function persistProgressRows(
  ...
  const ops = catalog.map((def) => {
    ...
    const locked = Boolean(
      def.unlocksAfter && !unlockedIds.has(def.unlocksAfter),
    );
...
async function tryAcquireUnlockLock(
  userId: string,
...
async function migrateLegacyUnlocks(userId: string): Promise<void> {
  try {
    const oid = new mongoose.Types.ObjectId(userId);
    const existing = await AchievementUnlockModel.countDocuments({
      userId: oid,
    });
    if (existing > 0) return;
    const legacy = await UserAchievementProgressModel.findOne({
      userId: oid,
    }).lean();
    ...
    if (
      legacy.totalPoints != null &&
      (stats.totalAchievementPoints ?? 0) < legacy.totalPoints
    ) {
    ...
    console.warn(
      "[achievements] legacy unlock migration failed:",
      userId,
      String(e),
    );
  }
...
export async function evaluateAchievementsForUser(
  ...
  const affected = options.events
    ? metricsAffectedByEvents(options.events)
    : null;
...
export async function processAchievementEvents(
  ...
  if (!userId || !mongoose.isValidObjectId(userId) || events.length === 0)
    return [];
...
export async function getAchievementsForUser(userId: string): Promise<{
  ...
  const items: AchievementProgressItemDto[] = catalog
    .map((def) => {
