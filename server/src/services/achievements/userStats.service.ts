...
/** Full rebuild from source collections — used on profile_sync and first create. */
export async function syncUserStatsFromSources(
  ...
  const [
    ...
    BlogPostModel.countDocuments({ authorId: oid, status: 'published', deletedAt: null }),
  ...
  const patch = {
    ...
    setupCount:
      user?.mySetup?.filter((item) => item?.label?.trim() || item?.imageUrl?.trim()).length ?? 0,
...
export async function addXpAndPoints(
  ...
  await stats.save();
  return { level: stats.level, leveledUp: stats.level > previousLevel, previousLevel };
}

/** XP thresholds: level N requires N*100 cumulative XP (level 1 at 0). */
