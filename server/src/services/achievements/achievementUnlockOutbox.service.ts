import { createRedisSubscriberClient, getRedis, isRedisAvailable } from '../../config/redis.js';
...
export async function enqueueAchievementUnlockOutbox(
  ...
  if (!redis) return;
  try {
    await redis.lPush(redisKeys.achievements.unlockOutbox, JSON.stringify(payload));
  } catch (e) {
    console.warn('[achievementUnlockOutbox] enqueue failed:', String(e));
  }
}

function parseOutboxMessage(raw: string): AchievementUnlockStreamPayload | null {
  try {
    ...
    if (!parsed.userId || !Array.isArray(parsed.unlocks) || parsed.unlocks.length === 0) {
      return null;
    }
...
async function deliverOutboxPayload(payload: AchievementUnlockStreamPayload): Promise<void> {
  const redis = getRedis();
  if (!redis) return;
  await redis.publish(
...
export async function processAchievementUnlockOutboxMessage(raw: string): Promise<void> {
  const payload = parseOutboxMessage(raw);
  if (!payload) return;
  await deliverOutboxPayload(payload);
}

/** Blocking consumer for `achievements:unlock:outbox` — retries Pub/Sub when publish failed. */
export async function startAchievementUnlockOutboxConsumer(): Promise<void> {
  ...
  if (!client) {
    console.warn('[Achievements] Unlock outbox consumer skipped — Redis unavailable');
    return;
  }
  ...
  void (async () => {
    for (;;) {
      ...
      try {
        const result = await client.brPop(redisKeys.achievements.unlockOutbox, 2);
