...
function sortedUniqueDayBucketsFromDates(dates: readonly Date[], now: Date): string[] {
...
/** Full ZSET + HASH rebuild from Mongo (§31 reconciliation). */
...
async function appendReadDayZsetNxAndTrim(
  ...
  const trimMax = readDaysZsetTrimExclusiveMaxArg(readDaysTrimMinRetainMsUtc(now));
  await redis.zRemRangeByScore(zKey, '-inf', trimMax);
}

/**
 * Best-effort Redis mirror after Mongo `BlogReadDay` upsert: **`user:{id}:readDays` ZSET** ([F.4]) +
 * **`user:{id}:streak:daily` HASH** (§33 Lua). Mongo remains SoT.
 */
