...
export function isMongoStorageError(err: unknown): boolean {
  ...
  if (e.name === 'MongoServerError' && /space|quota|full/i.test(msg)) return true;
  return false;
}

export function isEnospcError(err: unknown): boolean {
  return !!(err && typeof err === 'object' && (err as NodeJS.ErrnoException).code === 'ENOSPC');
}
...
async function readRedisState(): Promise<StorageStatus | null> {
  ...
  return {
    blocked: true,
    reason: r === 'mongo_quota' || r === 'disk_full' || r === 'manual' ? r : 'manual',
    since: since ?? null,
  };
}

async function writeRedisState(reason: StorageBlockReason, since: string): Promise<void> {
  const redis = getRedis();
  if (redis) {
...
export async function getStorageStatus(): Promise<StorageStatus> {
  ...
  if (env.STORAGE_FULL_MODE === 'force_on') {
    return { blocked: true, reason: 'manual', since: memorySince ?? new Date().toISOString() };
  }
...
async function sendStorageAlert(reason: StorageBlockReason, since: string): Promise<void> {
  if (!isAuthEmailConfigured()) {
    console.warn('[storageGuard] Email not configured — skipping alert for', reason);
    return;
  }
  ...
  if (redis) {
    ...
    if (alerted === '1') return;
    await redis.set(redisKeys.platform.storageAlerted, '1', { EX: ALERT_THROTTLE_SEC });
  }
...
export async function activateStorageBlock(reason: StorageBlockReason): Promise<void> {
...
export async function runStorageProbe(): Promise<void> {
  ...
  if (pct >= env.UPLOADS_DISK_WARN_PCT && pct < env.UPLOADS_DISK_BLOCK_PCT) {
    console.warn(`[storageGuard] Uploads disk at ${pct}% (warn threshold ${env.UPLOADS_DISK_WARN_PCT}%)`);
    return;
  }

  if (status.blocked && status.reason === 'disk_full' && pct < env.UPLOADS_DISK_WARN_PCT) {
    await deactivateStorageBlock();
    console.log(`[storageGuard] Disk recovered (${pct}%) — public writes re-enabled`);
  }
}
...
export function startStorageGuardProbe(): void {
  ...
  probeTimer = setInterval(() => void runStorageProbe(), env.STORAGE_PROBE_INTERVAL_MS);
  probeTimer.unref?.();
}

/** Paths that stay writable/readable when storage is blocked. */
export function isStorageGateExempt(method: string, originalUrl: string): boolean {
