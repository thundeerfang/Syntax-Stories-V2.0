...
export async function loadSearchIndexFromRedis(key: string): Promise<SearchIndexDoc[] | null> {
  const redis = getRedis();
...
async function buildBlogsRecentIndexDocs(): Promise<SearchIndexDoc[]> {
  ...
  const authors = await UserModel.find({ _id: { $in: authorIds } }).select('username').lean();
...
export async function rebuildSearchIndexes(): Promise<void> {
  ...
  await Promise.all([
    ...
    writeIndex(redisKeys.search.index.categories, categoriesToIndexDocs(categories)),
...
export async function startSearchIndexWorker(): Promise<void> {
  ...
  setInterval(() => void runSearchIndexRebuild(), 5 * 60_000);
}
