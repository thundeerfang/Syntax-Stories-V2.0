...
export function sendImageMasterError(
  res: Response,
  err: ImageMasterError,
): void {
  res
    .status(imageMasterErrorStatus(err.code))
...
export function parseCropCoords(
  body: Record<string, unknown>,
): ImageCropCoords | null {
...
export async function applyImageDelivery(
  ...
  if (spec.format === "png") {
    ...
    out = await image
      .jpeg({ quality: spec.jpegQuality ?? 82, mozjpeg: true })
      .toBuffer();
    ext = ".jpg";
  }
