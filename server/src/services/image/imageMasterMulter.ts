...
export function createImageMasterMulter(
...
export function runImageMasterUpload(
  ...
  const maxMb = Math.round(
    IMAGE_MASTER_PROFILES[profile].maxInputBytes / (1024 * 1024),
  );
  return (req: Request, res: Response, next: NextFunction): void => {
    upload.single(field)(req, res, (err: unknown) => {
      ...
      const msg =
        err instanceof Error ? err.message : IMAGE_UPLOAD_REJECT_MESSAGE;
