...
/** One-time migration: existing User.referredByUserId → ReferralConversion + UserStats. */
export async function backfillReferralConversionsOnce(): Promise<void> {
  ...
  console.info(`[referral] Backfilling ${referred.length} legacy referral rows…`);
  ...
  for (const u of referred) {
    ...
    const exists = await ReferralConversionModel.findOne({ refereeId }).select('_id').lean();
    if (exists) continue;
    ...
    try {
      await ReferralConversionModel.create({
        ...
        refereeId,
        status: 'verified',
        source: (u as { referralSource?: string }).referralSource?.slice(0, 32) ?? 'legacy',
  ...
  const counts = await ReferralConversionModel.aggregate<{ _id: unknown; n: number }>([
