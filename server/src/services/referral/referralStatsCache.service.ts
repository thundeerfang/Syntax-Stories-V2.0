...
export async function invalidateReferralUserStatsCache(userId: string): Promise<void> {
...
export async function addReferralLeaderboardScore(referrerId: string, delta = 1): Promise<void> {
  if (!isRedisAvailable() || delta <= 0) return;
  const redis = getRedis();
  if (!redis) return;
  try {
    await redis.zIncrBy(redisKeys.invite.leaderboard, delta, referrerId);
    await redis.zRemRangeByRank(redisKeys.invite.leaderboard, 0, -(LEADERBOARD_MAX + 1));
  } catch (e) {
    console.warn('[referral-leaderboard]', String(e));
  }
}

export type ReferralLeaderboardEntry = { userId: string; score: number; rank: number };
...
export async function getReferralLeaderboardTop(
  ...
  try {
    const rows = await redis.zRangeWithScores(redisKeys.invite.leaderboard, 0, n - 1, {
      REV: true,
    });
