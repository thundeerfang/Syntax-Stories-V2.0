...
import {
  ...
  type ReferralStatus,
} from "../../models/ReferralConversion.js";
...
export async function createReferralConversion(args: {
  ...
  try {
    ...
    if (!dup) throw e;
    const existing = await ReferralConversionModel.findOne({
      refereeId: refereeOid,
    }).lean();
    return {
      conversionId: existing?._id ? String(existing._id) : "",
      created: false,
    };
  }
}
export async function markReferralRejected(
  ...
  const doc = await ReferralConversionModel.findById(conversionId)
    .select("referrerId")
    .lean();
  if (doc?.referrerId) {
    await UserStatsModel.updateOne(
