...
export async function resumeRespectContributionsForPost(
  ...
  try {
    await session.withTransaction(async () => {
      const count = await BlogRespectModel.countDocuments({ postId }).session(
        session,
      );
...
export async function setRespectDesiredState(params: {
  ...
  if (viewerOid.equals(params.authorId)) {
    const edge = await BlogRespectModel.findOne({
      userId: viewerOid,
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      if (params.respecting) {
        ...
        await BlogRespectModel.deleteOne({ _id: existing._id }).session(
          session,
        );
        await BlogPostModel.updateOne(
      ...
      const a = await UserModel.findById(params.authorId)
        ...
        .select("blogRespectReceivedCount")
        .lean();
      authorBlogRespectReceivedCount = Math.max(
        0,
        a?.blogRespectReceivedCount ?? 0,
      );
    });
    return {
      ok: true,
      respecting,
      respectCount,
      authorBlogRespectReceivedCount,
      newRespectEdge,
    };
  } finally {
    await session.endSession();
...
export async function viewerRespectStatesForPosts(
  ...
  const rows = await BlogRespectModel.find({
    userId: viewerOid,
    postId: { $in: oids },
  })
    .select("postId")
...
export async function deleteAllRespectsForPost(
  postId: mongoose.Types.ObjectId,
): Promise<void> {
