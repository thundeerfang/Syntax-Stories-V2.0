...
export async function setRepostDesiredState(params: {
  ...
  if (viewerOid.equals(params.authorId)) {
    const edge = await BlogRepostModel.findOne({
      userId: viewerOid,
  ...
  if (
    !(postCheck.authorId as mongoose.Types.ObjectId).equals(params.authorId)
  ) {
    return { ok: false, code: "NOT_ELIGIBLE" };
  }
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      const edgeAfter = await BlogRepostModel.findOne({
        userId: viewerOid,
        postId: params.postId,
      })
        .session(session)
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      if (params.bookmarked) {
        if (!existing) {
          try {
            ...
            if (
              raced &&
              targetGroupId &&
              String(raced.groupId ?? "") !== String(targetGroupId)
            ) {
          ...
          targetGroupId &&
          String(existing.groupId ?? "") !== String(targetGroupId)
        ) {
          await BlogBookmarkModel.updateOne(
            { _id: existing._id },
            { $set: { groupId: targetGroupId } },
        ...
        await BlogBookmarkModel.deleteOne({ _id: existing._id }).session(
          session,
        );
        await BlogPostModel.updateOne(
...
export async function viewerRepostStatesForPosts(
  ...
  const rows = await BlogRepostModel.find({
    userId: viewerOid,
    postId: { $in: oids },
  })
    .select("postId")
...
export async function viewerBookmarkStatesForPosts(
  ...
  if (!oids.length) return out;
  const viewerOid = new mongoose.Types.ObjectId(viewerUserId);
  const rows = await BlogBookmarkModel.find({
    userId: viewerOid,
    postId: { $in: oids },
  })
    .select("postId")
...
export async function suspendRepostBookmarkContributionsForPost(
  ...
  await BlogPostModel.updateOne(
    { _id: postId },
    { $set: { repostCount: 0, bookmarkCount: 0 } },
  );
}
...
export async function deleteAllRepostsForPost(
  postId: mongoose.Types.ObjectId,
): Promise<void> {
  await BlogRepostModel.deleteMany({ postId });
}
export async function deleteAllBookmarksForPost(
  postId: mongoose.Types.ObjectId,
): Promise<void> {
