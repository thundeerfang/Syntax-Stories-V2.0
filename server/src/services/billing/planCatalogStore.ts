...
const DEFAULT_PLANS: Array<{
  ...
  },
  {
    ...
    features: [
      "Everything in Pro",
      "Higher limits where it matters",
      "Faster support",
    ],
    featured: true,
    badge: "Most popular",
...
async function loadRows(activeOnly: boolean) {
  ...
  return BillingPlanCatalogModel.find(filter)
    .sort({ sortOrder: 1, key: 1 })
    .lean();
}
export async function listBillingPlanCatalogFromStore(): Promise<
...
export async function getBillingPlanByKeyFromStore(
  ...
  await ensureDefaultBillingPlans();
  const row = await BillingPlanCatalogModel.findOne({
    key,
    active: true,
  }).lean();
  return row ? mapPublic(row as PlanRow) : undefined;
}
export async function getAvailableBillingPlanKeysFromStore(): Promise<
  PaidPlanKey[]
> {
  await ensureDefaultBillingPlans();
...
export async function createBillingPlanInStore(input: {
  ...
  const exists = await BillingPlanCatalogModel.findOne({
    key: input.key,
  }).lean();
  if (exists) {
...
export async function updateBillingPlanInStore(
  ...
  if (patch.amountDisplay !== undefined)
    doc.amountDisplay = patch.amountDisplay;
