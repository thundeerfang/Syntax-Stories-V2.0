...
function mapStripeStatus(
  ...
  if (allowed.has(s as ISubscription["status"]))
    return s as ISubscription["status"];
  return "canceled";
}
...
export async function applyStripeSubscription(
  ...
  return withBillingLock(subId, async () => {
    ...
    if (!user?._id) {
      console.warn(
        "[billing] applyStripeSubscription: no user for customer",
        customerId,
      );
      return { userId: null, updated: false };
    }
    ...
    const cps = fresh.current_period_start
      ...
      : undefined;
    const cpe = fresh.current_period_end
      ? new Date(fresh.current_period_end * 1000)
      : undefined;
    const trialEnd = fresh.trial_end
      ? new Date(fresh.trial_end * 1000)
      : undefined;
    ...
    const same =
      ...
      (subDoc.currentPeriodEnd?.getTime() ?? 0) ===
        (patch.currentPeriodEnd?.getTime() ?? 0) &&
      (graceUntil?.getTime() ?? null) ===
        (subDoc.graceUntil?.getTime() ?? null);
    if (same) {
