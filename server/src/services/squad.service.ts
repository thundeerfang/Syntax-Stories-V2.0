...
export async function allocateUniqueSquadSlug(
  handleBase: string,
): Promise<string> {
...
> {
  ...
  if (!role)
    return {
      ok: false,
      message: "Join this squad before posting or sharing here",
      status: 403,
    };
...
> {
  ...
  if (!params.userId) {
    return {
      ok: false,
      message: "Sign in to view this private squad",
      status: 403,
    };
  }
  const role = await getSquadMemberRole(
    params.squad._id as mongoose.Types.ObjectId,
    params.userId,
  );
  if (!role)
    return {
...
export async function createSquadWithAdmin(input: CreateSquadInput): Promise<{
  squad: ISquad;
  inviteToken?: string;
}> {
  const slug = await allocateUniqueSquadSlug(
    squadHandleSeedFromName(input.name),
  );
  ...
  const created = await SquadModel.create([
    {
      ...
      ...(input.visibility === "public" && input.category
  ...
  const plain = doc.toObject() as ISquad & {
    inviteToken?: string;
  };
...
> {
  ...
  try {
    await SquadMemberModel.create([
      { squadId, userId: uid, role: "member" as const },
    ]);
    ...
    if (code === 11000)
      return { ok: false, message: "Already a member", status: 409 };
    throw e;
  }
}
...
> {
  const squad = await SquadModel.findById(params.squadId)
    .select("invitePermission")
    .lean();
  if (!squad) return { ok: false, message: "Squad not found", status: 404 };
  const actorRole = await getSquadMemberRole(
    params.squadId,
    params.actorUserId,
  );
  if (!actorRole) {
  ...
  try {
    ...
    if (code === 11000)
      return { ok: false, message: "User is already a member", status: 409 };
    throw e;
  }
}
...
> {
  ...
  if (!post)
    return {
      ok: false,
      message: "Post not found or not shareable",
      status: 404,
    };
  const homeSquadId = (
    post as {
  ...
  try {
    ...
    if (code === 11000)
      return {
        ok: false,
        message: "This article is already in the squad feed",
        status: 409,
      };
    throw e;
  }
...
> {
  ...
  if (!actorRole || actorRole !== "admin") {
    return {
      ok: false,
      message: "Only squad admins can change roles",
      status: 403,
    };
  }
  if (params.role === "admin") {
    return {
      ok: false,
      message: "Transferring admin is not supported yet",
      status: 400,
    };
  }
  const uname = params.targetUsername.trim().toLowerCase();
  ...
  if (String(squad?.createdById) === String(user._id)) {
    return {
      ok: false,
      message: "Cannot demote the squad creator",
      status: 400,
    };
  }
...
async function deleteSquadCascade(
  squadId: mongoose.Types.ObjectId,
): Promise<void> {
...
> {
  ...
  return {
    ok: false,
    message: "This post is not in this squad feed",
    status: 400,
  };
}
...
> {
  ...
  if (role !== "admin") {
    return {
      ok: false,
      message: "Only squad admins can pin posts",
      status: 403,
    };
  }
  const inFeed = await assertPostInSquadFeed({
    squadId: params.squadId,
    postId: params.postId,
  });
  ...
  try {
    ...
    if (code === 11000)
      return { ok: false, message: "This post is already pinned", status: 409 };
    throw e;
  }
}
...
> {
  ...
  if (role !== "admin") {
    return {
      ok: false,
      message: "Only squad admins can unpin posts",
      status: 403,
    };
  }
  ...
  if (r.deletedCount === 0)
    return { ok: false, message: "Pin not found", status: 404 };
  return { ok: true };
}
export async function listPinnedPostIdsForSquad(
  squadId: mongoose.Types.ObjectId,
): Promise<Set<string>> {
  const rows = await SquadPinnedPostModel.find({ squadId })
    .select("postId")
    .lean();
  return new Set(
    rows.map((r) =>
...
> {
  ...
  if (row.role === "admin") {
    const [adminCount, total] = await Promise.all([
      SquadMemberModel.countDocuments({
        squadId: params.squadId,
        role: "admin",
      }),
  ...
  const remaining = await SquadMemberModel.countDocuments({
    squadId: params.squadId,
  });
  if (remaining === 0) {
...
> {
  ...
  if (role !== "admin") {
    return {
      ok: false,
      message: "Only squad admins can delete this squad",
      status: 403,
    };
  }
  await deleteSquadCascade(params.squadId);
...
export async function getSquadFeedStats(
  ...
  const agg = await BlogPostModel.aggregate<{
    postCount: number;
    viewCount: number;
  }>([
    {
...
> {
  ...
  if (!memberRow)
    return { ok: false, message: "Not a member of this squad", status: 404 };
  ...
  return {
    ...
    stats: {
      joinedAt:
        memberRow.createdAt?.toISOString?.() ?? new Date().toISOString(),
...
> {
  ...
  if (actorRole !== "admin") {
    return {
      ok: false,
      message: "Only squad admins can remove members",
      status: 403,
    };
  }
  const uname = params.targetUsername.trim().toLowerCase();
  ...
  if (String(user._id) === String(params.actorUserId)) {
    return {
      ok: false,
      message: "Use leave squad to remove yourself",
      status: 400,
    };
  }
  const squad = await SquadModel.findById(params.squadId)
  ...
  if (String(squad?.createdById) === String(user._id)) {
    return {
      ok: false,
      message: "Cannot remove the squad creator",
      status: 400,
    };
  }
  const row = await SquadMemberModel.findOne({
  ...
  await SquadMemberModel.deleteOne({
    squadId: params.squadId,
    userId: user._id,
  });
  await SquadModel.updateOne(
