...
export async function publishBlogPostStatsSnapshot(
  postId: mongoose.Types.ObjectId,
): Promise<void> {
  const post = await BlogPostModel.findById(postId)
    .select(
      "respectCount repostCount bookmarkCount commentCount viewCount slug authorId",
    )
  ...
  const payload: BlogPostStatsPayload = {
    ...
    stats: {
      ...
      bookmarkCount: normalizeCounter(
        (
          post as {
            bookmarkCount?: number;
          }
  ...
  try {
    ...
    console.warn(
      JSON.stringify({ event: "blog_stats_publish_failed", err: String(e) }),
    );
  }
  void import("./notifications/notificationMilestones.service.js").then(
    ({ evaluatePostEngagementMilestones }) =>
