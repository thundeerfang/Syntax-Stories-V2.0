...
async function handleGamificationMessage(message: GamificationStreamMessage): Promise<void> {
...
export async function startGamificationWorker(): Promise<void> {
  ...
  if (!sub) {
    console.warn('[Gamification] Stream worker skipped — Redis subscriber unavailable');
    return;
  }
      ...
      for (const row of pending) {
        try {
          if (row.type === 'referral.process_attribution') {
            const conversionId = (row.payload as { conversionId?: string }).conversionId;
