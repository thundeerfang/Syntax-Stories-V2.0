...
/** Fraud check → verify (if qualify mode) → notify → XP → achievements → leaderboard. */
export async function processReferralAttribution(conversionId: string): Promise<void> {
...
export async function verifyAndRewardReferral(conversionId: string): Promise<void> {
...
/** Call when referee completes profile (REFERRAL_QUALIFY_MODE=profile). */
export async function qualifyReferralByRefereeId(refereeUserId: string): Promise<void> {
