...
export async function initQrLogin(_req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!redis) {
      res
        .status(503)
        .json({ message: "Service temporarily unavailable", success: false });
      return;
    }
  ...
  try {
    ...
    if (!existing) {
      res
        .status(400)
        .json({
          message: "QR login session not found or expired",
          success: false,
        });
      return;
    }
...
export async function pollQrLogin(req: Request, res: Response): Promise<void> {
  try {
    ...
    await redis.del(key);
    await logSecurityEvent(userId, "session_created", req, {
      source: "qr_login",
    });
