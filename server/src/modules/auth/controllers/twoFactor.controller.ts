...
async function storeTwoFactorSetupSecret(
  userId: string,
  secret: string,
): Promise<void> {
  const redis = getRedis();
...
export async function verifyTwoFactorLogin(
  ...
  try {
    const { challengeToken, token } = req.body as {
    ...
    if (!challenge?.userId) {
      res
        .status(400)
        .json({ success: false, message: "Invalid or expired 2FA challenge" });
      return;
    }
    const dbUser = await UserModel.findById(challenge.userId).select(
      "+twoFactorSecret",
    );
    ...
    const isStaff =
      dbUser.staffRole === "editor" || dbUser.staffRole === "admin";
    if (isStaff) {
...
export async function enableTwoFactor(
  ...
  try {
    ...
    if (!token) {
      res
        .status(400)
        .json({ message: "2FA token is required", success: false });
      return;
    }
    ...
    if (!setupSecret) {
      res
        .status(400)
        .json({
          message: "2FA setup not initialized or expired",
          success: false,
        });
      return;
    }
    ...
    void writeAuditLog(req, AuditAction.TWOFa_ENABLED, {
...
export async function disableTwoFactor(
  ...
  try {
    ...
    void writeAuditLog(req, AuditAction.TWOFa_DISABLED, {
      actorId: String(user._id),
    });
    res
      .status(200)
