...
import { invalidateAllStaffAdminPermissionCaches } from '../services/rbac.service.js';
...
export async function postAccessResource(req: Request, res: Response): Promise<void> {
  const body = req.body as { slug?: string; displayName?: string; description?: string; sortOrder?: number };
  const slug = body.slug?.trim().toLowerCase();
  const displayName = body.displayName?.trim();
  if (!slug || !SLUG_RE.test(slug) || !displayName) {
    sendAdminError(res, 400, 'VALIDATION_ERROR', 'Valid slug and displayName are required');
    return;
  }
...
export async function patchAccessResource(req: Request, res: Response): Promise<void> {
  ...
  const body = req.body as { displayName?: string; description?: string | null; sortOrder?: number };
...
export async function postAccessResourceRestore(req: Request, res: Response): Promise<void> {
  ...
  const clash = await AdminResourceModel.findOne({ slug: doc.slug, deletedAt: null, _id: { $ne: doc._id } }).lean();
  if (clash) {
...
export async function postAccessActionRestore(req: Request, res: Response): Promise<void> {
  ...
  const clash = await AdminActionTypeModel.findOne({ slug: doc.slug, deletedAt: null, _id: { $ne: doc._id } }).lean();
  if (clash) {
...
export async function postAccessScopeTypeRestore(req: Request, res: Response): Promise<void> {
  ...
  const clash = await AdminScopeTypeModel.findOne({ slug: doc.slug, deletedAt: null, _id: { $ne: doc._id } }).lean();
  if (clash) {
...
export async function postAccessPermissionRestore(req: Request, res: Response): Promise<void> {
  ...
  const clash = await AdminAccessPermissionModel.findOne({ key: doc.key, deletedAt: null, _id: { $ne: doc._id } }).lean();
  if (clash) {
