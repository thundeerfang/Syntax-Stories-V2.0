...
/**
 * Catalog row for a management permission (resource / action / type + stable key).
 * Role documents store `permissions: string[]` as keys.
 */
export interface IAdminAccessPermission extends Document {
  /** Stable key, e.g. `user:list`. */
  key: string;
  resource: string;
  action: string;
  type: string;
...
const AdminAccessPermissionSchema = new Schema<IAdminAccessPermission>(
  {
    ...
    type: { type: String, required: true, trim: true, maxlength: 40, default: 'management' },
...
AdminAccessPermissionSchema.index({ key: 1 }, { unique: true, partialFilterExpression: { deletedAt: null } });
...
export const AdminAccessPermissionModel: Model<IAdminAccessPermission> =
  ...
  mongoose.model<IAdminAccessPermission>('admin_access_permissions', AdminAccessPermissionSchema);
