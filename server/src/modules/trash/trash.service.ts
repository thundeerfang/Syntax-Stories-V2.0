...
export async function listBlogTrash(page: number, pageSize: number) {
  ...
  const data = rows.map((r) => {
    const a = r.authorId as unknown as { username?: string; fullName?: string; email?: string } | null;
    return {
      _id: String(r._id),
      title: r.title,
      slug: r.slug,
...
export async function restoreBlogPostAsAdmin(postId: string, actorId: string, req: Request): Promise<void> {
...
export async function restoreUserAsAdmin(userId: string, actorId: string, req: Request): Promise<void> {
