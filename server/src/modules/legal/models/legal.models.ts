...
const legalPolicySchema = new Schema(
  {
    ...
    bodyFormat: { type: String, enum: ['markdown', 'mdx', 'richtext'], default: 'markdown' },
    status: {
      type: String,
    ...
    region: { type: String, enum: ['global', 'EU', 'IN', 'US'], default: 'global' },
...
legalPolicySchema.index(
  ...
  { unique: true, partialFilterExpression: { deletedAt: null, tenantId: null, productId: null } }
);
legalPolicySchema.index({ slug: 1 }, { unique: true, partialFilterExpression: { deletedAt: null } });
...
const legalPolicyRevisionSchema = new Schema(
  {
    ...
    policyId: { type: Schema.Types.ObjectId, ref: 'LegalPolicy', required: true },
    kind: { type: String, enum: ['terms', 'privacy', 'udd'], required: true },
    ...
    status: { type: String, enum: ['approved', 'published', 'superseded'], required: true },
    authorId: { type: Schema.Types.ObjectId, ref: 'User', required: true },
    reviewedById: { type: Schema.Types.ObjectId, ref: 'User' },
    approvedById: { type: Schema.Types.ObjectId, ref: 'User' },
    approvedAt: { type: Date },
  ...
  { timestamps: { createdAt: true, updatedAt: false }, collection: 'legal_policy_revisions' }
);
...
userLegalAcceptanceSchema.index({ userId: 1, policyKind: 1, revisionId: 1 }, { unique: true });
...
const dataDeletionRequestSchema = new Schema(
  {
    ...
    /** Optional note from user at request time (§17.5 body.message). */
...
const legalPreviewTokenSchema = new Schema(
  ...
  { timestamps: { createdAt: true, updatedAt: false }, collection: 'legal_preview_tokens' }
);
...
legalIdempotencyKeySchema.index({ createdAt: 1 }, { expireAfterSeconds: 72 * 3600 });
...
const legalAcceptIntentSchema = new Schema(
  ...
  { timestamps: { createdAt: true, updatedAt: false }, collection: 'legal_accept_intents' }
);

legalAcceptIntentSchema.index({ expiresAt: 1 }, { expireAfterSeconds: 0 });

export type LegalPolicyDoc = InferSchemaType<typeof legalPolicySchema> & { _id: mongoose.Types.ObjectId };
export type LegalPolicyRevisionDoc = InferSchemaType<typeof legalPolicyRevisionSchema> & {
  _id: mongoose.Types.ObjectId;
};
export type UserLegalAcceptanceDoc = InferSchemaType<typeof userLegalAcceptanceSchema> & {
  _id: mongoose.Types.ObjectId;
};
export type DataDeletionRequestDoc = InferSchemaType<typeof dataDeletionRequestSchema> & {
  _id: mongoose.Types.ObjectId;
};

export const LegalPolicyModel: Model<LegalPolicyDoc> =
  mongoose.models.LegalPolicy ?? mongoose.model<LegalPolicyDoc>('LegalPolicy', legalPolicySchema);
export const LegalPolicyRevisionModel: Model<LegalPolicyRevisionDoc> =
  mongoose.models.LegalPolicyRevision ??
  mongoose.model<LegalPolicyRevisionDoc>('LegalPolicyRevision', legalPolicyRevisionSchema);
export const UserLegalAcceptanceModel: Model<UserLegalAcceptanceDoc> =
  mongoose.models.UserLegalAcceptance ??
  mongoose.model<UserLegalAcceptanceDoc>('UserLegalAcceptance', userLegalAcceptanceSchema);
export const DataDeletionRequestModel: Model<DataDeletionRequestDoc> =
  mongoose.models.DataDeletionRequest ??
  mongoose.model<DataDeletionRequestDoc>('DataDeletionRequest', dataDeletionRequestSchema);
export const LegalPreviewTokenModel =
  mongoose.models.LegalPreviewToken ?? mongoose.model('LegalPreviewToken', legalPreviewTokenSchema);
export const LegalIdempotencyKeyModel =
  mongoose.models.LegalIdempotencyKey ?? mongoose.model('LegalIdempotencyKey', legalIdempotencyKeySchema);
export const LegalAcceptIntentModel =
  mongoose.models.LegalAcceptIntent ?? mongoose.model('LegalAcceptIntent', legalAcceptIntentSchema);
