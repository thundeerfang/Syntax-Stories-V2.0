...
import { legalKindParamSchema, type AcceptPolicyKind, type LegalKind } from './legalKinds.js';
import { getPublishedPolicyResponse, getRequiredVersionsForUser } from './legalPolicyQueries.js';
...
const deletionBodySchema = z.object({ message: z.string().max(2000).optional() });

function userId(req: Request): string {
  return (req as Request & { user: AuthUser }).user._id;
}

export async function getPublishedPolicyByKind(req: Request, res: Response): Promise<void> {
  const parse = legalKindParamSchema.safeParse(req.params.kind);
  if (!parse.success) {
    res.status(400).json({ ok: false, code: 'LEGAL_INVALID_KIND', message: 'Invalid kind' });
    return;
  }
  const kind = parse.data;
  const data = await getPublishedPolicyResponse(kind);
  if (!data) {
    res.status(404).json({ ok: false, code: 'LEGAL_NOT_FOUND', message: 'No published policy' });
    return;
  }
...
export async function postAcceptIntent(req: Request, res: Response): Promise<void> {
  ...
  const data = await getPublishedPolicyResponse(body.data.policyKind as LegalKind);
  if (!data || data.revision.revisionId !== body.data.revisionId) {
    res.status(409).json({ ok: false, code: 'LEGAL_REVISION_NOT_CURRENT', message: 'Revision is not the current published document.' });
    return;
  }
  ...
  res.status(200).json({ ok: true, nonce, expiresAt: expiresAt.toISOString(), revisionId: body.data.revisionId });
}
...
export async function postAccept(req: Request, res: Response): Promise<void> {
  ...
  if (!parsed.success) {
    res.status(422).json({ ok: false, message: 'Validation failed', issues: parsed.error.flatten() });
    return;
  }
  const b = parsed.data;
  await completeWithIdempotency(req, res, 'POST /legal/accept', uid, req.body, async () => {
    ...
    if (!intent || intent.expiresAt <= now) {
      return {
        status: 401,
        body: { ok: false, code: 'LEGAL_ACCEPT_INTENT_INVALID', message: 'Missing or expired accept intent (replay protection).' },
      };
    }

    const pub = await getPublishedPolicyResponse(b.policyKind as LegalKind);
    if (!pub) {
      return { status: 404, body: { ok: false, code: 'LEGAL_NOT_FOUND', message: 'No published policy' } };
    }
    const display = pub.revision;
    if (display.revisionId !== b.revisionId || display.version !== b.version) {
      return {
        status: 409,
        body: {
          ok: false,
    ...
    if (b.contentHash && b.contentHash !== display.contentHash) {
      return {
        status: 409,
        body: { ok: false, code: 'LEGAL_HASH_MISMATCH', message: 'Content hash does not match server revision.' },
      };
    }
    if (!verifyRevisionHash({ title: display.title, summary: display.summary, body: display.body, contentHash: display.contentHash })) {
      return { status: 500, body: { ok: false, message: 'Server integrity error for revision hash.' } };
    }
    ...
    try {
      ...
      if ((e as { code?: number }).code === 11_000) {
...
export async function getMeLegalStatus(req: Request, res: Response): Promise<void> {
  const uid = userId(req);
  const now = await getLegalDbNow();
...
export async function postDataDeletionRequest(req: Request, res: Response): Promise<void> {
  const uid = userId(req);
  ...
  await completeWithIdempotency(req, res, 'POST /legal/data-deletion-requests', uid, req.body ?? {}, async () => {
    const now = await getLegalDbNow();
    ...
    if (open) {
      return {
        status: 409,
        body: { ok: false, code: 'LEGAL_DELETION_OPEN', message: 'A deletion request is already open.' },
      };
    }
    ...
    if (recent) {
      const retryAfterSec = Math.ceil(
        (recent.requestedAt.getTime() + cooldownHours * 3600 * 1000 - now.getTime()) / 1000
      );
      return {
        status: 429,
        body: { ok: false, code: 'LEGAL_DELETION_COOLDOWN', retryAfterSec: Math.max(1, retryAfterSec) },
      };
    }
    ...
    legalJobQueue.push({ type: 'DATA_DELETION_PROCESS', requestId: doc._id.toString() });
    return {
      status: 201,
      body: {
        ok: true,
...
export async function listMyDeletionRequests(req: Request, res: Response): Promise<void> {
  const uid = userId(req);
  const limit = Math.min(50, Math.max(1, Number.parseInt(String(req.query.limit ?? '20'), 10) || 20));
  const items = await DataDeletionRequestModel.find({ userId: new mongoose.Types.ObjectId(uid) })
    .sort({ requestedAt: -1 })
    .limit(limit)
...
const patchPolicySchema = z.object({
  action: z.enum(['save_draft', 'submit_review', 'approve', 'publish', 'archive']),
...
export async function adminListPolicies(_req: Request, res: Response): Promise<void> {
  const items = await LegalPolicyModel.find({ deletedAt: null, tenantId: null, productId: null })
...
export async function adminGetPolicy(req: Request, res: Response): Promise<void> {
...
export async function adminListRevisions(req: Request, res: Response): Promise<void> {
  const limit = Math.min(100, Math.max(1, Number.parseInt(String(req.query.limit ?? '50'), 10) || 50));
  const policyId = String(req.params.policyId);
  const revs = await LegalPolicyRevisionModel.find({ policyId: new mongoose.Types.ObjectId(policyId) })
    .sort({ version: -1 })
    .limit(limit)
    .lean();
...
export async function adminPatchPolicy(req: Request, res: Response): Promise<void> {
  const uid = (req as Request & { user: AuthUser }).user._id;
  const parsed = patchPolicySchema.safeParse(req.body);
  if (!parsed.success) {
    res.status(422).json({ ok: false, message: 'Invalid body', issues: parsed.error.flatten() });
    return;
  }
  const b = parsed.data;
  const policy = await LegalPolicyModel.findById(String(req.params.policyId)).exec();
  if (!policy) {
    res.status(404).json({ ok: false, message: 'Not found' });
    return;
  }
  ...
  if (b.action === 'publish') {
    await completeWithIdempotency(req, res, 'PATCH /admin/legal/publish', uid, req.body, async () => {
      if (policy.status !== 'approved' && policy.status !== 'published') {
        return { status: 409, body: { ok: false, message: 'Publish requires approved or published policy' } };
      }
      try {
        assertSlugMatchesKind(policy.kind as LegalKind, policy.slug);
      } catch {
        return { status: 400, body: { ok: false, code: 'SLUG_KIND_MISMATCH', message: 'Slug does not match kind' } };
      }
      ...
      const effectiveAt =
        b.effectiveAt !== undefined && b.effectiveAt !== null ? new Date(b.effectiveAt) : policy.effectiveAt;
      ...
      const pub = await publishPolicyRevision({
        ...
        draft: {
          ...
          companyAddress: b.companyAddress ?? policy.companyAddress ?? undefined,
          publishedById: new mongoose.Types.ObjectId(uid),
          expectedPublishedRevisionId: b.expectedPublishedRevisionId,
        },
      });
      if (!pub.ok) {
        return { status: 409, body: { ok: false, code: pub.code, message: pub.message } };
      }
  ...
  if (b.action === 'save_draft') {
    ...
    if (b.effectiveAt !== undefined) policy.effectiveAt = b.effectiveAt ? new Date(b.effectiveAt) : undefined;
    ...
    if (b.companyAddress !== undefined) policy.companyAddress = b.companyAddress;
    if (policy.status === 'archived') {
      res.status(409).json({ ok: false, message: 'Cannot edit archived policy' });
      return;
    }
    await policy.save();
