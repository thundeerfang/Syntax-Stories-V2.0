...
async function processPublishPurge(msg: Extract<LegalJobMessage, { type: 'LEGAL_POLICY_PUBLISHED' }>): Promise<void> {
  logJob(msg);
  // CDN purge hooks: set env LEGAL_CDN_PURGE_WEBHOOK_URL to integrate later.
}

async function processReconsentFanout(msg: Extract<LegalJobMessage, { type: 'LEGAL_RECONSENT_FANOUT' }>): Promise<void> {
  logJob(msg);
...
async function processDeletionPipeline(msg: Extract<LegalJobMessage, { type: 'DATA_DELETION_PROCESS' }>): Promise<void> {
  const maxSteps = 20;
  for (let i = 0; i < maxSteps; i++) {
    ...
    const steps = (doc.executionSteps as { step: string; status: string; at?: Date }[] | undefined) ?? [];
    const next = steps.find((s: { status: string }) => s.status === 'pending' || s.status === 'failed');
    if (!next) {
