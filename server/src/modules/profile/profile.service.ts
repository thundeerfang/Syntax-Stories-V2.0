...
async function applyProfileUpdate(
  ...
  const expectedVersion =
    typeof rawEv === "number" && Number.isInteger(rawEv) && rawEv >= 0
      ? rawEv
      : undefined;
  ...
  if (profileSectionKeys.some((k) => updates[k] !== undefined)) {
    const doc = await profileRepository.findLeanByIdSelect(
      userId,
      profileSectionKeys.join(" "),
    );
  ...
  const updatedProfile = updated as ProfileSections & {
    _id: unknown;
  };
