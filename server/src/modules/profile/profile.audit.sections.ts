...
function auditStackTools(
  ...
  for (const t of newList) {
    if (!oldList.includes(t))
      log(AuditAction.STACK_TOOL_ADDED, "profile", { tool: t });
  }
  for (const t of oldList) {
    if (!newList.includes(t))
      log(AuditAction.STACK_TOOL_REMOVED, "profile", { tool: t });
  }
}
function auditCertifications(
  ...
  const oldC = (currentProfile.certifications ?? []) as Array<{
    certId?: string;
    name?: string;
  }>;
  const newC = (updatedProfile.certifications ?? []) as Array<{
    certId?: string;
    name?: string;
  }>;
  const oldIds = new Set(
    oldC.map((c) => (c.certId ?? "").trim()).filter(Boolean),
  );
  const newIds = new Set(
    newC.map((c) => (c.certId ?? "").trim()).filter(Boolean),
  );
  for (const c of newC) {
    ...
    if (oldIds.has(id)) {
      ...
      log(AuditAction.CERTIFICATION_ADDED, "certification", {
        certId: id,
        name: c.name,
      });
    }
  ...
  for (const id of oldIds) {
    if (!newIds.has(id))
      log(AuditAction.CERTIFICATION_REMOVED, "certification", { certId: id });
  }
}
function auditProjects(
  ...
  const oldP = (currentProfile.projects ?? []) as Array<{
    title?: string;
  }>;
  const newP = (updatedProfile.projects ?? []) as Array<{
    title?: string;
  }>;
  ...
  for (let i = 0; i < minLen; i++) {
    if (JSON.stringify(oldP[i]) !== JSON.stringify(newP[i])) {
      log(AuditAction.PROJECT_UPDATED, "project", {
        index: i,
        title: newP[i]?.title,
      });
    }
...
function auditOpenSource(
  ...
  const oldKeys = new Set(
    oldO.map((o, i) => (o.repositoryUrl ?? "").trim() || `i:${i}`),
  );
  const newKeys = new Set(
    newO.map((o, i) => (o.repositoryUrl ?? "").trim() || `i:${i}`),
  );
  for (let i = 0; i < newO.length; i++) {
    const o = newO[i];
    const key = (o.repositoryUrl ?? "").trim() || `i:${i}`;
    if (oldKeys.has(key)) {
      const prev = oldO.find(
        (x, j) => ((x.repositoryUrl ?? "").trim() || `i:${j}`) === key,
      );
...
function auditMySetup(
  ...
  const oldM = (currentProfile.mySetup ?? []) as Array<{
    label?: string;
    imageUrl?: string;
  }>;
  const newM = (updatedProfile.mySetup ?? []) as Array<{
    label?: string;
    imageUrl?: string;
  }>;
  if (newM.length > oldM.length) {
    for (let i = oldM.length; i < newM.length; i++) {
      const m = newM[i];
      log(AuditAction.MY_SETUP_ADDED, "my_setup", {
        label: m?.label,
        index: i,
      });
    }
  ...
  if (newM.length < oldM.length) {
    for (let i = newM.length; i < oldM.length; i++) {
      log(AuditAction.MY_SETUP_REMOVED, "my_setup", {
        label: oldM[i]?.label,
        index: i,
      });
    }
  ...
  for (let i = 0; i < minLen; i++) {
    if (JSON.stringify(oldM[i]) !== JSON.stringify(newM[i])) {
      log(AuditAction.MY_SETUP_UPDATED, "my_setup", {
        label: newM[i]?.label,
        index: i,
      });
    }
