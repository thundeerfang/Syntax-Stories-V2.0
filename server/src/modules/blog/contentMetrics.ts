...
/** Count editor blocks and approximate text lines from serialized blog content JSON. */
export function measureBlogContent(content: string | undefined): BlogContentMetrics {
  ...
  try {
    ...
    for (const item of parsed) {
      ...
      const block = item as { type?: string; payload?: unknown };
      blocks += 1;
      const type = typeof block.type === 'string' ? block.type : '';
      const payload = block.payload;
      if (payload && typeof payload === 'object' && !Array.isArray(payload)) {
        lines = appendBlockMetrics(type, payload as Record<string, unknown>, lines);
      }
    }
    return { lines, blocks };
  } catch {
