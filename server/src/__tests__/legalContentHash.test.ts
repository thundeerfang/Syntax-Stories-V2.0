...
describe("legalContentHash", () => {
  ...
  it("verifies revision hash", () => {
    ...
    expect(verifyRevisionHash({ title, summary, body, contentHash: h })).toBe(
      true,
    );
    expect(
      verifyRevisionHash({ title, summary, body, contentHash: "deadbeef" }),
    ).toBe(false);
  });
  it("stableStringify orders keys for idempotency hash", () => {
    ...
    expect(hashIdempotencyPayload({ b: 1, a: 2 })).toBe(
      hashIdempotencyPayload({ a: 2, b: 1 }),
    );
  });
