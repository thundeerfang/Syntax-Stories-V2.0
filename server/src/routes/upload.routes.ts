...
async function resolveUploaderUsername(req: Request): Promise<string> {
  const userId = (req as Request & { user?: AuthUser }).user?._id;
...
async function handleImageMasterUpload(
  ...
  try {
    const processed = await processUploadedImageBuffer(file.buffer, file.mimetype, cfg.profile);
    const crop = parseCropCoords(req.body as Record<string, unknown>);
    const delivered = await applyImageDelivery(processed.buffer, crop, cfg.delivery);
    ...
    res.status(201).json(uploadImageJson(url, file.originalname, username, blurDataUrl));
  } catch (err) {
    ...
    if (isEnospcError(err)) {
      ...
      res.status(503).json({
        ...
        code: 'STORAGE_FULL',
        reason: 'disk_full',
        message: 'Syntax Stories cannot accept new uploads right now. Please try again later.',
      });
      return;
    }
    console.error(err);
