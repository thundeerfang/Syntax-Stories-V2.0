...
export async function listNotifications(req: Request, res: Response): Promise<void> {
  ...
  const limit =
    typeof limitRaw === 'string' && /^\d+$/.test(limitRaw) ? Number.parseInt(limitRaw, 10) : 50;
...
export async function getNotificationPreferences(req: Request, res: Response): Promise<void> {
  ...
  res.status(200).json({ success: true, preferences: serializeNotificationPreferences(prefs) });
}
