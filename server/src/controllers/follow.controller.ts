...
async function ensureStoredFollowCounts(
  ...
  await UserModel.updateOne(
    { _id: userId },
    { $set: { followersCount: fc, followingCount: fic } },
  );
...
export async function getPublicProfile(
  ...
  try {
    ...
    const counts = await ensureStoredFollowCounts(
      u._id,
      u.followersCount,
      u.followingCount,
    );
    ...
    const durableLongest = (
      user as {
        readStreakLongest?: number;
      }
    ).readStreakLongest;
    if (durableLongest != null && durableLongest > 0) {
      readStreak.byMode.daily.longest = Math.max(
        readStreak.byMode.daily.longest,
        durableLongest,
      );
...
function seededShuffleFollowList<
  T extends {
    id: string;
  },
>(items: T[], seedStr: string): T[] {
  let seed = 2166136261;
...
async function paginatedFollowEdges(
  ...
  const follows = await FollowModel.find(
    filter as mongoose.FilterQuery<{
      createdAt: Date;
    }>,
  )
  ...
  try {
    ...
    try {
      await session.withTransaction(async () => {
        ...
        await applyFollowSideEffectsTransaction(
          session,
          currentUser,
          target,
          dayKey,
    ...
    void dispatchAchievementEvents(String(target._id), [
      { type: "profile_sync" },
    ]).catch((e) => console.error("[achievements] follow", e));
    const newlyUnlocked = await dispatchAchievementEvents(
