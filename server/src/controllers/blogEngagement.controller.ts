...
export async function setBlogRepost(
  req: Request,
  res: Response,
): Promise<void> {
  try {
    ...
    if (typeof body.reposting !== "boolean") {
      res
        ...
        .json({
          success: false,
          message: "Body must include reposting: true | false",
        });
      return;
    }
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to update repost" });
  }
}
export async function setBlogBookmark(
  req: Request,
  res: Response,
): Promise<void> {
  try {
    ...
    if (typeof body.bookmarked !== "boolean") {
      res
        ...
        .json({
          success: false,
          message: "Body must include bookmarked: true | false",
        });
      return;
    }
    const groupIdHex =
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to update bookmark" });
  }
}
export async function postBlogEngagementViewerState(
  req: Request,
  res: Response,
): Promise<void> {
  try {
    ...
    if (!Array.isArray(body.postIds)) {
      res
        .status(400)
        .json({ success: false, message: "postIds must be an array" });
      return;
    }
    const ids = body.postIds
      .filter((x): x is string => typeof x === "string")
      .slice(0, 50);
    const [viewerRespectStates, viewerRepostStates, viewerBookmarkStates] =
      await Promise.all([
    ...
    res
      ...
      .json({
        success: false,
        message: "Failed to load viewer engagement state",
      });
  }
}
