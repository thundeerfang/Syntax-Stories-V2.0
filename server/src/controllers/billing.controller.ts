...
export async function postCheckoutSession(
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: err.message || "Checkout failed", success: false });
  }
}
export async function postVerifyCheckout(
  req: Request,
  res: Response,
): Promise<void> {
  const user = (
  ...
  try {
    const subscription = await verifyCheckoutAndSync(
      user._id,
...
export async function getTransactions(
  ...
  const existingCount = await PaymentLedgerModel.countDocuments({
    userId: user._id,
  });
