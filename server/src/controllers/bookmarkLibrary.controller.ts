...
export async function listBookmarkGroups(
  req: Request,
  res: Response,
): Promise<void> {
  try {
...
export async function patchBookmarkGroup(
  req: Request,
  res: Response,
): Promise<void> {
  try {
    ...
    if (body.isDefault === true) {
      ...
      if (!r.ok) {
        res
          .status(400)
          .json({ success: false, message: r.message ?? "Failed" });
        return;
      }
    ...
    res
      .status(400)
      .json({ success: false, message: "No supported fields in body" });
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, message: "Failed to update group" });
  }
}
export async function removeBookmarkGroup(
  req: Request,
  res: Response,
): Promise<void> {
  try {
