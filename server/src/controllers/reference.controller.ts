...
} from "../models/ReferenceEntity.js";
...
function parseKind(raw: unknown): ReferenceEntityKind | null {
  ...
  return ENTITY_KINDS.has(kind as ReferenceEntityKind)
    ? (kind as ReferenceEntityKind)
    : null;
}
export async function searchReferenceEntities(
  req: Request,
  res: Response,
): Promise<void> {
  ...
  if (!kind) {
    res
      ...
      .json({
        success: false,
        message: "kind must be company, school, or organization",
      });
    return;
  }
...
export async function searchTechStackReference(
  req: Request,
  res: Response,
): Promise<void> {
  const q = typeof req.query.q === "string" ? req.query.q.trim() : "";
  ...
  if (!q) {
    res
      .status(200)
      .json({ success: true, items: [] satisfies TechStackItemDto[] });
    return;
  }
  const rx = new RegExp(escapeRegex(q), "i");
...
export async function resolveTechStackReference(
  req: Request,
  res: Response,
): Promise<void> {
  ...
  if (names === null) {
    res
      .status(400)
      .json({ success: false, message: "names must be an array of strings" });
    return;
  }
  if (names.length === 0) {
    res
