...
/** POST /api/contact */
export async function submitContactLead(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (isAuthed) {
      ...
      fullName = typeof u.fullName === 'string' ? u.fullName.trim().slice(0, 120) : '';
      email = typeof u.email === 'string' ? u.email.trim().toLowerCase().slice(0, 254) : '';
      if (!fullName) {
        res.status(400).json({ success: false, message: 'Name is required.' });
        return;
      }
      if (!email || !EMAIL_RE.test(email)) {
        res.status(400).json({ success: false, message: 'Account email missing or invalid.' });
        return;
      }
      username = typeof u.username === 'string' ? u.username.slice(0, 64) : undefined;
      userId = new mongoose.Types.ObjectId(auth!._id);
      const co = typeof body.company === 'string' ? body.company.trim().slice(0, 120) : '';
    ...
    if (!topicRaw || topicRaw.length < 2) {
      res.status(400).json({ success: false, message: 'Please enter a subject or topic.' });
      return;
    }
