...
function mapSquadSummary(s: {
  ...
  return {
    ...
    ).invitePermission ?? "all_members") as SquadInvitePermission,
    memberCount: s.memberCount,
    createdAt:
      s.createdAt instanceof Date ? s.createdAt.toISOString() : undefined,
...
export async function listMySquads(req: Request, res: Response): Promise<void> {
  try {
    ...
    const links = await SquadMemberModel.find({ userId: uid })
      .select("squadId role")
      .lean();
    ...
    const roleBy = new Map(
      links.map((l) => [String(l.squadId), l.role as SquadMemberRole]),
    );
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to load your squads" });
  }
}
export async function listSquadsForUser(
  ...
  try {
    ...
    const isSelf =
      viewerId != null && String(profileUser._id) === String(viewerId);
    ...
    const squads = await SquadModel.find(squadFilter)
      .sort({ updatedAt: -1 })
      .lean();
    const roleBy = new Map(
      links.map((l) => [String(l.squadId), l.role as SquadMemberRole]),
...
export async function patchSquad(req: Request, res: Response): Promise<void> {
  try {
    ...
    const role = await getSquadMemberRole(
      squad._id as mongoose.Types.ObjectId,
      user._id,
    );
    if (role !== "admin") {
      res
        ...
        .json({
          success: false,
          message: "Only squad admins can update the squad",
        });
      return;
    }
    ...
    if ("postPolicy" in body) {
      ...
      if (p !== "all_members" && p !== "staff_only") {
        res
          .status(400)
          .json({ success: false, message: "postPolicy is invalid" });
        return;
      }
    ...
    if ("requirePostApproval" in body) {
      if (typeof body.requirePostApproval !== "boolean") {
        res
          ...
          .json({
            success: false,
            message: "requirePostApproval must be a boolean",
          });
        return;
      }
    ...
    if ("invitePermission" in body) {
      ...
      if (ip !== "all_members" && ip !== "staff_only") {
        res
          .status(400)
          .json({ success: false, message: "invitePermission is invalid" });
        return;
      }
    ...
    const unsetDoc = Object.fromEntries(
      unsetFields.map((k) => [k, 1 as const]),
    );
    ...
    if (!next) {
      res
        .status(500)
        .json({ success: false, message: "Failed to reload squad" });
      return;
    }
...
export async function joinSquad(req: Request, res: Response): Promise<void> {
  try {
    ...
    const squad = await SquadModel.findOne({ slug })
      .select("+inviteToken visibility")
      .lean();
    if (!squad) {
      res.status(404).json({ success: false, message: "Squad not found" });
      return;
    ...
    if (squad.visibility === "public") {
      ...
      res
        .status(200)
        .json(attachAchievementsToResponse({ success: true }, newlyUnlocked));
      return;
    }
    const body = req.body as {
      inviteToken?: unknown;
    ...
    if (!inviteToken) {
      res
        ...
        .json({
          success: false,
          message: "inviteToken is required for private squads",
        });
      return;
    }
    const r = await joinPrivateSquadWithToken({
      squadId,
      userId: user._id,
      inviteToken,
    });
...
export async function postSquadMember(
  req: Request,
  res: Response,
): Promise<void> {
  try {
...
export async function patchSquadMemberRole(
  req: Request,
  res: Response,
): Promise<void> {
  try {
    ...
    const role =
      roleRaw === "moderator" || roleRaw === "member"
        ? (roleRaw as SquadMemberRole)
        : null;
    if (!role) {
      res
        .status(400)
        .json({ success: false, message: "role must be moderator or member" });
      return;
    }
...
export async function getSquadMemberStats(
  req: Request,
  res: Response,
): Promise<void> {
  try {
    ...
    if (!slug || !username) {
      res
        .status(400)
        .json({ success: false, message: "Invalid slug or username" });
      return;
    }
    const r = req as RequestWithOptionalAuth;
    const viewerId = r.authUser?._id;
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to load member stats" });
  }
}
export async function deleteSquadMember(
  req: Request,
  res: Response,
): Promise<void> {
  try {
...
export async function listSquadMembers(
  ...
  try {
    ...
    const rows = await SquadMemberModel.find({ squadId })
      ...
      .lean();
    const order: Record<SquadMemberRole, number> = {
      admin: 0,
      moderator: 1,
      member: 2,
    };
    const members = rows
      .map((row) => {
        ...
        return {
          userId,
          username:
            typeof populated?.username === "string" ? populated.username : "",
          fullName:
            typeof populated?.fullName === "string" ? populated.fullName : "",
          profileImg:
            typeof populated?.profileImg === "string"
              ? populated.profileImg
              : "",
...
export async function postSquadShare(
  req: Request,
  res: Response,
): Promise<void> {
  try {
...
export async function deleteSquadPin(
  req: Request,
  res: Response,
): Promise<void> {
  try {
    ...
    if (
      !slug ||
      !postIdParam ||
      !mongoose.Types.ObjectId.isValid(postIdParam)
    ) {
...
export async function getSquadFeed(req: Request, res: Response): Promise<void> {
  try {
    ...
    for (const s of shares) {
      ...
      const username =
        sharerRaw &&
        typeof sharerRaw === "object" &&
        typeof sharerRaw.username === "string"
        ...
        ...(username
          ? {
              sharedBy: {
                userId: sharerId,
                username,
                ...(typeof sharerRaw?.fullName === "string" &&
                sharerRaw.fullName.trim()
                  ? { fullName: sharerRaw.fullName.trim() }
                  : {}),
                ...(typeof sharerRaw?.profileImg === "string" &&
                sharerRaw.profileImg.trim()
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to load squad feed" });
  }
}
