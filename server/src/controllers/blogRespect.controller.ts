...
export async function setBlogRespect(
  req: Request,
  res: Response,
): Promise<void> {
  try {
    ...
    if (typeof body.respecting !== "boolean") {
      res
        ...
        .json({
          success: false,
          message: "Body must include respecting: true | false",
        });
      return;
    }
    ...
    const [viewerUnlocks, authorUnlocks] = await Promise.all([
      events.length > 0
        ...
        : Promise.resolve([]),
      result.newRespectEdge && result.respecting
        ? dispatchAchievementEvents(String(found.authorId), [
            { type: "profile_sync" },
          ])
        : Promise.resolve([]),
    ]);
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to update Respect" });
  }
}
export async function postBlogRespectViewerState(
  req: Request,
  res: Response,
): Promise<void> {
  try {
    ...
    const ids = body.postIds
      ...
      .slice(0, 50);
    const viewerRespectStates = await viewerRespectStatesForPosts(
      user._id,
      ids,
    );
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to load viewer Respect state" });
  }
}
