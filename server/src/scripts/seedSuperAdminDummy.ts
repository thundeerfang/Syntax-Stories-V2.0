...
async function main(): Promise<void> {
  ...
  const { seedAccessCatalog } =
    await import("../admin-platform/seeds/accessCatalog.seed.js");
  const { seedDefaultRoles } =
    await import("../admin-platform/seeds/defaultRoles.seed.js");
  const { seedBootstrapOperator } =
    await import("../admin-platform/seeds/bootstrapOperator.seed.js");
  ...
  console.log(
    "  Step 2:   Scan the QR code and enable 2FA (required before dashboard access)",
  );
