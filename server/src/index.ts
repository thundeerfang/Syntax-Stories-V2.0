...
async function start(): Promise<void> {
  ...
  try {
    ...
    console.warn(
      "[Gamification] Background workers failed to start (API continues):",
      String(e),
    );
  }
  const { startBillingBackgroundJobs } =
    await import("./services/billing/billingBackground.js");
  startBillingBackgroundJobs();
  try {
    const { startSearchIndexWorker } =
      await import("./services/search/searchIndex.service.js");
    await startSearchIndexWorker();
  } catch (e) {
    console.warn(
      "[search] Index worker failed to start (API continues):",
      String(e),
    );
  }
