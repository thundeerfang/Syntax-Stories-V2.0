...
import { NOTIFICATION_ICONS, NOTIFICATION_TYPES, type NotificationIcon, type NotificationType } from '../services/notifications/notification.types.js';
...
async function platformSecretMatches(header: string | undefined): Promise<boolean> {
...
export function startNotificationWebhookServer(): void {
  ...
  /**
   * Inbound webhook — create a notification for a user.
   * Auth: `X-Webhook-Secret` must match NOTIFICATION_WEBHOOK_INGEST_SECRET or platform webhook secret.
   */
  app.post('/webhooks/notifications/ingest', async (req, res) => {
    const secret = typeof req.headers['x-webhook-secret'] === 'string'
    ...
    if (!title || !message || !href) {
      res.status(400).json({ success: false, message: 'title, message, and href are required' });
      return;
    }
