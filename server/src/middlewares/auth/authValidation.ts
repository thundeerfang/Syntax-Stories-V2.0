...
const altchaField = z
  .union([z.string().max(20000), z.record(z.unknown())])
  .optional();
...
export function sendOtpValidation(
  req: Request,
  res: Response,
  next: NextFunction,
): void {
...
export function verifyOtpValidation(
  req: Request,
  res: Response,
  next: NextFunction,
): void {
  ...
  if (!r.success) {
    const msg =
      r.error.issues[0]?.message?.replaceAll('"', "") ?? "Validation error";
    res
      .status(400)
      .json({ message: msg, error: formatZodError(r.error), success: false });
    return;
  }
  req.body = r.data;
  next();
}
export function staffLoginValidation(
  req: Request,
  res: Response,
  next: NextFunction,
): void {
...
export function updateProfileValidation(
  req: Request,
  res: Response,
  next: NextFunction,
): void {
