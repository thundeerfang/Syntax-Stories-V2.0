...
export async function seedBootstrapOperator(
  options?: BootstrapOperatorOptions,
): Promise<void> {
  ...
  if (existingAdmin) {
    ...
    console.log(
      `[seed] Bootstrap operator ${email} — created (POST /auth/staff-login)`,
    );
  }
  if (twoFactorMode === "preconfigured") {
    const fixedSecret =
      options?.twoFactorSecret?.trim() ||
      process.env.ADMIN_BOOTSTRAP_2FA_SECRET?.trim();
    const { secret, otpauthUrl } = await applySeedTwoFactor(
      user._id,
      email,
      fixedSecret,
    );
