...
const DEFAULT_ROLES = [
  ...
  },
  {
    ...
    level: 200,
    description:
      "User lookup, feedback, contact leads, and billing read access.",
...
export async function seedDefaultRoles(): Promise<void> {
    ...
    await AdminRoleModel.updateOne(
      { name: role.name, deletedAt: null },
      {
        $set: {
          ...
          systemProtected:
            "systemProtected" in role ? Boolean(role.systemProtected) : false,
          deletedAt: null,
          deletedById: null,
        },
      },
  ...
  console.log(
    `[seed] Admin default roles: ${DEFAULT_ROLES.map((r) => r.name).join(", ")}`,
  );
}
