...
export async function getPasskeyStatusHandler(req: Request, res: Response): Promise<void> {
  try {
    if (!isPasskeyFeatureEnabled()) {
      res.status(200).json({ success: true, enabled: false, registered: false, count: 0 });
      return;
    }
...
export async function passkeyRegisterOptions(req: Request, res: Response): Promise<void> {
  try {
    if (!isPasskeyFeatureEnabled()) {
      res.status(400).json({ success: false, message: 'Passkey step-up is disabled' });
      return;
    }
    const user = requireUser(req, res);
    if (!user) return;
    ...
    const options = await createPasskeyRegistrationOptions(user._id, dbUser.email);
    res.status(200).json({ success: true, options });
  } catch (err) {
    console.error('[passkeyRegisterOptions]', err);
    res.status(500).json({ success: false, message: 'Could not start passkey registration' });
  }
}

export async function passkeyRegisterVerify(req: Request, res: Response): Promise<void> {
  try {
    const user = requireUser(req, res);
    if (!user) return;
    const body = req.body as { response?: unknown; deviceLabel?: string };
    if (!body.response) {
...
export async function passkeyPreferences(req: Request, res: Response): Promise<void> {
  try {
    const user = requireUser(req, res);
    if (!user) return;
    const { stepUpEnabled } = req.body as { stepUpEnabled?: boolean };
    if (typeof stepUpEnabled !== 'boolean') {
      res.status(400).json({ success: false, message: 'stepUpEnabled boolean required' });
      return;
    }
...
export async function passkeyRemove(req: Request, res: Response): Promise<void> {
  try {
    const user = requireUser(req, res);
    if (!user) return;
    const credentialId = (req.body as { credentialId?: string }).credentialId;
    await removePasskey(user._id, credentialId);
    void writeAuditLog(req, AuditAction.ADMIN_PASSKEY_REMOVED, { actorId: user._id });
...
export async function passkeyStepUpOptions(req: Request, res: Response): Promise<void> {
  try {
    const user = requireUser(req, res);
    if (!user) return;
    ...
    const msg = err instanceof Error ? err.message : 'Biometric step-up unavailable';
    res.status(400).json({ success: false, message: msg });
  }
}

export async function passkeyStepUpVerify(req: Request, res: Response): Promise<void> {
  try {
    const user = requireUser(req, res);
    if (!user) return;
