...
export async function getPasskeyStatus(userId: string) {
  const user = await UserModel.findById(userId).select(
    "passkeys passkeyStepUpEnabled twoFactorEnabled",
  );
  if (!user) return null;
...
export async function createPasskeyRegistrationOptions(
  ...
  await storeChallenge(
    redisKeys.auth.passkeyRegister(userId),
    options.challenge,
  );
  return options;
}
...
export async function setPasskeyStepUpEnabled(
  userId: string,
  enabled: boolean,
): Promise<void> {
  ...
  await UserModel.updateOne(
    { _id: userId },
    { $set: { passkeyStepUpEnabled: enabled } },
  );
}
export async function removePasskey(
  ...
  if (credentialId) {
    ...
    await UserModel.updateOne(
      { _id: userId },
      { $set: { passkeys: [], passkeyStepUpEnabled: false } },
    );
  }
  const user = await UserModel.findById(userId).select("passkeys");
  if (user && (user.passkeys?.length ?? 0) === 0) {
    await UserModel.updateOne(
      { _id: userId },
      { $set: { passkeyStepUpEnabled: false } },
    );
  }
}
export async function createPasskeyStepUpOptions(
  userId: string,
  sessionId: string,
) {
  const user = await UserModel.findById(userId).select(
    "passkeys passkeyStepUpEnabled email",
  );
  if (!user) throw new Error("User not found");
  ...
  await storeChallenge(
    redisKeys.auth.passkeyStepUp(sessionId),
    options.challenge,
  );
  return options;
}
