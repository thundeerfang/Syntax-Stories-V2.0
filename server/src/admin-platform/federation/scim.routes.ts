...
scimAdminRouter.get("/Users", async (_req, res) => {
  const operators = await AdminUserModel.find()
    .select("userId email displayName isActive")
    .lean();
  const resources = [];
  for (const op of operators) {
    const user = await UserModel.findById(op.userId)
      .select("email fullName staffRole")
      .lean();
    if (!user) continue;
    resources.push(
      scimUser({
