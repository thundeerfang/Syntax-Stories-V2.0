...
samlAdminRouter.get("/metadata", (req, res) => {
  if (!env.FEATURE_SAML_SSO) {
    res
      .status(503)
      .json({ success: false, message: "SAML SSO is not enabled" });
    return;
  }
...
samlAdminRouter.post("/acs", async (req, res, next) => {
  try {
    ...
    if (!adminRow) {
      res
        .status(403)
        .json({ success: false, message: "No active operator for this email" });
      return;
    }
    ...
    if (!user) {
      res
        .status(403)
        .json({ success: false, message: "Operator account not found" });
      return;
    }
