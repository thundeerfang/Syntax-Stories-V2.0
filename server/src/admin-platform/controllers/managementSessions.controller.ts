...
/**
 * GET /api/v1/admin/management/sessions — active sessions for the signed-in operator.
 */
...
export async function revokeMySession(req: Request, res: Response): Promise<void> {
  ...
  const session = await SessionModel.findOne({ _id: sessionId, userId: actor.user._id });
  ...
  if (sessionId === actor.user.sessionId) {
    sendAdminError(res, 400, 'VALIDATION_ERROR', 'Use logout to end your current session.');
    return;
  }
...
export async function revokeOtherSessions(req: Request, res: Response): Promise<void> {
  ...
  await SessionModel.updateMany(
    { userId: actor.user._id, _id: { $ne: actor.user.sessionId }, revoked: false },
    { $set: { revoked: true, revokedReason: 'admin_revoke_others' } }
  );

  await logSecurityEvent(actor.user._id, 'session_revoked', req, { scope: 'others' });
  void writeAuditLog(req, AuditAction.SESSION_REVOKED, {
    actorId: actor.user._id,
