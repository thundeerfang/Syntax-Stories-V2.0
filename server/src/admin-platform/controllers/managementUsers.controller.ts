...
export async function searchUsers(req: Request, res: Response): Promise<void> {
  ...
  if (q.length < 2) {
    sendAdminError(
      ...
      400,
      "VALIDATION_ERROR",
      "Query q must be at least 2 characters",
    );
    return;
  }
...
export async function getUsers(req: Request, res: Response): Promise<void> {
  ...
  const nextCursor =
    hasMore && last
      ? Buffer.from(JSON.stringify({ id: String(last._id) }), "utf8").toString(
          "base64url",
        )
      : null;
  sendAdminOk(res, {
...
export async function getUserById(req: Request, res: Response): Promise<void> {
  ...
  if (env.FEATURE_ADMIN_REBAC) {
    const rebac = rebacAllows(actor, "user:read", {
      type: "user",
      id: targetId,
    });
    if (!rebac.allow) {
  ...
  const profileUser = user as unknown as IUser & {
    _id: {
      toString(): string;
    };
  };
  const emailVerification = resolveEmailVerificationForAdmin(profileUser);
  const [publishedBlogs, draftBlogs, subscription, ledgerCount, activity] =
    await Promise.all([
...
export async function getUserFollows(
  ...
  const uid = new mongoose.Types.ObjectId(id);
  const filter = type === "followers" ? { following: uid } : { follower: uid };
  const userField = type === "followers" ? "follower" : "following";
  const links = await FollowModel.find(filter)
    .sort({ createdAt: -1 })
    .limit(limit)
    .lean();
  const userIds = links.map(
    (l) => l[userField as "follower" | "following"] as mongoose.Types.ObjectId,
  );
  const users =
    userIds.length > 0
...
export async function patchUserProfile(
  ...
  if (typeof expected !== "number" || !Number.isFinite(expected)) {
    sendAdminError(
      ...
      400,
      "VALIDATION_ERROR",
      "expectedProfileVersion is required",
    );
    return;
  }
  ...
  if ((user.profileVersion ?? 0) !== expected) {
    sendAdminError(
      ...
      409,
      "CONFLICT",
      "Profile was modified elsewhere. Refresh and retry.",
    );
    return;
  }
...
export async function postUnlockUser(
  req: Request,
  res: Response,
): Promise<void> {
  const r = req as StaffManagementRequest;
