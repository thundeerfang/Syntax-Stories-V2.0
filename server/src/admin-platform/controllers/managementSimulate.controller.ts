...
/**
 * POST /api/v1/admin/management/iam/simulate
 * Preview whether a role (or permission list) would allow an action.
 */
export async function postIamSimulate(req: Request, res: Response): Promise<void> {
  ...
  if (!action) {
    sendAdminError(res, 400, 'VALIDATION_ERROR', 'action is required (permission key)');
    return;
  }
  ...
  if (body.roleId?.trim()) {
    ...
    permissions = body.permissions.map((p) => String(p).trim()).filter((p) => catalog.has(p));
  } else {
    sendAdminError(res, 400, 'VALIDATION_ERROR', 'roleId or permissions[] required');
    return;
  }
