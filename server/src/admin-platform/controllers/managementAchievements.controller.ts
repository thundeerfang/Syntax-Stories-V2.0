...
export async function listAchievementsAdmin(_req: Request, res: Response): Promise<void> {
...
export async function postAchievement(req: Request, res: Response): Promise<void> {
  ...
  if (!key) {
    sendAdminError(res, 400, 'VALIDATION_ERROR', 'key is required (alphanumeric and hyphens, max 64)');
    return;
  }
  if (!slug) {
    sendAdminError(res, 400, 'VALIDATION_ERROR', 'slug is required (alphanumeric and hyphens, max 64)');
    return;
  }
  if (!title || title.length > 120) {
    sendAdminError(res, 400, 'VALIDATION_ERROR', 'title is required (max 120 characters)');
    return;
  }
  ...
  const points =
    typeof body.points === 'number' && Number.isFinite(body.points)
      ? Math.max(0, Math.min(10_000, Math.floor(body.points)))
      : null;
  if (points === null) {
    sendAdminError(res, 400, 'VALIDATION_ERROR', 'points must be a number between 0 and 10000');
    return;
  }

  const target =
    typeof body.target === 'number' && Number.isFinite(body.target)
      ? Math.max(1, Math.min(1_000_000, Math.floor(body.target)))
      : null;
  if (target === null) {
    sendAdminError(res, 400, 'VALIDATION_ERROR', 'target must be a number >= 1');
    return;
  }
  ...
  const unlocksAfter =
    body.unlocksAfter === undefined || body.unlocksAfter === null || body.unlocksAfter === ''
      ? null
      : parseKey(body.unlocksAfter);

  if (body.unlocksAfter && !unlocksAfter) {
    sendAdminError(res, 400, 'VALIDATION_ERROR', 'unlocksAfter must be a valid achievement key');
    return;
  }
...
export async function patchAchievement(req: Request, res: Response): Promise<void> {
  ...
  if (body.slug !== undefined) {
    const slug = parseSlug(body.slug);
    if (!slug) {
      sendAdminError(res, 400, 'VALIDATION_ERROR', 'slug must be 1–64 alphanumeric/hyphen characters');
      return;
    }
    patch.slug = slug;
  }
  if (body.title !== undefined) {
    const title = typeof body.title === 'string' ? body.title.trim() : '';
    if (!title || title.length > 120) {
      sendAdminError(res, 400, 'VALIDATION_ERROR', 'title must be 1–120 characters');
      return;
    }
    patch.title = title;
  }
  ...
  if (body.points !== undefined) {
    ...
    patch.points = Math.max(0, Math.min(10_000, Math.floor(body.points)));
  }
  if (body.target !== undefined) {
    ...
    patch.target = Math.max(1, Math.min(1_000_000, Math.floor(body.target)));
  }
