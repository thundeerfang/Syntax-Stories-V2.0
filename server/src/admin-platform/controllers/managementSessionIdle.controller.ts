...
export async function getSessionIdleStatus(req: Request, res: Response): Promise<void> {
...
export async function postSessionTouch(req: Request, res: Response): Promise<void> {
  const mgmt = req as StaffManagementRequest;
  const sessionId = mgmt.user.sessionId;
  const userId = mgmt.user._id;
  if (!sessionId) {
