...
export async function listBillingPlansAdmin(
  _req: Request,
  res: Response,
): Promise<void> {
...
export async function getAvailableBillingPlanKeys(
  _req: Request,
  res: Response,
): Promise<void> {
...
export async function postBillingPlan(
  req: Request,
  res: Response,
): Promise<void> {
  ...
  const amountDisplay =
    typeof body.amountDisplay === "string" ? body.amountDisplay.trim() : "";
  const cadence =
  ...
  if (!name || name.length > 80) {
    sendAdminError(
      ...
      400,
      "VALIDATION_ERROR",
      "name is required (max 80 characters)",
    );
    return;
  }
  ...
  if (amountMinor === null) {
    sendAdminError(
      ...
      400,
      "VALIDATION_ERROR",
      "amountMinor must be a number (paise)",
    );
    return;
  }
  const sortOrder =
...
export async function patchBillingPlan(
  ...
  if (body.mostPopular !== undefined)
    patch.mostPopular = Boolean(body.mostPopular);
