...
export async function postImpersonateUser(req: Request, res: Response): Promise<void> {
  if (!env.FEATURE_ADMIN_IMPERSONATION) {
    sendAdminError(res, 503, 'FEATURE_DISABLED', 'Impersonation is not enabled.');
    return;
  }
...
export async function postEndImpersonation(req: Request, res: Response): Promise<void> {
  const actor = req as StaffManagementRequest;
  const sessionId = actor.user.sessionId;
  if (!sessionId) {
    sendAdminError(res, 400, 'VALIDATION_ERROR', 'Session required');
