...
function mapUser(
  u: { _id?: mongoose.Types.ObjectId; username?: string; fullName?: string; profileImg?: string } | null
): Pick<AdminBlogEngagementRow, 'username' | 'fullName' | 'userRef' | 'profileImg'> {
  if (!u || !u._id) {
...
async function ensurePost(postId: mongoose.Types.ObjectId) {
  return BlogPostModel.findOne({
    ...
    .select('title viewCount respectCount commentCount repostCount bookmarkCount')
    .lean();
}
...
export async function loadBlogEngagement(
  ...
  if (metric === 'views') {
    ...
    const loggedIn: AdminBlogEngagementRow[] = events.map((e) => {
      const u = mapUser(
        e.actorId as { _id?: mongoose.Types.ObjectId; username?: string; fullName?: string; profileImg?: string } | null
      );
      return {
        id: String(e._id),
        kind: 'user' as const,
  ...
  if (metric === 'respects') {
    ...
    const items: AdminBlogEngagementRow[] = rows.map((r) => {
      const u = mapUser(
        r.userId as { _id?: mongoose.Types.ObjectId; username?: string; fullName?: string; profileImg?: string }
      );
      return {
        id: String(r._id),
        kind: 'user',
    ...
    return { postTitle: post.title, metric, total: post.respectCount ?? items.length, items };
  }
  ...
  if (metric === 'reposts') {
    ...
    return { postTitle: post.title, metric, total: post.repostCount ?? items.length, items };
  }
  ...
  if (metric === 'bookmarks') {
    ...
    return { postTitle: post.title, metric, total: post.bookmarkCount ?? items.length, items };
  }
  ...
  if (metric === 'comments') {
    ...
    return { postTitle: post.title, metric, total: post.commentCount ?? items.length, items };
  }
