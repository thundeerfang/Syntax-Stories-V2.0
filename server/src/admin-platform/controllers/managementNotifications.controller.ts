...
export async function postAdminNotificationWebhookTest(
  _req: Request,
  res: Response,
): Promise<void> {
  const config = (await getPlatformNotificationConfig(
  ...
  if (!config.webhookEnabled || !config.webhookUrl.trim()) {
    sendAdminError(
      ...
      400,
      "VALIDATION_ERROR",
      "Enable webhook delivery and set a URL first.",
    );
    return;
  }
  const { deliverNotificationWebhook } =
    await import("../../services/notifications/notificationWebhook.service.js");
