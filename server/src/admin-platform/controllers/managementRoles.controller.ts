...
import { validatePermissionDependencies } from "../iam/permissionDependencies.js";
...
export async function getRoles(req: Request, res: Response): Promise<void> {
  ...
  const roles = await AdminRoleModel.find(q)
    .sort({ level: -1, name: 1 })
    .lean();
  sendAdminOk(res, {
    roles: roles.map((r) => ({
...
export async function postRole(req: Request, res: Response): Promise<void> {
  ...
  if (body.level > actorMax) {
    sendAdminError(
      ...
      403,
      "FORBIDDEN",
      "Cannot create a role above your effective level.",
    );
    return;
  }
  ...
  const depErrors = validatePermissionDependencies(perms);
  if (depErrors.length > 0) {
    sendAdminError(res, 400, "VALIDATION_ERROR", depErrors.join("; "));
    return;
  }
  ...
  void writeAuditLog(req, AuditAction.ADMIN_ROLE_CREATED, {
    actorId: actor.user._id,
    targetType: "admin_role",
    targetId: String(doc._id),
    metadata: { name, level: body.level, permissionCount: perms.length },
  });
...
export async function patchRole(req: Request, res: Response): Promise<void> {
  ...
  if (body.name !== undefined) {
    ...
    const clash = await AdminRoleModel.findOne({
      name,
      deletedAt: null,
      _id: { $ne: doc._id },
    }).lean();
  ...
  if (body.level !== undefined) {
    ...
    if (body.level > actorMax) {
      sendAdminError(
        ...
        403,
        "FORBIDDEN",
        "Cannot set level above your effective level.",
      );
      return;
    }
  ...
  await doc.save();
  await invalidateAllStaffAdminPermissionCaches();
  const users = await AdminUserModel.find({ roleId: doc._id })
    .select("userId")
    .lean();
  for (const u of users) {
    await invalidateAdminPermissionCache(String(u.userId));
  }
  void writeAuditLog(req, AuditAction.ADMIN_ROLE_UPDATED, {
    actorId: actor.user._id,
    targetType: "admin_role",
    targetId: String(doc._id),
    metadata: {
...
export async function deleteRoleSoft(
  req: Request,
  res: Response,
): Promise<void> {
  const actor = req as StaffManagementRequest;
  ...
  const activeUsers = await AdminUserModel.countDocuments({
    roleId: doc._id,
    isActive: true,
  });
  if (activeUsers > 0) {
    sendAdminError(
      ...
      409,
      "CONFLICT",
      "Archive admin users using this role first, or reassign them.",
    );
    return;
  }
  ...
  await doc.save();
  await invalidateAllStaffAdminPermissionCaches();
  void writeAuditLog(req, AuditAction.ADMIN_ROLE_DELETED, {
    actorId: actor.user._id,
    targetType: "admin_role",
    targetId: String(doc._id),
