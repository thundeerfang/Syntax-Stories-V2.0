...
export async function getBlogEngagementByMetric(req: Request, res: Response): Promise<void> {
  const id = typeof req.params.id === 'string' ? req.params.id.trim() : '';
  const metricRaw = typeof req.params.metric === 'string' ? req.params.metric.trim() : '';
