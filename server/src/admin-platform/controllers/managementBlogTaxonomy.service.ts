...
async function followerCountByCategorySlug(slug: string): Promise<number> {
  return BlogCategoryFollowModel.countDocuments({
    categorySlug: slug.toLowerCase(),
  });
}
export async function listAdminBlogCategories(opts: {
  q?: string;
  sort?: string;
}) {
  ...
  const rows = await BlogCategoryModel.find(filter)
  ...
  if (sort === "posts") {
    ...
    items.sort(
      (a, b) =>
        b.followerCount - a.followerCount || a.name.localeCompare(b.name),
    );
  } else if (sort === "name") {
...
export async function createAdminBlogCategory(body: {
  ...
  const slug =
    ...
    (await reserveUniqueSlug(name, blogTaxonomySlugTaken, {
      maxLen: CATEGORY_SLUG_MAX,
    }));
...
export async function bulkCreateAdminBlogCategories(
...
export async function listAdminBlogTags(opts: { q?: string; sort?: string }) {
  ...
  const rows = await BlogTagModel.find(filter)
    .sort({ sortOrder: 1, name: 1 })
    .lean();
  const items = await Promise.all(
    rows.map(async (t) => {
...
export async function createAdminBlogTag(body: {
  ...
  if (exists)
    return { error: "CONFLICT" as const, message: "Tag slug already exists" };
