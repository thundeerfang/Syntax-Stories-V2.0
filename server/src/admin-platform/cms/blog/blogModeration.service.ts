...
async function loadPostForModeration(postId: string) {
  ...
  if (post.deletedAt) {
    throw new BlogModerationError(409, 'Post is in trash — restore it first', 'CONFLICT');
  }
  return post;
}
...
async function notifyAuthor(
  ...
  const user = await UserModel.findById(authorId).select('email fullName username').lean();
  if (!user?.email) return;
  const name = user.fullName?.trim() || user.username || 'there';
  await sendBlogModerationEmail({ to: user.email, authorName: name, postTitle, action });
}
...
export async function adminSoftDeleteBlog(
  ...
  await Promise.all([
    ...
    suspendRepostBookmarkContributionsForPost(post._id as mongoose.Types.ObjectId),
  ]);
  ...
  void writeAuditLog(req, AuditAction.ADMIN_BLOG_SOFT_DELETED, {
    ...
    metadata: { title: post.title, slug: post.slug, authorId: String(post.authorId) },
  });

  void notifyAuthor(post.authorId as mongoose.Types.ObjectId, post.title, 'deleted');
...
export async function adminSuspendBlog(
  ...
  void notifyAuthor(post.authorId as mongoose.Types.ObjectId, post.title, 'suspended');
...
export async function adminUnsuspendBlog(
  ...
  const hadPublished =
    post.publishedAt instanceof Date && !Number.isNaN(post.publishedAt.getTime());
