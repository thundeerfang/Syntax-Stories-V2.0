...
export type AdminBlogContentParse = {
  ...
  images: Array<{ url: string; source: 'thumbnail' | 'block'; blockIndex?: number; blockType?: string }>;
  textExcerpt: string;
};
...
function previewFromBlock(block: Record<string, unknown>): string {
  ...
  switch (type) {
    case 'paragraph':
    case 'heading': {
      const text = typeof p.text === 'string' ? p.text : typeof p.markdown === 'string' ? p.markdown : '';
...
function collectImagesFromBlock(
  ...
  const push = (url: unknown) => {
    if (typeof url === 'string' && url.trim().length > 0) {
      out.push({ url: url.trim(), source: 'block', blockIndex: index, blockType: type });
    }
  };
...
/** Parse stored blog `content` JSON for admin inspection (no mutation). */
export function parseBlogContentForAdmin(
  ...
  blocks.forEach((raw, index) => {
    ...
    const sectionId = typeof block.sectionId === 'string' ? block.sectionId : undefined;
    blockSummaries.push({
      index,
      id,
      type,
