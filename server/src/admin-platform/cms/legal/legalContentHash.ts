...
export function canonicalLegalPayload(
  title: string,
  summary: string,
  body: string,
): string {
...
export function computeLegalContentHash(
  title: string,
  summary: string,
  body: string,
): string {
  const bytes = Buffer.from(
    canonicalLegalPayload(title, summary, body),
    "utf8",
  );
...
export function verifyRevisionHash(params: {
  ...
  return (
    computeLegalContentHash(params.title, params.summary, params.body) ===
    params.contentHash
  );
}
