...
function isMutationExempt(p: string, method: string): boolean {
  ...
  if (
    method === "POST" &&
    /^\/api\/blog\/p\/[^/]+\/[^/]+\/comments\/[^/]+\/like$/.test(p)
  ) {
    return true;
  }
  if (
    method === "POST" &&
    (p === "/api/blog/engagement/viewer-state" ||
      p === "/api/blog/respect/viewer-state")
  ) {
    return true;
  }
  return false;
  ...
  try {
    ...
    if (env.LEGAL_ENFORCEMENT_MODE === "strict") {
      res.status(403).json({
        ...
        message:
          "You must accept the latest Terms and Privacy Policy before this action.",
      });
      return;
    }
