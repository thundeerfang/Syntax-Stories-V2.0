...
export async function postAccept(req: Request, res: Response): Promise<void> {
  ...
  if (!parsed.success) {
    res
      .status(422)
      .json({ ok: false, message: 'Validation failed', issues: parsed.error.flatten() });
    return;
  }
  const b = parsed.data;
  await completeWithIdempotency(req, res, 'POST /legal/accept', uid, req.body, async () => {
    ...
    if (!pub) {
      return {
        status: 404,
        body: { ok: false, code: 'LEGAL_NOT_FOUND', message: 'No published policy' },
      };
    }
    ...
    if (
      ...
      return {
        status: 500,
        body: { ok: false, message: 'Server integrity error for revision hash.' },
      };
    }
...
export async function adminBootstrapPolicies(_req: Request, res: Response): Promise<void> {
  const { ensureLegalPoliciesSeed } = await import('./ensureLegalPoliciesSeed.js');
...
export async function adminGetRevision(req: Request, res: Response): Promise<void> {
  const policyId = String(req.params.policyId);
...
export async function adminDeleteRevision(req: Request, res: Response): Promise<void> {
  const policyId = String(req.params.policyId);
  const revisionId = String(req.params.revisionId);
  ...
  if (rev.status === 'published') {
    res.status(409).json({
      ok: false,
      message: 'Cannot delete a published revision. Supersede it by publishing a newer version first.',
    });
    return;
  }
    ...
    await completeWithIdempotency(
      ...
      async () => {
        if (policy.status !== 'approved' && policy.status !== 'published') {
          return {
            status: 409,
            body: { ok: false, message: 'Publish requires approved or published policy' },
          };
        }
        try {
          ...
          return {
            status: 400,
            body: { ok: false, code: 'SLUG_KIND_MISMATCH', message: 'Slug does not match kind' },
          };
        }
