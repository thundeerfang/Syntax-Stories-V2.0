...
export async function listPublishedArticles(req: Request, res: Response): Promise<void> {
  try {
    ...
    const icon = typeof req.query.icon === 'string' ? req.query.icon : undefined;
    const rawSort = typeof req.query.sort === 'string' ? req.query.sort : undefined;
    const sort = rawSort === 'oldest' ? 'oldest' : 'latest';
    const envelope = await helpService.listPublished(page, pageSize, { category, q, icon, sort });
...
export async function getHelpIconFacets(_req: Request, res: Response): Promise<void> {
  try {
    const icons = await helpService.listPublishedIconFacets();
...
export async function listAdminArticles(req: Request, res: Response): Promise<void> {
  try {
    ...
    const pageSize = Math.min(
      100,
      Math.max(1, Number.parseInt(String(req.query.pageSize ?? '30'), 10) || 30)
    );
...
export async function patchAdminArticle(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (body.icon !== undefined) patchInput.icon = normalizeHelpIcon(body.icon);
    if (body.sortOrder !== undefined) patchInput.sortOrder = Number(body.sortOrder);
    if (body.expectedDraftVersion !== undefined)
      patchInput.expectedDraftVersion = body.expectedDraftVersion;
  ...
  try {
    ...
    const expectedPublishedVersion =
      req.body?.expectedPublishedVersion !== undefined
        ...
        : undefined;
    const doc = await helpService.publish(
      userId,
      staffRole,
      paramId(req, 'id'),
...
export async function getHelpHubConfig(_req: Request, res: Response): Promise<void> {
  try {
    const data = await helpService.getHubConfig();
    res.setHeader('Cache-Control', 'public, max-age=60, stale-while-revalidate=300');
...
export async function getAdminHelpHubConfig(_req: Request, res: Response): Promise<void> {
  try {
    const data = await helpService.getHubConfig();
    res.json({ success: true, data });
  } catch (err) {
...
export async function patchAdminHelpHubConfig(req: Request, res: Response): Promise<void> {
  try {
    const body = req.body as {
      title?: string;
      description?: string;
    ...
    const data = await helpService.updateHubConfig(body);
    res.json({ success: true, data });
  } catch (err) {
    sendError(res, err);
  }
