...
function clientMeta(req: Request): {
  ...
  return {
    ip,
    userAgent,
    fingerprint: computeDeviceFingerprint(ip, userAgent),
  };
}
...
export async function listTrustedDevices(userId: string) {
  return TrustedDeviceModel.find({ userId, revokedAt: null })
    .sort({ lastSeenAt: -1 })
    .lean();
}
...
> {
  ...
  return {
    ok: false,
    reason: "Unrecognized device. Sign in again from a trusted device.",
  };
}
...
> {
  ...
  const count = await TrustedDeviceModel.countDocuments({
    userId,
    revokedAt: null,
  });
  if (count === 0) {
  ...
  return {
    ok: false,
    reason:
      "New device detected. Trust this device from Security settings after signing in.",
  };
}
