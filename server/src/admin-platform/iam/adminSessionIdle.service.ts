...
/** Changes on each API process start — used to re-prompt 2FA after deploy/restart. */
...
async function ensureGraceDeadline(sessionId: string): Promise<string | null> {
  const redis = getRedis();
  if (!redis) return new Date(Date.now() + ADMIN_STEP_UP_GRACE_SEC * 1000).toISOString();
...
export async function touchAdminSessionActivity(sessionId: string): Promise<Date | null> {
...
export async function getAdminSessionIdleStatus(
  ...
  const session = await SessionModel.findById(sessionId).select('lastActiveAt revoked').lean();
  if (!session || session.revoked) return null;

  const lastActiveAt = session.lastActiveAt ?? new Date();
  const idleExpiresAt = new Date(lastActiveAt.getTime() + ADMIN_IDLE_STEP_UP_SEC * 1000);
  const now = Date.now();
...
/** Call after successful admin 2FA / step-up to refresh idle + boot ack. */
export async function completeAdminStepUp(sessionId: string, userId: string): Promise<void> {
