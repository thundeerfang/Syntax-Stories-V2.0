...
function getKey(): Buffer {
  ...
  if (!raw) {
    throw new Error(
      "ADMIN_USER_REF_KEY or JWT_SECRET required for admin user refs",
    );
  }
...
export function encodeAdminUserRef(objectId: string): string {
  ...
  return (
    ADMIN_USER_REF_PREFIX + Buffer.concat([iv, tag, enc]).toString("base64url")
  );
}
export function decodeAdminUserRef(ref: string): string | null {
  ...
  try {
    ...
    const raw = Buffer.from(
      trimmed.slice(ADMIN_USER_REF_PREFIX.length),
      "base64url",
    );
...
export function adminUserRefFromObjectId(
...
export function resolveManagementUserParam(param: string):
  | {
      objectId: string;
    }
  | {
