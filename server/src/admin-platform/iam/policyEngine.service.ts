...
import { actorHasZone, zoneForPermission, type SecurityZone } from './securityZones.config.js';
...
export type AuthorizeContext = {
  ...
  userAgent?: string;
  environment?: string;
  /** Target role level for assignment mutations. */
...
/**
 * Hybrid RBAC + ABAC + zone checks (Phase 4).
 * Call after `staffManagementContext` attached permissions.
 */
export async function authorize(input: AuthorizeInput): Promise<AuthorizeResult> {
  ...
  const hasPermission =
    perms?.has(permission) === true || (graph != null && graphAllows(graph, permission));
  if (!hasPermission) {
    return {
      allow: false,
  ...
  if (
    ...
    (permission === 'admin_role:manage' || permission === 'admin_assignment:manage')
  ) {
...
export function requireZoneForPermission(permission: string): SecurityZone | null {
