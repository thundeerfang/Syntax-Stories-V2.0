...
export async function getActiveTemporalPermissions(
  userId: string,
): Promise<string[]> {
...
export async function listTemporalGrantsForUser(
  userId: string,
  ...
  return TemporalPermissionGrantModel.find(filter)
    .sort({ expiresAt: 1 })
    .lean();
}
export async function revokeTemporalGrant(
  grantId: string,
  userId: string,
): Promise<boolean> {
