...
async function loadRoleMeta(userId: string): Promise<{
  roleIds: string[];
  roleName: string | null;
}> {
  ...
  const { AdminUserRoleModel } =
    await import("../rbac/models/AdminUserRole.js");
  const links = await AdminUserRoleModel.find({ userId })
    .select("roleId")
    .lean();
  const roleIds = links.map((l) => String(l.roleId));
  if (roleIds.length === 0) return { roleIds: [], roleName: null };
  const roles = await AdminRoleModel.find({
    _id: { $in: roleIds },
    deletedAt: null,
  })
    .select("name")
...
export async function deleteStaffPermissionSnapshot(
  sessionId: string,
): Promise<void> {
  const redis = getRedis();
  if (!redis) return;
