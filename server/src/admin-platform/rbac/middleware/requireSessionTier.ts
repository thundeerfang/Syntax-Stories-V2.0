...
import { tierMeetsRequired, type SessionTier } from '../../iam/sessionTier.config.js';
...
export function requireSessionTier(minTier: SessionTier) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (!sessionId) {
      sendAdminError(res, 403, 'SESSION_TIER_REQUIRED', 'Session tier could not be verified.');
      return;
    }
    const session = await SessionModel.findById(sessionId).select('sessionTier').lean();
    const tier = (session?.sessionTier as SessionTier | undefined) ?? 'standard';
