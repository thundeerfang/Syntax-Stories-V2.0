...
export type RewardSourceType = 'referral' | 'achievement' | 'quest' | 'campaign';
...
const RewardGrantSchema = new Schema<IRewardGrant>(
  ...
  { timestamps: { createdAt: true, updatedAt: false }, collection: 'rewardgrants' }
);
...
export const RewardGrantModel: Model<IRewardGrant> =
  mongoose.models?.RewardGrant ?? mongoose.model<IRewardGrant>('RewardGrant', RewardGrantSchema);
