...
export function oauthLinkHandler(
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!k?.trim()) {
      return res.redirect(
        `${base}/settings?error=${encodeURIComponent("Invalid link request")}`,
      );
    }
    const redis = getRedis();
    if (!redis) {
      return res.redirect(
        `${base}/settings?error=${encodeURIComponent("Linking unavailable")}`,
      );
    }
...
export function oauthCallbackHandler(
  ...
  const { strategy, failureLabel, auditProvider, clientCallbackSlug, idField } =
    params;
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    passport.authenticate(
      strategy,
      { session: false },
      async (err: unknown, userObj?: unknown) => {
        if (err) {
        ...
        if (!u?._id) {
          return res.redirect(
            `${base}/login?error=${encodeURIComponent(failureLabel)}`,
          );
        }
        ...
        const { accessToken, refreshToken, session } =
          await createSessionAndTokens(String(u._id), req);
