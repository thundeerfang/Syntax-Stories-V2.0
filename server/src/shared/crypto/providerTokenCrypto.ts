...
export function sealProviderToken(
  ...
  return (
    PROVIDER_TOKEN_PREFIX + Buffer.concat([iv, tag, enc]).toString("base64url")
  );
}
export function unsealProviderToken(
  ...
  if (!key) {
    console.warn(
      "[providerTokenCrypto] cannot decrypt: OAUTH_PROVIDER_TOKEN_KEY missing",
    );
    return undefined;
  }
  try {
    const raw = Buffer.from(
      sealed.slice(PROVIDER_TOKEN_PREFIX.length),
      "base64url",
    );
