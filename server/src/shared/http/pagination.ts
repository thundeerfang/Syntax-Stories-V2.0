/** List/query pagination defaults and caps. */
...
export function parseLimit(
  raw: unknown,
  profile: { default: number; max: number }
): number {
...
/** Admin list endpoints: floor positive numbers, else default. */
export function parseAdminListLimit(raw: unknown): number {
  const n = Number(raw);
  const base = Number.isFinite(n) && n > 0 ? Math.floor(n) : PAGINATION.adminList.default;
  return Math.min(base, PAGINATION.adminList.max);
}

/** Admin taxonomy list endpoints. */
export function parseAdminTaxonomyListLimit(raw: unknown): number {
  const n = Number(raw);
  const base = Number.isFinite(n) && n > 0 ? Math.floor(n) : PAGINATION.adminTaxonomy.default;
