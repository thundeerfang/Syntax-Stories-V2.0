...
function parseClientMetaJson(raw) {
    if (raw == null || raw === '')
        return undefined;
    try {
...
export function parseMultipartFeedback(body, isAuthed) {
    const flat = {};
    ...
    if (!isAuthed) {
        ...
        if (!isValidEmail(em))
            return { ok: false, message: 'email: Valid email is required.' };
        return {
            ok: true,
            data: {
...
//# sourceMappingURL=feedbackMultipart.validation.js.map