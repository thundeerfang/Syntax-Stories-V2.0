...
function generateReferralCodeChars(length) {
...
export function normalizeReferralCode(raw) {
    ...
    if (!code || code.length < 8 || code.length > 16)
        return null;
    if (!REFERRAL_CODE_REGEX.test(code))
        return null;
    return code;
}
function signReferralCookiePayload(code) {
...
function readSignedReferralCookie(raw) {
...
export function parseSignupOAuthNonceFromState(state) {
    if (typeof state !== 'string' || !state.startsWith('signup:'))
        return null;
...
async function readOAuthReferralFromReq(req) {
 ...
 */
export async function resolveReferralInput(req) {
...
export async function resolveCodeForDisplay(code) {
...
export async function lookupReferrerIdByCode(code) {
    const norm = normalizeReferralCode(code);
    if (!norm)
        return null;
    const redis = getRedis();
    if (redis) {
        try {
            ...
            if (cached === NEGATIVE_CACHE_SENTINEL)
                return null;
            if (cached && mongoose.isValidObjectId(cached))
                return cached;
        }
        catch (err) {
            console.warn(JSON.stringify({ event: 'referral_redis_degraded', err: String(err) }));
...
export async function invalidateReferralCodeCache(normalizedCode) {
...
export async function ensureReferralCodeForUser(userId) {
    ...
    for (let attempt = 0; attempt < 12; attempt++) {
        ...
        try {
            const updated = await UserModel.findOneAndUpdate({ _id: userId, $or: [{ referralCode: { $exists: false } }, { referralCode: null }, { referralCode: '' }] }, { $set: { referralCode: code } }, { new: true, select: 'referralCode' }).lean();
...
export async function applyReferralOnNewUser(args) {
    if (!env.REFERRALS_ENABLED)
        return;
    const { newUser, refCode, source } = args;
    const rc = refCode;
    if (rc == null || typeof rc !== 'string' || !rc.trim())
        return;
    ...
    if (referrerId.equals(newUser._id)) {
        return;
    }
    ...
    const now = new Date();
    const res = await UserModel.updateOne({ _id: newUser._id, referredByUserId: null }, {
        $set: {
    ...
    if (source)
        newUser.set('referralSource', source.slice(0, 32));
...
export const REFERRAL_COOKIE = { name: COOKIE_NAME, maxMs: COOKIE_MAX_MS };
export function buildSignedReferralCookieValue(code) {
