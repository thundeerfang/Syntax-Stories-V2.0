...
export function oauthLinkHandler(strategy, authenticateOptions = {}) {
...
export function oauthCallbackHandler(params) {
    const { strategy, failureLabel, auditProvider, clientCallbackSlug, idField } = params;
    return (req, res, next) => {
        const base = getFrontendRedirectBase();
        passport.authenticate(strategy, { session: false }, async (err, userObj) => {
            if (err) {
            ...
            if (dbUser?.twoFactorEnabled) {
                try {
                    ...
                    return res.redirect(`${base}/${clientCallbackSlug}?twoFactorRequired=1&challengeToken=${encodeURIComponent(challengeToken)}`);
                }
                catch {
            ...
            const providerId = typeof rawId === 'string' || typeof rawId === 'number' ? String(rawId) : '';
            ...
            if (exchangeCode) {
                return res.redirect(`${base}/${clientCallbackSlug}?code=${encodeURIComponent(exchangeCode)}`);
            }
            return res.redirect(`${base}/login?error=${encodeURIComponent('Sign-in is temporarily unavailable (session exchange). Ensure Redis is configured.')}`);
        })(req, res, next);
    };
}
//# sourceMappingURL=oauthExpress.js.map