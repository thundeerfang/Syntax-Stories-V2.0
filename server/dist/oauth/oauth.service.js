import { UserModel, DEFAULT_AVATAR_URL } from '../models/User.js';
...
async function attachFreeSubscription(userId) {
...
function signupUsername(provider, n) {
...
function signupEmail(provider, n) {
...
function newUserBaseDoc(provider, n, accessToken) {
...
function passportShape(provider, user) {
    const out = { _id: user._id };
    if (provider === 'google')
        out.googleId = user.googleId;
    if (provider === 'github')
        out.gitId = user.gitId;
    if (provider === 'facebook')
        out.facebookId = user.facebookId;
    if (provider === 'x')
        out.xId = user.xId;
    if (provider === 'discord')
        out.discordId = user.discordId;
    return out;
}
async function finalizeOAuthAccountLink(input) {
...
async function handleLink(req, provider, linkKey, accessToken, n) {
...
async function handleLogin(provider, accessToken, n) {
    const label = PROVIDER_LABEL[provider];
    switch (provider) {
        case 'google': {
            ...
            if (!existingUser || !existingUser.isGoogleAccount) {
                throw new Error(`No account is linked to this ${label}. Please sign up or link ${label} from settings.`);
            }
...
async function handleSignup(provider, accessToken, n, req) {
    ...
    if (existingByEmail) {
        throw new Error(`An account with this email already exists. Please sign in, then link ${label} from settings.`);
    }
 ...
 */
export async function handleOAuthProviderAuth(input) {
