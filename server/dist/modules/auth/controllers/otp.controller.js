...
import { sendAuthEmail, isAuthEmailConfigured, getEmailSendErrorMessage, isMailSendError, } from '../../../infrastructure/mail/sendAuthEmail.js';
...
function otpEmailSendFailureMessage(err, redisUnavailableMessage) {
...
export async function getAltchaChallenge(_req, res) {
...
export async function sendOtp(req, res) {
    try {
      ...
      `,
        ...
        console.error(err);
        const message = otpEmailSendFailureMessage(err, 'Service temporarily unavailable. Try again later.');
        const redisDown = err instanceof Error && err.message === 'Redis required for OTP';
        res.status(500).json({
            message,
            success: false,
            ...(redisDown ? { code: 'REDIS_UNAVAILABLE' } : {}),
        });
    }
}
...
async function failOtpVerification(req, res, redis, normalizedEmail, reason) {
    ...
    const msg = reason === 'stale_otp_version'
...
async function createUserFromEmailSignup(req, normalizedEmail, signupFullName) {
...
async function resolveUserAfterOtpVerified(req, res, userPrecheck, purpose, stored, normalizedEmail) {
    const signupFullName = purpose === 'signup' ? stored.fullName : undefined;
...
export async function verifyOtp(req, res) {
    try {
        const { email, code, otpVersion } = req.body;
        const normalizedEmail = email.toLowerCase().trim();
        ...
        if (await rejectVerifyIfOtpRateLimited(redis, normalizedEmail, res))
            return;
        const userPrecheck = await UserModel.findOne({ email: normalizedEmail });
        const purpose = userPrecheck ? 'login' : 'signup';
        const stored = purpose === 'login'
