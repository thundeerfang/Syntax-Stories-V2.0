...
function assertPayloadJsonSize(value, maxChars, label) {
    let encoded;
    try {
...
function sanitizeGenericPayload(p) {
    if (p == null)
        return {};
    if (!isPlainObject(p))
...
function sanitizeParagraphPayload(p) {
    if (p == null)
        return {};
    if (!isPlainObject(p))
        throw new Error('paragraph payload must be an object');
    const out = {};
...
function sanitizeHeadingPayload(p) {
    if (p == null)
        return {};
    if (!isPlainObject(p))
        throw new Error('heading payload must be an object');
    const out = {};
    if (typeof p.text === 'string')
...
function sanitizeImagePayload(p) {
    if (p == null)
        return {};
    if (!isPlainObject(p))
    ...
    if (typeof p.url === 'string')
        out.url = truncateString(p.url, MAX_URL_LEN);
    if (typeof p.title === 'string')
        out.title = truncateString(p.title, MAX_CAPTION);
    if (typeof p.altText === 'string') {
        out.altText = truncateString(p.altText, MAX_CAPTION);
    }
...
function sanitizeVideoEmbedPayload(p) {
    if (p == null)
        return {};
    if (!isPlainObject(p))
    ...
    if (p.layout === 'row' || p.layout === 'column')
        out.layout = p.layout;
    if (p.size === 'sm' || p.size === 'md' || p.size === 'lg')
        out.size = p.size;
    return out;
}
function sanitizeCodePayload(p) {
    if (p == null)
        return {};
    if (!isPlainObject(p))
        throw new Error('code payload must be an object');
    const out = {};
    if (typeof p.code === 'string') {
        out.code = truncateString(p.code, MAX_CODE_BODY_CHARS);
    }
    else if (typeof p.text === 'string') {
        out.code = truncateString(p.text, MAX_CODE_BODY_CHARS);
    }
    const lang = p.language;
    if (typeof lang === 'string' && /^[a-z0-9#+.\-]{1,72}$/i.test(lang.trim())) {
        out.language = lang.trim().toLowerCase().slice(0, 72);
    }
    const src = p.languageSource;
    if (src === 'auto' || src === 'manual')
        out.languageSource = src;
    return out;
}
function sanitizeGithubPayload(p) {
    if (p == null)
        return {};
    if (!isPlainObject(p))
        throw new Error('githubRepo payload must be an object');
    const out = {};
    for (const key of ['owner', 'repo', 'name', 'url', 'avatarUrl']) {
        if (typeof p[key] === 'string')
            out[key] = truncateString(p[key], MAX_URL_LEN);
    }
    if (typeof p.description === 'string')
        out.description = truncateString(p.description, MAX_GITHUB_DESC);
    return out;
}
function sanitizeTablePayload(p) {
    if (p == null)
        return { rows: [] };
    if (!isPlainObject(p))
...
function sanitizeMermaidPayload(p) {
    if (p == null)
        return { source: '' };
    if (!isPlainObject(p))
    ...
    if (typeof p.source === 'string') {
        out.source = truncateString(p.source, MAX_MERMAID_SOURCE_CHARS);
    }
    else {
        out.source = '';
    }
...
function sanitizeUnsplashPayload(p) {
    if (p == null)
        return {};
    if (!isPlainObject(p))
 ...
 */
export function validateBlogPostContent(raw) {
 ...
 */
export function sanitizeThumbnailUrl(input) {
...
//# sourceMappingURL=blogContentValidation.js.map