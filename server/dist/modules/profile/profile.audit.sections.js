...
function auditStackTools(log, currentProfile, updatedProfile) {
...
function auditEducation(log, currentProfile, updatedProfile) {
    ...
    for (const id of oldIds) {
        if (!newIds.has(id))
            log(AuditAction.EDUCATION_REMOVED, 'education', { eduId: id });
    }
}
function auditWorkExperiences(log, currentProfile, updatedProfile) {
    ...
    for (const id of oldIds) {
        if (!newIds.has(id))
            log(AuditAction.WORK_REMOVED, 'work', { workId: id });
    }
}
function auditCertifications(log, currentProfile, updatedProfile) {
...
function auditProjects(log, currentProfile, updatedProfile) {
...
function auditOpenSource(log, currentProfile, updatedProfile) {
    ...
    for (let i = 0; i < oldO.length; i++) {
        ...
        if (!newKeys.has(key))
            log(AuditAction.OPEN_SOURCE_REMOVED, 'open_source', { repositoryUrl: oldO[i].repositoryUrl, title: oldO[i].title });
    }
}
function auditMySetup(log, currentProfile, updatedProfile) {
...
export function runProfileSectionAudits(payload) {
...
//# sourceMappingURL=profile.audit.sections.js.map