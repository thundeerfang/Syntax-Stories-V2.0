...
function sendProfileSuccess(res, user) {
    res.status(200).json({
        success: true,
        data: { user },
    });
}
function sendProfileError(res, status, code, message, details) {
    const body = { success: false, code, message };
    if (details !== undefined)
        body.details = details;
    res.status(status).json(body);
}
function logProfileWriteTiming(section, totalMs) {
...
export async function updateProfile(req, res) {
    ...
    try {
        ...
        if (!result.ok) {
            sendProfileError(res, result.status, result.code ?? ProfileErrorCode.NO_VALID_FIELDS, result.message);
            return;
        }
        sendProfileSuccess(res, result.user);
    }
        ...
        const code = err?.code;
        if (code === 11000) {
            sendProfileError(res, 409, ProfileErrorCode.USERNAME_TAKEN, 'Username is already taken. Choose another.');
            return;
        }
        console.error(err);
        sendProfileError(res, 500, ProfileErrorCode.INTERNAL_ERROR, 'Internal Server Error 💀');
