...
function flushSectionContent(sections, header, content) {
    if (content.length === 0)
        return;
...
function looksLikeSectionHeader(trimmed, normalized) {
    if (normalized.length >= 50 || trimmed.length >= 80)
        return false;
    if (SECTION_HEADERS.some((h) => normalized.startsWith(h) || normalized === h))
        return true;
    return SECTION_HEADER_SHORT.test(normalized);
}
function canonicalSectionKey(token, normalizedLine) {
    let key = token;
    if (key === 'work')
        key = 'experience';
    if (key === 'certificate' || key === 'licenses')
        key = 'certifications';
    if (key === 'academic')
        key = 'education';
    if (key === 'employment' || key === 'professional')
        key = 'experience';
    if (key === 'technical' || key === 'technologies')
        key = 'skills';
...
function firstDateFragmentInBlock(block) {
    return (RE_MONTH_NAME_YEAR_A.exec(block)?.[0] ??
 ...
 */
function parseWorkExperienceFirstLine(titleLine) {
...
function looksLikeWorkExperienceHeader(line) {
 ...
 */
function parseWorkExperienceFirstLineRelaxed(titleLine) {
    const line = titleLine.trim();
    const relaxedClosed = /\s*\((\d{1,2}\/\d{4})\s*[-–—]\s*(\d{1,2}\/\d{4}|present|current|now)\s*\)\s*$/i;
...
function extractLinkedIn(text) {
    ...
    if (m)
        return m[0].startsWith('http') ? m[0].trim() : `https://${m[0].trim()}`;
    return undefined;
}
...
function extractSkills(text) {
    const skills = [];
    const skillsSection = findSections(text).get('skills') ??
    ...
    const KNOWN_PLATFORM = /aws|docker|kubernetes|html|css|git|redux|angular|vue|mongodb|postgres|graphql|rest|api|linux|agile|scrum/i;
...
function parseEducationSchoolLine(line) {
    const trimmed = line.trim();
...
function firstDegreeInBlock(block) {
...
function isNextLineNewWorkRole(nextLine) {
    return (!!(parseWorkExperienceFirstLine(nextLine) ?? parseWorkExperienceFirstLineRelaxed(nextLine)) ||
        looksLikeWorkExperienceHeader(nextLine));
}
function isLikelyWorkDescriptionLine(nextLine) {
    return (nextLine.length > 10 &&
        !/^\d{1,2}\/\d{4}\)?\s*$/i.test(nextLine) &&
        !/^\d{4}\s*[-–—]/.test(nextLine));
}
function gatherWorkDescriptionLines(lines, startJ) {
    const desc = [];
    let j = startJ;
    while (j < lines.length) {
        const nextLine = lines[j];
        if (isNextLineNewWorkRole(nextLine))
            break;
        if (isLikelyWorkDescriptionLine(nextLine))
            desc.push(nextLine);
        j++;
    }
    return { desc, endJ: j };
...
function parseHeuristicWorkRow(lines, i, titleLine) {
...
function normalizeAndHint(extracted) {
    ...
    if (normalized.education?.length) {
        normalized.education = normalized.education.map((ed, i) => {
            const missing = [];
            if (!ed.startDate?.trim())
                missing.push('startDate');
            if (!ed.endDate?.trim() && !ed.currentEducation)
                missing.push('endDate');
            if (missing.length) {
...
export function parseCvFromText(text) {
    const extracted = {};
    const missingFields = [];
    ...
    if (skills.length > 0)
        extracted.stackAndTools = skills.slice(0, STACK_AND_TOOLS_MAX);
    else
        missingFields.push('stackAndTools');
    ...
    if (workExperiences?.length)
        extracted.workExperiences = workExperiences;
    else
...
//# sourceMappingURL=parseCvFromPdf.js.map