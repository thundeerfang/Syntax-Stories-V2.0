...
async function tryBlurDataUrl(imagePath) {
    try {
...
const router = Router();
const { avatars: AVATARS_DIR, covers: COVERS_DIR, media: MEDIA_DIR, logos: LOGOS_DIR, schoolLogos: SCHOOL_LOGOS_DIR, orgLogos: ORG_LOGOS_DIR } = getDefaultUploadStorage().dirs;
...
const logoFilter = (_req, file, cb) => {
    ...
    if (allowed)
        ...
        cb(new Error('Only images (JPEG, PNG, WebP, SVG) are allowed for logos'));
};
...
function assertRasterMagicBytesOrCleanup(filePath, mimetype) {
...
function getPublicUrl(req, pathSegment) {
...
function applyOptionalRasterCrop(image, body, meta) {
...
async function finalizeRasterLogo(req, inputPath, outDir, file) {
...
router.post('/avatar', verifyToken, uploadAvatar.single('avatar'), async (req, res) => {
    ...
    try {
        ...
        image = applyOptionalRasterCrop(image, req.body, meta);
        await image
...
router.post('/cover', verifyToken, uploadCover.single('cover'), async (req, res) => {
    if (!req.file) {
        res.status(400).json({ success: false, message: 'No file uploaded' });
        return;
    }
...
router.post('/media', verifyToken, uploadMedia.single('media'), async (req, res) => {
    if (!req.file) {
        res.status(400).json({ success: false, message: 'No file uploaded' });
        return;
    }
...
router.post('/company-logo', verifyToken, uploadLogo.single('logo'), async (req, res) => {
    if (!req.file) {
        res.status(400).json({ success: false, message: 'No file uploaded' });
        return;
    }
...
router.post('/school-logo', verifyToken, uploadSchoolLogo.single('logo'), async (req, res) => {
    if (!req.file) {
        res.status(400).json({ success: false, message: 'No file uploaded' });
        return;
    }
...
router.post('/org-logo', verifyToken, uploadOrgLogo.single('logo'), async (req, res) => {
    if (!req.file) {
        res.status(400).json({ success: false, message: 'No file uploaded' });
        return;
    }
