#!/usr/bin/env node
/**
 * Strip comments and collapse extra blank lines in TS under src/.
 * Uses the TypeScript printer (removeComments) so strings stay intact.
 *
...
const targetRoots = (args.length ? args : DEFAULT_DIRS).map((d) =>
  path.isAbsolute(d) ? d : path.join(serverRoot, d),
);
...
for (const root of targetRoots) {
  if (!fs.existsSync(root)) {
    console.warn(`[strip-comments] skip missing: ${path.relative(serverRoot, root)}`);
    continue;
  }
  for (const file of walk(root)) {
    ...
    if (stripFile(file)) {
      changed += 1;
      console.log(`updated ${path.relative(serverRoot, file)}`);
    }
  }
}
