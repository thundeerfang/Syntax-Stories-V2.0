...
export async function fetchTagsExplore(): Promise<TagsExploreResponse> {
  ...
  const data = (await readJson(r)) as TagsExploreResponse & {
    message?: string;
  };
...
export async function fetchTagsList(opts?: {
  ...
  const data = (await readJson(r)) as PaginatedTagsResponse & {
    message?: string;
  };
  if (!r.ok) throw new Error(data.message ?? r.statusText);
  return {
...
export async function fetchCategoriesList(opts?: {
  ...
  const r = await blogPublicFetch(url);
  const data = (await readJson(r)) as PaginatedCategoriesResponse & {
    message?: string;
  };
  if (!r.ok) throw new Error(data.message ?? r.statusText);
  return {
