...
export async function fetchLegalMeStatus(accessToken: string): Promise<LegalMeStatusResponse> {
...
export async function postDataDeletionRequest(
  ...
  const url = resolveSameOriginRequestUrl(`${legalBase()}/data-deletion-requests`);
  const headers: Record<string, string> = {
    Authorization: `Bearer ${accessToken}`,
    'Content-Type': 'application/json',
  };
