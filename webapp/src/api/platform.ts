...
export type { PublicPlatformStatsDto, PublicPlatformStatsResponse } from '@/contracts/platformApi';

export async function fetchPlatformStats(): Promise<PublicPlatformStatsDto> {
  const res = await fetch(`${base()}/api/platform/stats`, { credentials: 'include' });
