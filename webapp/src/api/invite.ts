...
export const inviteApi = {
  resolveCode(code: string): Promise<InviteResolveResponse> {
    ...
    return fetch(`${getApiBase()}/api/invites/resolve?${params}`).then(async (r) => {
      if (!r.ok) {
        return { valid: false as const, message: 'Could not verify referral code.' };
      }
      return r.json() as Promise<InviteResolveResponse>;
    });
  },

  recordShare(token: string, channel: InviteShareChannel, referralCode?: string): Promise<void> {
    return fetch(`${getApiBase()}/api/invites/share`, {
      ...
      credentials: 'include',
      body: JSON.stringify({ channel, ...(referralCode ? { referralCode } : {}) }),
    }).then((r) => {
  ...
  getLeaderboard(limit = 20): Promise<{
    ...
    return fetch(`${getApiBase()}/api/invites/leaderboard?limit=${limit}`).then((r) => {
