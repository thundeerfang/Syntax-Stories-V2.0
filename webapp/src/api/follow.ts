...
export const followApi = {
  getPublicProfile: (username: string) =>
    fetch(
      `${getApiBase()}/api/follow/profile/${encodeURIComponent(username)}`,
    ).then((r) => {
  ...
  getFollowCounts: (username: string) =>
    fetch(
      `${getApiBase()}/api/follow/counts/${encodeURIComponent(username)}`,
    ).then((r) => {
      if (!r.ok) throw new Error(r.statusText);
      return r.json() as Promise<{
        success: boolean;
  ...
  follow: async (username: string, accessToken: string) => {
    const r = await fetch(
      `${getApiBase()}/api/follow/${encodeURIComponent(username)}`,
      {
        method: "POST",
        headers: {
