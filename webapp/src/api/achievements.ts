...
export const achievementsApi = {
  list: async (accessToken: string): Promise<AchievementsListResponse> => {
    const r = await blogAuthFetch(`${apiBase()}/api/achievements`, { method: 'GET' }, accessToken);
    const data = (await readJson(r)) as AchievementsListResponse & { message?: string };
  ...
  summary: async (accessToken: string): Promise<AchievementsSummaryResponse> => {
    const r = await blogAuthFetch(
      `${apiBase()}/api/achievements/summary`,
      { method: 'GET' },
      accessToken
    );
    const data = (await readJson(r)) as AchievementsSummaryResponse & { message?: string };
    if (!r.ok) throw new Error(data.message ?? r.statusText);
    return data;
  },
};
