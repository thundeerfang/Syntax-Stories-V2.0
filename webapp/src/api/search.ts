...
import type { SearchGroupKey, SearchHit, UnifiedSearchResponse } from '@contracts/searchApi';
import {
  resolveTechStackApi,
...
export type { EntityOption, TechStackItem, TechStackCategory } from '@contracts/referenceApi';
...
/** Single entry point for site-wide and scoped search. */
export const searchApi = {
  ...
  /** @mention autocomplete — unified users search with mention context (min 1 char). */
  ...
  resolveTechStack: (names: string[]): Promise<TechStackItem[]> => resolveTechStackApi(names),
