export const FOLLOWED_CATEGORIES_STORAGE_KEY =
  "syntax-stories.followed-category-slugs";
export const FOLLOWED_CATEGORIES_CHANGED_EVENT =
  "syntax-stories:followed-categories-changed";
...
export function readFollowedCategorySlugs(
...
export function isCategorySlugFollowed(
  slug: string,
  userId: string | null | undefined,
): boolean {
  if (!userId?.trim()) return false;
...
export function canSyncCategoryFollowState(
  viewer: CategoryFollowViewer,
): boolean {
  if (viewer.isHydrated === false) return false;
  return Boolean(viewer.token?.trim() && viewer.userId?.trim());
}
export function isCategoryFollowedForViewer(
  slug: string,
  viewer: CategoryFollowViewer,
): boolean {
