...
> {
  ...
  for (const m of items.slice(0, 5)) {
    if (m.isPending && m.pendingFile) {
      if (!token)
        throw new Error("Sign in again to finish uploading staged media.");
...
export function normalizeMediaLinkInput(raw: string): string | null {
  const t = raw.trim();
  if (!t) return null;
  const candidates = /^https?:\/\//i.test(t)
    ? [t]
    : [t, `https://${t.replace(/^\/+/, "")}`];
...
export function AddMediaLinksDialog({
  ...
  const handleAdd = () => {
    ...
    if (room <= 0) {
      toast.error("You already have 5 media items for this entry.", {
        id: "syntax-max-media",
      });
      return;
    }
    ...
    if (!normalized) {
      toast.error("Enter a valid URL (https://… or domain.com).", {
        ...
        <header className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1 flex flex-col gap-1.5">
            ...
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
              One URL per line. Links share the 5-item limit with uploaded
              images.
            </p>
          </div>
          <button
        ...
        <div className="space-y-3">
          <div className="space-y-1">
            ...
            <p className="text-[9px] text-muted-foreground">
              Remaining media slots for this entry: {Math.max(0, maxCount)}{" "}
              (links + images).
            </p>
          </div>
          <div className="space-y-1">
