...
/** Append desktop return origin to any OAuth start URL (e.g. settings link flow). */
export function withDesktopOAuthReturnOrigin(href: string): string {
  return withDesktopReturnOrigin(href);
}

/** OAuth login start URL (`/auth/{provider}/login`). */
...
/** OAuth signup start URL with optional referral code. */
