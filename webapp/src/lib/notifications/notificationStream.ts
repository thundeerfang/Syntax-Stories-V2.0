...
export type NotificationStreamEvent =
  | { type: 'snapshot'; notifications: unknown[]; unreadCount: number }
  | { type: 'notification'; payload: NotificationStreamPayload }
  | { type: 'raw'; payload: unknown };

/** Authenticated SSE via fetch + ReadableStream (EventSource cannot send Bearer tokens). */
