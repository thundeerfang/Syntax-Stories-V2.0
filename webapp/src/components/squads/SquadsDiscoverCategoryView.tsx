...
import { isSquadCategory, type SquadCategory, squadCategoryLabel } from '@/lib/squadCategory';
  ...
  if (!category) {
    return (
      <div className="flex min-h-0 flex-1 flex-col gap-6 pt-1 md:pt-2">
        ...
        <Link
          href="/squads"
          className="inline-flex w-fit rounded-none border-2 border-border bg-primary px-5 py-2.5 font-mono text-[11px] font-bold uppercase tracking-wide text-primary-foreground hover:opacity-90"
        >
          Open squads
        </Link>
      </div>
        ...
        {loading ? (
          <ul className={SQUAD_DISCOVER_CARD_GRID_CLASS}>
            {Array.from({ length: 8 }).map((_, i) => (
              <li key={i}>
                <div className="min-h-[17rem] w-full animate-pulse border-2 border-border bg-muted/25 shadow-[3px_3px_0_0_var(--border)]" />
              </li>
            ))}
          </ul>
        ) : rows.length === 0 ? (
