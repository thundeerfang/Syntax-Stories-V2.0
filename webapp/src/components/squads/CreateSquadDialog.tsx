...
import {
  ...
  SQUAD_DESCRIPTION_MAX,
  SQUAD_LOGO_MAX_BYTES,
  SQUAD_NAME_MAX,
} from "@/lib/squads/squadFormLimits";
import { IMAGE_ACCEPT_RASTER } from "@/components/ui/form";
import { uploadCover, uploadMedia } from "@/api/upload";
import { toast } from "sonner";
import { cn } from "@/lib/core/utils";
function resolveSquadMediaSrc(
...
export function CreateSquadDialog({
  ...
  const [invitePermission, setInvitePermission] =
    useState<SquadInvitePermission>("all_members");
  ...
  return (
    <>
        ...
        <div className="flex flex-col gap-6">
            ...
            {!descOpen ? (
              ...
              <label className="grid gap-1.5">
                <div className="flex items-baseline justify-between gap-2">
                  ...
                  <span className="font-mono text-[10px] font-bold text-muted-foreground">
                    {description.length}/{SQUAD_DESCRIPTION_MAX}
                  </span>
                </div>
                <textarea
                  value={description}
                  ...
                  rows={3}
                  maxLength={SQUAD_DESCRIPTION_MAX}
          ...
          <section className="grid gap-3">
            <div className="flex items-center justify-between gap-2">
              ...
              <InfoHintDialog
                title={isEdit ? "Squad visibility" : "Squad type"}
              >
                {isEdit ? (
                  <>
                    <p>
                      Visibility cannot be changed after creation. Your squad
                      stays public or private for its lifetime.
                    </p>
                    <p>
                      Public squads appear in the directory and are open for
                      anyone to join. Private squads are invite-only and hidden
                      from browse lists.
                    </p>
                  </>
                ) : (
                  <>
                    <p>
                      Public squads are searchable, listed in the squad
                      directory, and open for everyone to join — best for
                      communities and creators.
                    </p>
                    <p>
                      Private squads are invite-only and hidden from the
                      directory — best for teams and smaller groups.
                    </p>
            ...
            >
              ...
              >
                ...
                <p className="mt-1.5 text-[11px] leading-snug text-muted-foreground">
                  Searchable, listed in the squad directory, and open for
                  everyone to join. Ideal for communities and creators.
                </p>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <p className="mt-1.5 text-[11px] leading-snug text-muted-foreground">
                  Invite-only, hidden from the directory—best for teams and
                  smaller groups who collaborate privately.
                </p>
              </button>
            </div>
            {visibility === "public" ? (
            ...
            <div className="grid gap-2">
              <div className="flex items-center justify-between gap-2">
                ...
                <InfoHintDialog title="Post content">
                  <p>
                    Choose who may publish new posts or share existing posts
                    into this squad feed.
                  </p>
                  <p>
                    <span className="font-bold text-foreground">
                      All members
                    </span>{" "}
                    — anyone in the squad can author new posts or share into the
                    feed.
                  </p>
                  <p>
                    <span className="font-bold text-foreground">
                    ...
                    — only admins and moderators can post or share; other
                    members can still read and engage.
                  </p>
                </InfoHintDialog>
              </div>
            ...
            <div className="flex flex-col gap-2 border-2 border-border bg-muted/10 px-3 py-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-muted-foreground">
                  When on, new posts can be held for moderator review before
                  publishing (stored for a future workflow).
                </p>
              </div>
              <Switch
            ...
            <div className="grid gap-2">
              <div className="flex items-center justify-between gap-2">
                ...
                <InfoHintDialog title="Invitation permissions">
                  <p>
                    Choose who may add other people to this squad by username.
                  </p>
                  <p>
                    <span className="font-bold text-foreground">
                      All members
                    </span>{" "}
                    — any member can invite others by username.
                  </p>
                  <p>
                    <span className="font-bold text-foreground">
                      Staff only
                    </span>{" "}
                    — only admins and moderators can add members; regular
      ...
      <ImageUploadCropDialog
        ...
        maxSizeBytes={SQUAD_LOGO_MAX_BYTES}
        aspect={1}
      ...
      <ImageUploadCropDialog
        ...
        subtitleClassName="text-[10px] font-bold uppercase tracking-widest text-muted-foreground"
        maxSizeBytes={SQUAD_BANNER_MAX_BYTES}
        aspect={4}
        ...
        onConfirm={async (file) => {
          ...
          toast.success(
            isEdit ? "Banner added — save changes to apply" : "Banner ready",
          );
        }}
      />
    </>
