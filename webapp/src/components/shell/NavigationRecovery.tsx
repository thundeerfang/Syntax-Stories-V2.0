...
/**
 * Clears stuck route `loading.tsx` boundaries and client loading flags after bfcache restore,
 * browser back (including OAuth cancel), and external redirect returns.
 */
export function NavigationRecovery() {
  ...
  useEffect(() => {
    ...
    const onVisibility = () => {
      if (document.visibilityState === 'visible' && consumeOAuthNavigationPending()) recover();
    };
