...
import { CheckCircle2, ImagePlus, MessageSquare, Monitor } from "lucide-react";
...
import { FEEDBACK_FORM_ID, FEEDBACK_SUBTITLE } from "@/lib/feedback/constants";
import {
  countFeedbackWords,
  FEEDBACK_MAX_DESC,
  ...
  const handleScreenCapture = useCallback(async () => {
    ...
    try {
      ...
      applyAttachment(
        file,
        "capture",
        buildUploadImageMeta("Screen capture", user?.username ?? "user").title,
      );
    } catch (e) {
      const name =
  ...
  const runSubmit = useCallback(
    async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if (weeklyRemaining != null && weeklyRemaining <= 0) {
        toast.error(
          "You have reached the feedback limit for now. Try again later.",
        );
        return;
      }
      if (sessionPending) {
  ...
  return (
    <>
      <FormDialog
        ...
        title="Feedback"
        titleIcon={
          <MessageSquare className="size-5" strokeWidth={2.5} aria-hidden />
        }
        ...
        panelClassName={cn(
          ...
          capturing && layout.feedbackCaptureHideUi,
        )}
        backdropClassName={capturing ? layout.feedbackCaptureHideUi : undefined}
        footer={formFooter}
        footerClassName="flex flex-row flex-wrap items-center justify-end gap-2"
        interactionLock={submitting}
        interactionLockContent={
          submitting ? <FeedbackSuccessSubmitSkeleton /> : undefined
        }
      >
              ...
              <div className="flex min-h-[14rem] flex-1 flex-col border-2 border-border bg-muted/10 shadow lg:min-h-0">
                ...
                <div className="shrink-0 space-y-1.5 p-2">
                  ...
                  >
                    ...
                    {Math.round(FEEDBACK_MAX_IMAGE_BYTES / (1024 * 1024))} MB.
                    Server scans (malware) and re-encodes.
                  </p>
                  <Button
                    type="button"
                    variant="outline"
                  ...
                  >
                    <Monitor
                      className="mr-1.5 inline size-3.5 shrink-0 opacity-80"
                      aria-hidden
                    />
            ...
            <div className="min-w-0 space-y-4 lg:col-span-2 lg:row-start-2">
              <div className="space-y-1.5">
                ...
                <p className="text-[9px] text-muted-foreground">
                  {countFeedbackWords(description)}/{FEEDBACK_MIN_DESC_WORDS}{" "}
                  words · {description.trim().length}/{FEEDBACK_MAX_DESC}{" "}
                  characters
                </p>
              </div>
            </div>
      ...
      <ImageUploadCropDialog
        ...
        title="Screenshot"
        titleIcon={
          <ImagePlus className="size-5" strokeWidth={2.5} aria-hidden />
        }
