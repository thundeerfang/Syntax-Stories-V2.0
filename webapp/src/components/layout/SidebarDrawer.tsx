  ...
  return (
    <div className="rounded-none">
      <button
        ...
        onClick={() => onToggle(id)}
        className={cn(
          'flex w-full items-center justify-between gap-3 rounded-none border-2 border-border bg-card px-3 py-2.5 text-left text-[10px] font-black uppercase tracking-widest text-foreground shadow-[2px_2px_0px_0px_var(--border)] transition-[transform,box-shadow] duration-150 hover:bg-muted/40 active:translate-x-0.5 active:translate-y-0.5 active:shadow-none'
        )}
        aria-expanded={open}
      >
      ...
      <AnimatePresence initial={false}>
        {open && (
          <motion.div
            key={id}
            initial={accordionContentMotion.initial}
...
function AccordionLink({
  ...
  return (
    <Link
      ...
      onClick={onNavigate}
      className={cn(
        'flex w-full min-w-0 items-center justify-between gap-2 rounded-none py-2 pl-1 pr-0 text-[10px] font-black uppercase tracking-widest transition-colors',
        ...
        return (
          <Link key={f.id} href={href} onClick={onNavigate} className={rowClass(active)}>
            <span className="flex min-w-0 flex-1 items-center gap-2">
              {f.isDefault ? (
                <span
                  className="size-2 shrink-0 rounded-full border-2 border-background bg-purple-600 shadow-[0_0_0_1px_var(--border)]"
                  title="Default feed"
...
function BookmarkFolderAccordionLink({
  ...
  return (
    <Link
      ...
      onClick={onNavigate}
      className={cn(
        'flex w-full min-w-0 items-center gap-2 rounded-none py-2 pl-1 pr-0 text-[10px] font-black uppercase tracking-widest transition-colors',
        isActive ? 'text-primary' : 'text-foreground/75 hover:text-foreground'
      )}
    >
      <span className="flex min-w-0 flex-1 items-center gap-2">
      ...
      <span
        className="flex h-6 min-w-[1.5rem] shrink-0 items-center justify-center rounded-full border-2 border-border bg-muted/40 px-1.5 font-mono text-[10px] font-black tabular-nums text-foreground"
    ...
    <div className="flex h-full min-h-0 flex-col items-center bg-transparent py-3">
      <Link
        ...
        className={cn(
          ...
          'mb-2 flex size-10 shrink-0 items-center justify-center gap-0 rounded-none p-0'
        )}
      >
            ...
            >
              <motion.span
                ...
                className={cn(
                  ...
                  isActive
                    ? 'size-8 bg-primary text-primary-foreground shadow-[2px_2px_0_0_var(--border)]'
                    ...
                    return (
                      <li key={href}>
                        <Link
                          ...
                          onClick={close}
                          className={cn(
                            'group flex cursor-pointer items-center gap-3 rounded-none transition-[background-color,color,box-shadow] duration-150 ease-out',
                            isActive
                              ? 'border-2 border-border bg-primary px-3 py-2.5 text-[11px] font-black uppercase tracking-widest text-primary-foreground shadow-[4px_4px_0px_0px_var(--border)]'
