...
import { ACCOUNT_DROPDOWN_MENU_WIDTH_PX } from "@/variable";
...
function readMenuAnchor(triggerEl: HTMLElement | null): {
  top: number;
  left: number;
} | null {
  ...
  return {
    top: rect.bottom + 8,
    left: Math.max(8, rect.right - ACCOUNT_DROPDOWN_MENU_WIDTH_PX),
  };
}
export function AccountDropdown() {
  ...
  const presenceIndicatorEnabled = useUIStore(
    (s) => s.presenceIndicatorEnabled,
  );
  const setPresenceIndicatorEnabled = useUIStore(
    (s) => s.setPresenceIndicatorEnabled,
  );
  ...
  const fullAvatarUrl = resolveProfileMediaUrl(
  ...
  const menuSections: Array<{
  ...
  }> = [
    {
      ...
      items: [
        ...
        {
          href: "/settings?section=syntax-card",
          label: "Syntax card",
          icon: CreditCard,
        },
      ],
    ...
    <div className="relative inline-block" ref={rootRef}>
      <button
        ...
        className={cn(
          ...
          open
            ? "border-primary shadow-none translate-x-[2px] translate-y-[2px]"
            : "shadow",
        )}
      >
              ...
              >
                ...
                <div className="flex flex-col">
                  {menuSections.map((section, sectionIdx) => (
                    <div
                      key={section.id}
                      className="py-0.5 border-b-2 border-border last:border-b-0"
                    >
                      {section.items.map((item, itemIdx) => (
                        <motion.div
                          ...
                          transition={{
                            delay: sectionIdx * 0.1 + itemIdx * 0.03,
                          }}
                        >
                          <Link
                            href={item.href}
                    ...
                    <div className="px-4 py-2 flex items-center justify-between border-b-2 border-border">
                      ...
                      <div className="scale-75 origin-right">
                        <Switch
                          ...
                          onCheckedChange={setFeedbackVisible}
                        />
                      </div>
                    </div>
