...
} from "@/lib/api/fetchStorageHealth";
import { resolvePublicApiBase } from "@/lib/api/publicApiBase";
import { cn } from "@/lib/core/utils";
import { useScrollLock } from "@/hooks/useScrollLock";
import { CLOUD_ROBOTICS_LOTTIE_SRC, STORAGE_GATE_Z } from "@/variable";
...
function ReasonIcon({
  ...
  if (reason === "mongo_quota")
    return <Database className="size-4" strokeWidth={2.25} aria-hidden />;
  if (reason === "disk_full")
    return <HardDrive className="size-4" strokeWidth={2.25} aria-hidden />;
          ...
          >
            ...
            >
              <div className="flex flex-col items-center px-5 py-8 text-center sm:px-8">
                ...
                >
                  <span
                    className="inline-flex size-1.5 animate-pulse bg-destructive"
                    aria-hidden
                  />
                  Storage full
                </p>
                ...
                <div
                  className="mb-8 flex justify-center text-foreground"
                  aria-hidden
                >
                  <div className="size-[120px] opacity-80">
                    <DotLottieReact
                      src={CLOUD_ROBOTICS_LOTTIE_SRC}
                      loop
                      autoplay
                ...
                <div className="mb-6 max-w-sm space-y-2">
                  ...
                  >
                    {reasonLabel(health.reason)} You can still browse, but new
                    posts, uploads, and feedback cannot be saved until storage
                    is restored.
                  </p>
                </div>
