...
import { AlertCircle, ChevronLeft, ChevronRight, WifiOff } from 'lucide-react';
...
const BOOKMARKS_COLUMN_SLIDE = cn(
  ...
  function CompactBlogPostsSwiper(
    ...
    if (loading) {
      return (
        <div
          className={cn(
            'min-h-[220px] w-full border-2 border-border bg-muted/30',
            className,
          )}
          aria-busy="true"
        >
          <div className="h-36 animate-pulse bg-muted-foreground/10" />
        </div>
      );
    }
    ...
    if (error != null) {
      ...
      return (
        <div
          className={cn(
            'flex w-full flex-col gap-3 border-2 border-border bg-card p-4',
            className,
          )}
        >
          <div className="flex items-start gap-3">
            <div
              className="flex size-10 shrink-0 items-center justify-center rounded-full border-2 border-border bg-muted"
              aria-hidden
            >
              {isConn ? (
                <WifiOff className="size-5 text-muted-foreground" strokeWidth={2.25} aria-hidden />
              ) : (
                <AlertCircle className="size-5 text-destructive" strokeWidth={2.25} aria-hidden />
              )}
            </div>
          ...
          <button
            type="button"
            onClick={() => onRetry()}
            className="self-start border-2 border-border bg-background px-3 py-2 font-mono text-[10px] font-bold uppercase tracking-wide"
          >
            Retry
          </button>
        </div>
          ...
          <div className="mt-auto flex shrink-0 justify-center gap-2 pt-4">
            {posts.map((p, i) => (
              <button
                ...
                className={cn(
                  'size-2 rounded-full border border-border transition-[transform,background-color]',
                  i === index
