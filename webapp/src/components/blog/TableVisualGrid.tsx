...
export function TableVisualGrid({
  ...
  const setDims = useCallback(
    (rows: number, cols: number) => {
      const r = Math.min(MAX_TABLE_ROWS, Math.max(MIN_ROWS, rows));
      const c = Math.min(MAX_TABLE_COLS, Math.max(MIN_COLS, cols));
  ...
  return (
    <div className={cn("flex min-h-0 flex-1 flex-col gap-3", className)}>
      <div
        className={cn(
          "flex flex-wrap items-center gap-3 text-[10px]",
          toolbarClassName,
        )}
      >
        ...
        <div className="flex items-center gap-1 border-2 border-border bg-muted/20 px-1 py-0.5">
          ...
          <span className="min-w-[1.5rem] text-center font-mono font-bold">
            {colCount}
          </span>
          <button
            type="button"
            ...
            disabled={colCount >= MAX_TABLE_COLS}
            onClick={() => bumpCols(1)}
          >
