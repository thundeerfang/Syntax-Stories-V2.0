...
export function BlogCard({
  ...
  const authorAvatarClass = cn(
    "h-8 w-8 shrink-0  border-2 border-border object-cover",
  );
    ...
    >
      ...
      {squadFeedPin?.canModerate ? (
        <button
          type="button"
          title={
            squadFeedPin.isPinned ? "Unpin from squad" : "Pin to squad top"
          }
              ...
              {squadFeedShare ? (
                ...
                >
                  ...
                  {(() => {
                    ...
                    const label =
                      squadFeedShare.sharedBy.fullName?.trim() || uname;
                    return avatarSrc ? (
                      <img
                        src={avatarSrc}
                        alt=""
      ...
      {ownerActions ? (
        <BlogCardOwnerActionsOverlay actions={ownerActions} />
      ) : null}
    </article>
  );
}
