...
export function BlogWriteDeployOverlay({
  ...
  const [squadMongoId, setSquadMongoId] = useState<string | null>(
    initialSquadMongoId ?? null,
  );
  useEffect(() => {
    if (!open) return;
    ...
    const seed =
      initialSquadMongoId && allowed.has(initialSquadMongoId)
        ? initialSquadMongoId
        : null;
    setSquadMongoId(seed);
  }, [open, snapshot.category, snapshot.tags, initialSquadMongoId, mySquads]);
  const summaryLine = useMemo(
    () => blogPublishSummaryPreviewPlain(summaryHtml),
    [summaryHtml],
  );
  ...
  const addTag = useCallback((raw: string) => {
    ...
    if (!t) return;
    setTags((prev) =>
      prev.includes(t) || prev.length >= 20 ? prev : [...prev, t],
    );
    setTagInput("");
  }, []);
            ...
            {mySquads.length > 0 ? (
              <div className="min-w-0 sm:col-span-2">
                ...
                <p className="mt-1.5 text-[11px] leading-snug text-muted-foreground">
                  Publish or save this draft as a squad post for the feed you
                  pick. Leave unset for a personal post only.
                </p>
              </div>
            ) : null}
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  ...
                  {taxonomyTags.slice(0, 28).map((t) => (
                    <button
                      ...
                      type="button"
                      disabled={
                        tags.includes(t.slug) || tags.length >= 20 || busy
                      }
