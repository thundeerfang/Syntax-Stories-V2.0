...
export function CertificationCard({
  ...
  const displayCertId = Number.isNaN(numId)
    ? rawId
    : String(numId).padStart(2, "0");
                ...
                <HoverCard
                  content={
                    <LinkPreviewCardContent
                      domain={domainFromUrl(e.credentialUrl)}
                    />
                  }
                  side="top"
                  align="start"
              ...
              {e.skills && e.skills.length > 0 && (
                <>
                  {visibleSkills.map((s: string) => (
                    ...
                    >
                      <Code2 className="size-2.5 text-primary/60" />{" "}
                      {s.toUpperCase()}
                    </span>
                  ))}
