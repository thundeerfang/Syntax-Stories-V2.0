...
import { LEGAL_PRIVACY_PATH, LEGAL_TERMS_PATH } from "@/variable";
type FormSubmit = {
  preventDefault(): void;
  ...
  if (iconSrc != null) {
    iconEl = (
      <img
        src={iconSrc}
        alt=""
        className={cn("h-5 w-5 shrink-0 object-contain", iconClassName)}
      />
    );
  } else if (Icon) {
...
function renderAuthSignupStep(p: AuthDialogRenderProps): ReactNode {
  ...
  const oauthRef =
    p.referralValidationState === "valid" ? p.referralInput.trim() : null;
  return (
    <>
...
function renderAuthVerifyEmailStep(p: AuthDialogRenderProps): ReactNode {
  return (
    <>
      ...
      <form
        onSubmit={
          p.twoFactorActive ? p.handleVerifyTwoFactor : p.handleVerifyCode
        }
        className="mt-3 space-y-3"
      >
        <div className="space-y-1.5">
          ...
          <input
            ...
            className={authInputClassName(
              "py-4 text-center text-lg font-black tracking-[0.5em]",
            )}
          />
        </div>
        <Button
      ...
      {!p.twoFactorActive && (
        <div className="mt-4 text-center">
          ...
          >
            {p.resendCooldownSec > 0
              ? `Resend code in ${p.resendCooldownSec}s`
              : "Resend code"}
          </button>
        </div>
      )}
