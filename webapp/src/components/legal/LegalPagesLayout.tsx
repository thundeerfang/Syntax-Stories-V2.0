/**
 * Legal shell: three separate retro cards — mobile ToC, policy header, and body —
 * plus a desktop ToC card in the left column (no single wrapping parent frame).
 */
import type { ReactNode } from 'react';
import { LEGAL_MAIN_BODY_PAD, LEGAL_MIN_BELOW_HEADER, LEGAL_RETRO_CARD } from './legalUi';
import { LegalPolicyHeaderProvider } from './LegalPolicyHeaderContext';
import { LegalPolicyPageHeader } from './LegalPolicyPageHeader';
import { LegalTableOfContentsMobile, LegalTableOfContentsSidebar } from './LegalTableOfContents';
    ...
    <LegalPolicyHeaderProvider>
      <div className={`flex w-full flex-col ${LEGAL_MIN_BELOW_HEADER}`}>
        ...
        {/*
          ...
          On small screens only header, ToC card, main participate — title first, then outline, then body (no extra full-width “menu” bar).
        */}
        <div className="grid min-h-0 flex-1 grid-cols-1 grid-rows-[auto_auto_minmax(0,1fr)] gap-4 lg:grid-cols-[minmax(0,20rem)_minmax(0,1fr)] lg:grid-rows-[auto_minmax(0,1fr)] lg:gap-5">
          ...
          <div className={`min-h-0 min-w-0 max-h-[min(40vh,22rem)] overflow-hidden lg:hidden ${LEGAL_RETRO_CARD}`}>
          ...
          >
            <div className={`max-h-full overflow-y-auto ${LEGAL_MAIN_BODY_PAD}`}>
              {children}
            </div>
          </main>
        </div>
