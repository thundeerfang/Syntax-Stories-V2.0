...
function DefaultHero() {
  return (
    <div className="relative mx-auto flex size-[7.5rem] shrink-0 items-center justify-center">
      <div
        className="absolute inset-0 rounded-full bg-gradient-to-br from-violet-200 via-fuchsia-200 to-violet-400 opacity-95 shadow-[inset_0_0_0_2px_rgba(255,255,255,0.35)]"
        aria-hidden
      />
      <div
        className="relative flex size-[4.5rem] items-center justify-center rounded-full bg-white/20 backdrop-blur-[2px]"
        aria-hidden
      >
        <UsersRound className="size-10 text-white drop-shadow-sm" strokeWidth={2} aria-hidden />
      </div>
      {/* Decorative “orbit” chips — abstract stand-ins for member avatars */}
      <span
        className="absolute -right-1 top-2 size-9 rounded-full border-2 border-white/80 bg-gradient-to-br from-amber-200 to-orange-300 shadow-md"
        aria-hidden
      />
      <span
        className="absolute -left-2 bottom-3 size-9 rounded-full border-2 border-white/80 bg-gradient-to-br from-sky-200 to-indigo-300 shadow-md"
        aria-hidden
      />
      <span
        className="absolute -bottom-1 left-1/2 size-9 -translate-x-1/2 rounded-full border-2 border-white/80 bg-gradient-to-br from-emerald-200 to-teal-300 shadow-md"
        aria-hidden
      />
    </div>
  );
 ...
 * Layout: hero strip, dark body, footer — similar in spirit to {@link ConfirmDialog} but for onboarding / tips.
 */
export function InfoSwiperDialog({
  ...
  return (
    <Dialog
      ...
      closeOnEscape
      panelClassName={cn(
        'max-w-lg overflow-hidden rounded-3xl border-2 border-violet-500 bg-black p-0 text-white shadow-[0_0_40px_rgba(139,92,246,0.28)]',
        panelClassName,
      )}
    ...
    >
      ...
      <div className="relative shrink-0 bg-gradient-to-b from-violet-600 to-violet-700 px-6 pb-8 pt-10">
        <button
          type="button"
          onClick={onClose}
          className="absolute right-4 top-4 z-10 flex size-10 items-center justify-center rounded-full border-2 border-white/90 bg-transparent text-white transition-colors hover:bg-white/15"
          aria-label="Close"
        >
      ...
      >
        ...
        <button
          type="button"
          onClick={goNext}
          className="shrink-0 rounded-xl border-2 border-transparent bg-white px-5 py-2.5 text-sm font-bold text-black shadow-sm transition-transform hover:bg-zinc-100 active:translate-y-px"
        >
          {primaryLabel}
        </button>
      </div>
