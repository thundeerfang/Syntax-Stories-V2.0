...
export type UseDropdownResult<T extends HTMLElement = HTMLDivElement> = Readonly<{
  open: boolean;
  setOpen: Dispatch<SetStateAction<boolean>>;
  toggle: () => void;
  close: () => void;
  /** Wraps the trigger (and inline panels when not portaled). */
  rootRef: RefObject<T | null>;
  /** Portaled panel — omit when the menu is inside `rootRef`. */
  contentRef: RefObject<HTMLDivElement | null>;
}>;

/**
 * Shared open/close behavior for app dropdowns: outside click, outside scroll, Escape.
 */
