...
export function createBlockInSection(
  type: BlockType,
  sectionId: string,
): Block {
  ...
  if (type === "heading") {
    return {
      ...
      type: "heading",
      sectionId,
      payload: { text: "", level: 2 as HeadingLevel },
    };
  }
  if (type === "code") {
    return {
...
function ParagraphBlockEditor({
  ...
  const effectiveDoc: JSONContent | undefined =
    payload.doc ?? coerceParagraphDoc(payload);
  return (
    <div className="group border-0 bg-muted/10 p-3 space-y-2 ring-1 ring-border/35">
      <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground mb-1">
        ...
        <div className="flex items-center gap-0.5 shrink-0">
          <button
            type="button"
            className="text-muted-foreground hover:text-primary p-1 opacity-0 group-hover:opacity-100 transition-opacity focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-primary/40"
          ...
          <button
            type="button"
            className="text-destructive hover:text-destructive/80 p-1 opacity-0 group-hover:opacity-100 transition-opacity focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-destructive/50"
            onClick={onRemove}
      ...
      <RichParagraphEditor
        ...
        onChange={(nextDoc) =>
          onUpdate({ ...payload, doc: nextDoc, version: "rich-text" })
        }
      />
      <ParagraphBlockHelpDialog
        open={helpOpen}
...
function HeadingBlockEditor({
  ...
  return (
    <div className="border-0 bg-muted/10 p-3 space-y-2 ring-1 ring-border/35">
      <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground mb-1">
        <span className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            type="button"
            className="text-destructive hover:text-destructive/80 p-1 focus:outline-none focus:ring-2 focus:ring-destructive/50 ml-1"
            onClick={onRemove}
...
function BlockCard({
  ...
  return (
    <div className="border-0 bg-muted/10 p-3 space-y-3 ring-1 ring-border/35">
...
type ImagePayloadWithLegacy = ImagePayload & {
  altText?: string;
};
...
function patchImagePayload(
  p: ImagePayloadWithLegacy,
  patch: Partial<ImagePayload>,
): ImagePayload {
    ...
    >
      ...
      >
        <div className="flex flex-wrap items-center justify-end gap-2">
          <div
            className="flex items-center gap-0.5 border-2 border-white/25 bg-black/55 px-1 py-1 shadow backdrop-blur-md"
            role="toolbar"
          ...
          <button
            ...
            className="flex h-9 w-9 shrink-0 items-center justify-center border-2 border-white/25 bg-black/55 text-white shadow backdrop-blur-md transition-colors hover:border-destructive hover:bg-destructive"
        ...
        <div className="pointer-events-auto w-full pb-10 sm:pb-11">
          ...
          <input
            ...
            className="w-full border-2 border-white/25 bg-black/45 px-3 py-2 text-xs text-white placeholder:text-white/45 shadow backdrop-blur-sm focus:border-primary focus:outline-none focus:ring-0"
          />
        </div>
      </div>
      ...
      <div className="absolute bottom-2 right-2 z-20 max-w-[min(88%,18rem)]">
        ...
        <input
          ...
          className="w-full min-w-0 border-2 border-white/25 bg-black/60 px-2.5 py-1.5 text-[10px] font-medium leading-snug text-white shadow placeholder:text-white/45 backdrop-blur-md focus:border-primary focus:outline-none focus:ring-0"
        />
      </div>
    </div>
  );
      ...
      >
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          <button
            ...
            className="flex h-9 w-9 shrink-0 items-center justify-center border-2 border-white/25 bg-black/55 text-white shadow backdrop-blur-md transition-colors hover:border-primary hover:bg-primary/90"
...
function ImageBlockEditor({
  ...
  const handleFile = useCallback(
    async (file: File | null) => {
      ...
      if (file.size > BLOG_INLINE_IMAGE_MAX_MB * 1024 * 1024) {
        toast.error(`Image must be under ${BLOG_INLINE_IMAGE_MAX_MB}MB.`);
        return;
      }
      setUploading(true);
      try {
  ...
  const renderSelectedUploadedImage = (imageUrl: string) => {
    const overlays = (
      <UploadedImageWithOverlays
        ...
        onCaptionChange={(value) =>
          onUpdate(patchImagePayload(p, { title: value }))
        }
        onRemove={onRemove}
    ...
    if (layout === "square") {
      return (
        <div className="border-2 border-border bg-muted/50 p-2 sm:p-3 shadow">
    ...
    return (
      <>
        ...
        <div className="relative w-full">
          {renderSelectedUploadedImage(url)}
        </div>
      </>
    );
  }
      ...
      >
        <div className="relative aspect-video w-full">
          ...
          <div className="pointer-events-none absolute inset-0 flex items-center justify-center bg-linear-to-t from-black/50 to-transparent">
            <div className="flex h-9 w-9 items-center justify-center border-2 border-white/30 bg-black/55 shadow backdrop-blur-[2px]">
...
function VideoEmbedBlockEditor({
  ...
  const commitAll = useCallback(
    (
      ...
      const layoutOut: VideoEmbedLayoutDirection =
        videos.length < 2 ? "row" : nextLayout;
      const clampedSize = clampVideoEmbedSize(
        videos.length,
        nextSize,
        layoutOut,
      );
  ...
  const effectiveLayout: VideoEmbedLayoutDirection =
    previewCount < 2 ? "row" : layout;
  ...
  const showAddVideoButton =
    previewCount >= 2 && slots.length < VIDEO_EMBED_MAX;
  const addSlot = () => {
      ...
      <div className="space-y-4">
        <div className="border-2 border-border bg-muted/25 p-3 shadow">
          <div
            className={cn(
              "grid gap-4",
              showColumnLayout ? "sm:grid-cols-2" : "grid-cols-1",
            )}
          >
              ...
              <div className="flex gap-2 items-start">
                ...
                {slots.length > 1 ? (
                  <button
                    ...
                    className="shrink-0 border-2 border-border bg-muted/50 p-2.5 text-muted-foreground shadow transition-colors hover:border-destructive hover:bg-destructive/10 hover:text-destructive"
        ...
        {showAddVideoButton ? (
          <button
            type="button"
            onClick={addSlot}
            className="inline-flex items-center gap-2 border-2 border-dashed border-border bg-muted/20 px-3 py-2 text-[10px] font-bold uppercase tracking-wide text-foreground shadow transition-colors hover:border-primary hover:bg-primary/5"
          >
...
function GithubRepoBlockEditor({
  ...
  if (isSelected) {
    ...
    return (
      <div className="relative group w-full border-2 border-border bg-card overflow-hidden">
        <a
          href={displayUrl}
          target="_blank"
        ...
        <div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
          <button
            type="button"
            onClick={onRemove}
            className="p-1.5 border border-border bg-card text-destructive hover:bg-destructive/10 focus:outline-none focus:ring-2 focus:ring-primary"
        ...
        <div>
          ...
          <div className="flex gap-2 mt-1">
            ...
            <button
              ...
              className="px-3 py-1.5 text-[10px] font-bold uppercase border-2 border-border bg-primary text-primary-foreground hover:brightness-110 disabled:opacity-50"
            >
              {fetching ? "…" : "Fetch"}
            </button>
          </div>
          {fetchError && (
            <p className="text-[10px] text-destructive mt-1">{fetchError}</p>
          )}
        </div>
          ...
          <>
            ...
            <div>
              <button
                ...
                className="px-3 py-1.5 text-[10px] font-bold uppercase border-2 border-border bg-muted hover:bg-muted/80 disabled:opacity-50"
              >
                {loadingRepos ? "Loading…" : "Your repos"}
              </button>
              {myRepos.length > 0 && (
                <div className="mt-2 max-h-48 overflow-y-auto border border-border p-1.5 bg-muted/20 space-y-1">
                  {myRepos.map((r) => (
                    <button
                      key={r.id}
                      type="button"
                      onClick={() => selectRepo(r)}
                      className="w-full flex items-center gap-2 p-2 border border-transparent hover:border-primary hover:bg-muted/50 text-left"
                    >
                      {r.owner?.avatar_url ? (
                        <img
  ...
  const renderSelectedImage = () => {
    const overlays = (
      <UnsplashImageWithOverlays
        ...
        onPhotographerChange={(value) =>
          onUpdate({ ...payload, photographer: value })
        }
...
export function BlogWriteEditor({
  ...
  if (visibleBlocks.length === 0) {
    return (
      <div className="pb-16 flex flex-col items-center justify-center py-16 px-4 border-2 border-dashed border-border bg-muted/20">
        ...
        {visibleBlocks.map((block) => {
          ...
          if (block.type === "paragraph") {
            ...
            blockContent = (
              <div className="group flex items-center gap-2 py-2">
                ...
                <button
                  type="button"
                  className="text-destructive hover:text-destructive/80 p-1 opacity-0 group-hover:opacity-100 transition-opacity"
            ...
            blockContent = (
              <MermaidBlockEditor
                blockId={block.id}
                payload={
                  (block.payload ?? { source: "" }) as MermaidDiagramPayload
                }
                onUpdate={(p) => updateBlock(block.id, p)}
                onRemove={() => removeBlock(block.id)}
              />
            ...
            blockContent = (
              <div className="border border-dashed border-border bg-card p-3 text-[11px] text-muted-foreground flex items-center justify-between">
                ...
                <button
                  type="button"
                  className="text-destructive hover:text-destructive/80 p-1 focus:outline-none focus:ring-2 focus:ring-destructive/50"
                  onClick={() => removeBlock(block.id)}
          ...
          return (
            ...
            >
              {isDropTarget && (
                <div
                  className="absolute left-0 right-0 -top-2 z-10 h-0.5 bg-primary pointer-events-none"
                  aria-hidden
                />
              )}
              <button
                ...
                className="mt-2 shrink-0 p-1 cursor-grab active:cursor-grabbing text-muted-foreground hover:text-foreground hover:bg-muted/50 touch-none select-none"
