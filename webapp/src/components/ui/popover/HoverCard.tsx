...
export function computeHoverCardPositionAuto(
  ...
  for (const side of sides) {
    ...
    const dist =
      Math.abs(clamped.top - raw.top) + Math.abs(clamped.left - raw.left);
...
export function motionAxisOffset(side: Side): {
  y: number;
  x: number;
} {
  switch (side) {
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className={cn(
              "fixed w-[280px] min-h-[80px] max-h-[320px] overflow-hidden border-2 border-border bg-card shadow pointer-events-auto",
              contentClassName,
            )}
