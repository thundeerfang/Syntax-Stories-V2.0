...
function getAvatarSrc(profileImg: string | undefined, seed: string): string {
...
export function SearchHitRow({
  ...
  const showAvatar = hit.type === 'user' || (hit.type === 'squad' && hit.imageUrl);
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected ? 'bg-primary text-primary-foreground' : 'bg-background hover:bg-muted/30'
      )}
    >
      <div className="relative shrink-0">
        {showAvatar ? (
          <div className="relative size-10 border-2 border-border bg-muted overflow-hidden">
            <img
              src={getAvatarSrc(hit.imageUrl, hit.id)}
              alt=""
      ...
      {selected ? (
        <div className="flex shrink-0 items-center gap-2 bg-foreground px-2 py-1 text-background">
          <span className="hidden text-[9px] font-black uppercase sm:inline">Go</span>
