...
export const LEGAL_RETRO_CARD = 'retro-card-lg';

/** Small square icon frame (ToC strip, policy header). */
const LEGAL_RETRO_ICON_TILE_BASE =
  'flex shrink-0 items-center justify-center border-2 border-border bg-card shadow';
...
export const LEGAL_TOC_LINK =
  'block border-2 border-transparent px-2.5 py-2 font-mono text-[10px] font-bold uppercase tracking-wide text-muted-foreground transition-all hover:border-border hover:bg-card hover:text-foreground hover:shadow active:translate-x-px active:translate-y-px active:shadow-none';

/** UDD / action block below policy body */
export const LEGAL_ACTION_PANEL =
  'border-2 border-dashed border-border bg-muted/10 p-5 shadow sm:p-6';
...
export const LEGAL_VERSION_BADGE =
  'flex shrink-0 flex-col gap-1 border-2 border-border bg-muted/50 px-3 py-2.5 shadow sm:px-4';
...
export const LEGAL_PRIMARY_CTA =
  'inline-flex min-h-[2.75rem] items-center justify-center gap-2 border-2 border-primary bg-primary px-5 py-2.5 text-xs font-black uppercase tracking-widest text-primary-foreground shadow transition-all hover:opacity-95 active:translate-x-px active:translate-y-px active:shadow-none disabled:cursor-not-allowed disabled:opacity-50';
