...
function TocNavList({ items, onNavigate }: { items: TocItem[]; onNavigate?: () => void }) {
  if (items.length === 0) {
    return null;
  }
      ...
      <div className={LEGAL_TOC_STRIP}>
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 pt-0.5">
            ...
            <p className={LEGAL_TOC_SUB}>Headings from this policy become section links here.</p>
          </div>
        </div>
      </div>
...
/** Desktop / wide: ToC inside the left retro card */
export function LegalTableOfContentsSidebar() {
  return <LegalTableOfContentsPanel />;
}

/** Narrow screens: same panel as desktop */
