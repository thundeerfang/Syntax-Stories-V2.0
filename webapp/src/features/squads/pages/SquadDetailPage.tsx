...
} from "@/components/squads/SquadSlugSections";
...
function resolveSquadBannerSrc(url: string | undefined): string | undefined {
  return resolveSquadMediaUrl(url);
}
...
export default function SquadDetailPage() {
  ...
  const bannerSrc = squad
    ? resolveSquadBannerSrc(squad.coverBannerUrl)
    : undefined;
  ...
  return (
    <div className={cn(shell.contentRail, "relative min-h-0 flex-1")}>
      <div
                  ...
                  const shareChrome =
                    row.kind === "shared"
                      ? {
                          sharedBy: {
                            username:
                              row.sharedBy?.username?.trim() || "member",
