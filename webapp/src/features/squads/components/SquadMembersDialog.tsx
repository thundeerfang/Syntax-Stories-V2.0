...
import { ConfirmDialog, FormDialog, DIALOG_Z_INDEX_STACKED } from '@/components/ui/dialog';
...
function roleBadgeClass(role: SquadMemberRole): string {
  ...
  if (role === 'moderator') return 'border-amber-500/40 bg-amber-400/15 text-amber-800 dark:text-amber-200';
...
type SquadMemberRowProps = Readonly<{
  ...
  setMenuUser: (username: string | null | ((cur: string | null) => string | null)) => void;
...
function SquadMemberRow({
  ...
  const updateMenuPosition = useCallback(() => {
    ...
    const itemCount = 1 + (canFollow ? 1 : 0) + (canPromote ? 1 : 0) + (canRemove ? 1 : 0);
  ...
  const menuPortal =
    ...
    createPortal(
      <ul
        ref={menuPortalRef}
        role="menu"
        style={{ top: menuPos.top, left: menuPos.left, zIndex: MEMBER_MENU_Z_INDEX }}
      ...
      >
            <li role="none">
              ...
              >
                <BarChart3 className="size-3.5 shrink-0 text-primary" strokeWidth={2.25} aria-hidden />
                View contribution
              </button>
            </li>
            {canFollow ? (
            ...
            {canPromote ? (
              <li role="none">
                <button
                  ...
                  className="flex w-full items-center gap-2 px-3 py-2 text-left font-mono text-[10px] font-bold uppercase tracking-wide hover:bg-muted/60"
                >
                  <Shield className="size-3.5 shrink-0 text-primary" strokeWidth={2.25} aria-hidden />
                  Change role
                </button>
              </li>
            ) : null}
...
export function SquadMembersDialog({
  ...
  const [promoteTarget, setPromoteTarget] = useState<SquadMembersDialogRow | null>(null);
  const [contributionTarget, setContributionTarget] = useState<SquadMembersDialogRow | null>(null);
  const [removeTarget, setRemoveTarget] = useState<SquadMembersDialogRow | null>(null);
  ...
  const confirmRemove = async () => {
    ...
    setRemoveBusy(true);
    try {
      await squadsApi.removeMember(squadSlug, removeTarget.username, accessToken);
