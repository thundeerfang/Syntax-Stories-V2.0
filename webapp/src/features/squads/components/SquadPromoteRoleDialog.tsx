...
function RoleSelectDropdown({
  ...
  return (
    <div ref={ref} className="relative w-full">
      ...
      {open ? (
        <ul role="listbox" aria-label="Squad role" className={retroDropdownPanel}>
          {ROLE_OPTIONS.map((o) => (
...
export function SquadPromoteRoleDialog({
  ...
  const selectedOption = ROLE_OPTIONS.find((o) => o.value === role) ?? ROLE_OPTIONS[0];
  ...
  const save = async () => {
    ...
    setBusy(true);
    try {
      await squadsApi.setMemberRole(squadSlug, member.username, role, accessToken);
      toast.success(role === 'moderator' ? 'Promoted to moderator' : 'Set as member');
      await onUpdated();
      onClose();
    } catch (e) {
