...
import { TRENDING_AUTOPLAY_MS, TRENDING_STACK_DEPTH } from "@/variable";
...
function TrendingStackedHero({
  ...
  const visible = Math.min(TRENDING_STACK_DEPTH, n);
  ...
  return (
    ...
    >
      <div className="relative mx-auto w-full">
        <div
          className={cn("relative isolate w-full overflow-visible", STAGE_H)}
        >
          {Array.from({ length: visible }, (_, d) => {
            ...
            const backCardScale = isFront
              ? 1
              : Math.max(0.82, 1 - depth * 0.045);
            const z = 55 - depth;
            const rotateY = 0;
            const rotateZ = 0;
            const fanX = 0;
...
export function TrendingPage() {
  ...
  const loadHero = useCallback(async () => {
    ...
    setHeroError(null);
    try {
      const globalFeed = await blogApi.getPublishedFeed(
        GLOBAL_HERO_LIMIT,
        undefined,
        token,
      );
  ...
  useEffect(() => {
    ...
    if (!el || categoriesLoading || categoriesLoadingMore || !categoriesHasMore)
      return;
