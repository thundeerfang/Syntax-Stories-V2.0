...
import { PresenceIndicatorSettingsCard } from "@/components/settings";
export function EditProfileContent() {
  ...
  const [profileImgBlurDataUrl, setProfileImgBlurDataUrl] = useState<
    string | null
  >(null);
  const [coverBanner, setCoverBanner] = useState(user?.coverBanner ?? "");
  const [coverBannerBlurDataUrl, setCoverBannerBlurDataUrl] = useState<
    string | null
  >(null);
  ...
  const handleCoverUploadSuccess = async (result: {
    ...
    try {
      await updateProfile(
        alt
          ? { coverBanner: result.url, coverBannerAlt: alt }
  ...
  const handleProfilePicUploadSuccess = async (result: {
    ...
    try {
      await updateProfile(
        alt
          ? { profileImg: result.url, profileImgAlt: alt }
    ...
    <SettingsTabRoot>
      <SettingsTabPanel className="space-y-5">
            ...
            <div className="flex items-end gap-4 -mt-10">
              <div className="relative">
                <div className="relative size-20 md:size-24 border-2 border-border shadow bg-muted overflow-hidden">
                  <OptimizedRemoteImage
                    src={resolveProfileMediaUrl(profileImg, username)}
                ...
                <BlockShadowButton
                  ...
                  variant="primary"
                  shadow="sm"
                  className="absolute -right-1 -bottom-1 !gap-0 !p-0 size-7 min-h-0 shrink-0"
        ...
        <ImageUploadCropDialog
          ...
          titleIcon={
            <LucideImage className="size-4 shrink-0 text-primary" aria-hidden />
          }
        ...
        <section className="border-4 border-border bg-card p-5">
          ...
          <div className="relative flex items-center">
            <input
              ...
              onChange={(e) =>
                setPortfolioUrl(
                  e.target.value.slice(0, PROFILE_PORTFOLIO_URL_MAX),
                )
              }
