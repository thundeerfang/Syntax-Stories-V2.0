...
/**
 * Missing profile fields flow (P6) — checklist + per-item completion dialog.
 */
...
const COMPLETE_ITEM_FIELD_LABELS: Record<string, string> = {
          ...
          {missingFields.map((field) => (
            <div key={field}>
              <label className="block text-[10px] font-bold uppercase text-muted-foreground mb-1">
                {COMPLETE_ITEM_FIELD_LABELS[field] ?? field}
              </label>
              <input
                type="text"
        ...
        <div className="flex gap-2 pt-2">
          ...
          <button
            ...
            className={cn(
              ...
              canSave && !saving
                ? 'bg-primary text-primary-foreground shadow hover:shadow-none hover:translate-x-0.5 hover:translate-y-0.5'
