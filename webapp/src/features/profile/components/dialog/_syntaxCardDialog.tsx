...
/**
 * Syntax Card panel — inline card export UI used from settings.
 */

import React, { forwardRef, useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { Download, Globe, Loader2, Code2, Share2, Copy, Check } from 'lucide-react';
...
function SyntaxCardChip({ label }: Readonly<{ label: string }>) {
...
function resolveSquadIconUrl(squad: SyntaxCardSquad): string {
  const raw = squad.iconUrl?.trim();
  if (raw) {
...
function SyntaxCardSquadLogo({ squad }: Readonly<{ squad: SyntaxCardSquad }>) {
  return (
    <div className="size-[72px] shrink-0 overflow-hidden border-2 border-black bg-white shadow">
      <img
        src={resolveSquadIconUrl(squad)}
        alt=""
...
function openSocialComposer(
  ...
  switch (platform) {
    ...
    case 'linkedin':
      ...
      break;
    case 'instagram':
      globalThis.open('https://www.instagram.com/', '_blank', 'noopener,noreferrer');
      break;
  }
}

function socialShareFallbackToast(platform: SocialSharePlatform, imageCopied: boolean): void {
  if (imageCopied) {
...
function SyntaxCardStatIcon({
  kind,
}: Readonly<{ kind: 'posts' | 'respects' | 'followers' }>) {
  if (kind === 'posts') {
    return (
      <svg viewBox="0 0 24 24" className="size-8 shrink-0 stroke-white" fill="none" strokeWidth="2.5" aria-hidden>
...
function SyntaxCardStatsRow({
  ...
  respects,
  followers,
}: Readonly<{ posts: string; respects: string; followers: string }>) {
  const items = [
      ...
      <div className="grid grid-cols-3 divide-x-2 divide-black">
        {items.map((item) => (
          <div key={item.label} className="px-4 py-5 text-center">
            <p className="text-[48px] font-black italic leading-none text-white">{item.value}</p>
...
/** Fixed 1080×1350 export surface — Instagram portrait (4:5). */
        ...
        <div className="shrink-0">
          ...
          >
            ...
            {profileUrl ? (
              ...
              >
                <Globe className="size-8 shrink-0 text-white" strokeWidth={2.5} aria-hidden />
            ...
            <p className="mt-4 flex flex-wrap items-center justify-center gap-x-4 text-[26px] font-bold uppercase tracking-widest text-[#71717a]">
              ...
              {joinedLabel ? (
                <>
                  ...
                  <span className="text-[22px] tracking-[0.12em] text-[#a1a1aa]">{joinedLabel}</span>
                </>
              ) : null}
            </p>
          </div>
...
export interface SyntaxCardPanelProps {
  ...
  coverBanner?: string;
  userId?: string | null;
  /** When false, card data is not fetched. Defaults to true. */
  enabled?: boolean;
  className?: string;
}
...
async function loadFollowedCategoryNames(userId: string): Promise<string[]> {
  ...
  const bySlug = new Map(
    (tax.categories ?? []).map((c: BlogTaxonomyRow) => [c.slug.toLowerCase(), c.name] as const)
  );
  return slugs
...
export function SyntaxCardPanel({
  ...
  const loadCardData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [profileRes, postsRes, squadsRes, categoryNamesRes] = await Promise.all([
        followApi.getPublicProfile(username.trim()),
        blogApi.getUserPublishedPosts(username.trim(), 50),
        token ? squadsApi.listMine(token).catch(() => ({ squads: [] })) : Promise.resolve({ squads: [] }),
        userId?.trim() && token
  ...
  const shareToSocial = useCallback(
    async (platform: SocialSharePlatform) => {
      ...
      const file = await dataUrlToPngFile(dataUrl, `syntax-card-${username}.png`);
      const nativeResult = await tryNativeShareWithImage(file, shareText, profileUrl);
  ...
  const copyEmbedCode = useCallback(async () => {
    ...
    try {
      ...
      setEmbedCopied(true);
      toast.success('Embed HTML copied — replace IMAGE_URL with your hosted PNG');
