    ...
    <div className="flex shrink-0 items-center gap-1">
      {statusTabs.map((id) => {
        ...
        return (
          ...
          >
            {active ? <Icon className="size-3.5 shrink-0" strokeWidth={2.5} aria-hidden /> : null}
            {label}
          </button>
        );
      })}
      {showRepostsLink ? (
        <Link href="/reposts" className={cn(tabBtnClass(false), 'no-underline')}>
          <Repeat2 className="size-3.5 shrink-0" strokeWidth={2.5} aria-hidden />
          Reposts
        </Link>
      ) : null}
    </div>
