...
import {
  getWriteEditorSessionPostId,
  setWriteEditorSessionPostId,
} from "@/lib/blog/writeBlogSession";
...
export default function WriteBlogPage() {
  ...
  const [deployOverlayInitialSquadId, setDeployOverlayInitialSquadId] =
    useState<string | null>(null);
  const [mySquadsForPublish, setMySquadsForPublish] = useState<
    Pick<SquadSummary, "_id" | "name" | "slug">[]
  >([]);
  const [publishDialogSnapshot, setPublishDialogSnapshot] =
    useState<BlogPublishTaxonomy>({
      category: "",
      tags: [],
      language: "en",
    });
  ...
  const [draftSyncStatus, setDraftSyncStatus] = useState<
    "idle" | "offline" | "local" | "syncing" | "synced"
  >("idle");
  const [activePostId, setActivePostId] = useState<string | null>(null);
  const [loadedPostStatus, setLoadedPostStatus] = useState<
  ...
  const latestForSyncRef = useRef({
    title: "",
    summary: "",
    blocks: [] as Block[],
    thumbnailPreviewUrl: null as string | null,
  });
  ...
  const idleEditRevisionTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    void squadsApi
      ...
      .then((r) => {
        if (cancelled) return;
        setMySquadsForPublish(
          r.squads.map((s) => ({ _id: s._id, name: s.name, slug: s.slug })),
        );
      })
      .catch(() => {
  ...
  useEffect(() => {
    if (rightSidebarOpen) {
      ...
      if (wasClosed) {
        requestAnimationFrame(() => {
          revisionHistorySectionRef.current?.scrollIntoView({
            block: "nearest",
            behavior: "smooth",
          });
        });
  ...
  const captureBaseline = useCallback(() => {
    setContentBaseline(
      serializeWriteWorkspace({ title, summary, blocks, thumbnailPreviewUrl }),
    );
  ...
  const [squadWriteCtx, setSquadWriteCtx] = useState<{
    slug: string;
    name: string;
  } | null>(null);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const now = Date.now();
    if (now - lastAutosyncRevisionAtRef.current < 90000) return;
    lastAutosyncRevisionAtRef.current = now;
    appendRevision({
      kind: "autosynced",
      label: "Draft synced to server",
      at: now,
    });
  }, [draftSyncStatus, workspaceReady, appendRevision]);
  useEffect(() => {
    if (!workspaceReady || !isDirty) return;
    if (idleEditRevisionTimerRef.current)
      clearTimeout(idleEditRevisionTimerRef.current);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (targetId) {
          ...
          try {
            ...
            if (Array.isArray(parsed))
              nextBlocks = stripLegacyGifBlocks(parsed as Block[]);
          } catch {
            nextBlocks = [];
          }
        ...
        if (!cancelled)
          toast.error(e instanceof Error ? e.message : "Could not load editor");
      } finally {
  ...
  const addBlock = useCallback(
    (type: BlockType) => {
      ...
      setBlocks((prev) => [
        ...prev,
        createBlockInSection(type, PRIMARY_SECTION_ID),
      ]);
    },
    [blocks],
  );
  ...
  const centreMaxWidthClass = resolveCentreMaxWidthClass(
    leftSidebarOpen,
    rightSidebarOpen,
  );
  return (
                ...
                <section className="mt-auto shrink-0 border-t border-border/40 pt-4">
                  ...
                  <div className="space-y-2 border-2 border-border bg-black/5 p-3">
                    ...
                    <p className="text-[8px] leading-snug text-muted-foreground">
                      Title, summary, and every block (paragraphs, headings,
                      code, tables, captions, …).
                    </p>
                  </div>
                </section>
              </motion.div>
                ...
                <div className="relative mb-8">
                  ...
                  <textarea
                    ...
                    onPaste={(e) => {
                      ...
                      const maxInsert =
                        TITLE_MAX - before.length - after.length;
                      const toInsert = plain.slice(0, Math.max(0, maxInsert));
                      const newTitle = (before + toInsert + after)
                        .replaceAll(/\s+/g, " ")
                        .slice(0, TITLE_MAX);
                      setTitle(newTitle);
                      requestAnimationFrame(() => {
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    ...
                    >
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {submitAction === "draft" ? "Saving..." : "Save_Draft"}
                      </div>
                    </button>
                  </div>
                </div>
                    ...
                    {thumbnailPreviewUrl ? (
                      ...
                      >
                        ...
                        <span className="text-[9px] text-muted-foreground">
                          JPEG, PNG, GIF, WebP · max {THUMB_MAX_MB}MB
                        </span>
                      </button>
                    )}
                  </div>
