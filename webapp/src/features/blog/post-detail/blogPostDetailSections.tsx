...
const BlogImagePreviewContext = createContext<BlogImagePreviewOpen | null>(
  null,
);
...
function BlogImagePreviewModal({
  ...
  return (
    <Dialog
      ...
      title="Image preview"
      titleIcon={
        <ImageIcon className="size-5 shrink-0" strokeWidth={2.25} aria-hidden />
      }
      description={
        alt ? (
...
export function BlogImagePreviewProvider({
  children,
}: Readonly<{
  children: ReactNode;
}>) {
    ...
    >
      ...
      >
        <StatRow
          ...
          icon={Zap}
          label="Respect"
          value={formatStat(respectCount)}
        />
        <StatRow
          compact={compact}
          icon={Repeat2}
          label="Repost"
          value={formatStat(repostCount)}
        />
        <StatRow
          compact={compact}
          icon={Bookmark}
          label="Saved"
          value={formatStat(bookmarkCount)}
        />
        <StatRow
          compact={compact}
          icon={Eye}
          label="Views"
          value={formatStat(viewCount)}
        />
        <StatRow
          compact={compact}
          icon={MessageSquare}
...
export { MermaidBlockDisplay } from "@/components/ui/MermaidBlockDisplay";
...
export function BlogPostDetailSideRail({
  ...
  return (
    <div className="flex flex-col gap-4">
      <section
        className={blog.railCard}
        aria-labelledby="blog-rail-author-more"
      >
        <h2
      ...
      <section className={blog.railCard} aria-labelledby="blog-rail-share">
        <h2
          id="blog-rail-share"
          className="mb-3 flex items-center gap-2 font-mono text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground"
        >
        ...
        <div className="mb-3 flex flex-wrap gap-2">
          <a
            ...
            className={cn(
              blog.socialIconBtn,
              !postUrl && "pointer-events-none opacity-50",
            )}
      ...
      <section
        className={blog.authorCard}
        aria-labelledby="blog-rail-author-card"
      >
        <h2
      ...
      <span className="relative inline-flex h-[22px] w-[22px] shrink-0 items-center justify-center">
        {hover ? (
          ...
          <Bookmark
            className={cn(
              "h-5 w-5 shrink-0",
              bookmarked ? "text-primary" : "text-current",
            )}
            strokeWidth={2.5}
            fill="none"
            aria-hidden
...
export function BlogPostCommentsDock({
  ...
  useEffect(() => {
    ...
    const syncFooterLift = () => {
      ...
      const shouldUpdate =
        nextLift === 0 || prevLift === 0 || diff >= FOOTER_LIFT_DEADBAND_PX;
      ...
      >
        ...
        >
          <Link href={`/u/${c.author.username}`} className="shrink-0">
            <img
              src={resolveProfileMediaUrl(c.author.profileImg, c.author.username)}
              alt=""
        ...
        >
          <button
            ...
            onClick={() => {
              ...
              document
                .getElementById("blog-comment-composer-anchor")
                ?.scrollIntoView({
                  behavior: "smooth",
                  block: "center",
                });
...
export function BlogCommentsSection({
  ...
  const [deleteTarget, setDeleteTarget] = useState<PublicBlogComment | null>(
    null,
  );
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          {replyParentId && replyLabel ? (
            <div className="flex flex-wrap items-center justify-between gap-2 border-2 border-dashed border-border bg-muted/30 px-3 py-2 font-mono text-[10px] text-muted-foreground">
              <span>
                Replying to{" "}
                <span className="font-bold text-foreground">@{replyLabel}</span>
              </span>
              <button
                type="button"
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1 space-y-2">
              <label
                htmlFor={`blog-comment-editor-${formKey}`}
                className="sr-only"
              >
                Share your thoughts
              </label>
