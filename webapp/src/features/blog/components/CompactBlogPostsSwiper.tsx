...
export const CompactBlogPostsSwiper = forwardRef<
  CompactBlogPostsSwiperHandle,
  CompactBlogPostsSwiperProps
>(function CompactBlogPostsSwiper(
  ...
  const syncIndexFromScroll = useCallback(() => {
    ...
    const i = Math.min(n - 1, Math.max(0, Math.round(el.scrollLeft / stride)));
  ...
  return (
    <div className={cn('relative flex w-full flex-col', className)}>
      ...
      >
        {posts.map((post, i) => (
          <div key={post.id} className={slideCn} onFocusCapture={() => setIndex(i)}>
            <div className="h-full w-full">
              <BlogCard post={post} className="h-full w-full" />
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="mt-auto flex shrink-0 justify-center gap-2 pt-4">
          {posts.map((p, i) => (
            <button
              ...
              onClick={() => {
                const slide = scrollerRef.current?.children.item(i) as HTMLElement | undefined;
