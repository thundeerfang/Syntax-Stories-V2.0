...
export function useBlogCardEngagement(post: Post): BlogCardEngagementState {
  ...
  useEffect(() => {
    if (typeof post.viewerHasRespected === 'boolean') setRespecting(post.viewerHasRespected);
    if (typeof post.respectCount === 'number') setRespectCount(post.respectCount);
    if (typeof post.viewerHasReposted === 'boolean') setReposting(post.viewerHasReposted);
    if (typeof post.repostCount === 'number') setRepostCount(post.repostCount);
    if (typeof post.viewerHasBookmarked === 'boolean') setBookmarked(post.viewerHasBookmarked);
    if (typeof post.bookmarkCount === 'number') setBookmarkCount(post.bookmarkCount);
  }, [
    post.id,
  ...
  /** Feed often loads before auth hydrates; fill viewer flags when the API omitted them. */
  useEffect(() => {
