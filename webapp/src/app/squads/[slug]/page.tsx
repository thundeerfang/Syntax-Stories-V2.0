...
import type { SquadFeedPinChrome } from '@/components/blog/BlogCard';
...
import { InfoSwiperDialog } from '@/components/ui/InfoSwiperDialog';
import { DeleteConfirmDialog, WarningConfirmDialog } from '@/components/ui/delete';
import { SquadMembersDialog, type SquadMembersDialogRow } from '@/components/squads/SquadMembersDialog';
import { SquadsDiscoverCategoryView } from '@/components/squads/SquadsDiscoverCategoryView';
import { SQUADS_INTRO_SLIDES } from '@/components/squads/squadsIntroSlides';
import { SHELL_CONTENT_RAIL_CLASS } from '@/lib/shellContentRail';
import { isSquadCategory, squadCategoryLabel } from '@/lib/squadCategory';
...
export default function SquadDetailPage() {
  ...
  const categorySegment = slug ? decodeURIComponent(slug) : '';
  if (categorySegment && isSquadCategory(categorySegment)) {
    return (
    ...
    <div className={cn(SHELL_CONTENT_RAIL_CLASS, 'relative min-h-0 flex-1')}>
      ...
      <div className="relative z-10 flex min-h-0 flex-1 flex-col gap-6 pb-10">
        {!squad ? (
          ...
          <>
            <header className="relative z-20 mt-2 overflow-visible border-4 border-border bg-card shadow-[8px_8px_0_0_var(--border)]">
                    ...
                    <div className="relative -mt-14 shrink-0 sm:-mt-16">
                      {squad.iconUrl ? (
                        <img
                          src={squad.iconUrl}
                          alt=""
                          className="size-24 border-4 border-border bg-card object-cover shadow-[4px_4px_0_0_var(--border)] sm:size-32"
                        />
                      ) : (
                        <div className="flex size-24 items-center justify-center border-4 border-border bg-primary text-3xl font-black text-primary-foreground shadow-[4px_4px_0_0_var(--border)] sm:size-32 sm:text-4xl">
                          {squad.name.charAt(0).toUpperCase()}
                        </div>
                      )}
                      {squad.visibility === 'private' ? (
                        <div
                          className="absolute -right-2 -top-2 rounded-full border-2 border-border bg-yellow-400 p-1.5 text-black shadow-sm"
                          aria-hidden
                        >
                  ...
                  <div className="relative z-30 flex flex-wrap items-center gap-2 sm:shrink-0">
                    ...
                    <div ref={menuRef} className="relative z-40">
                      ...
                      {menuOpen ? (
                        <ul
                          role="menu"
                          className="absolute right-0 top-[calc(100%+8px)] z-[100] min-w-[14rem] border-2 border-border bg-card py-1 shadow-[4px_4px_0_0_var(--border)]"
                        >
                          {isAdmin && token ? (
                            <li role="none">
                              <button
            ...
            <Tabs value={tab} onValueChange={setTab} className="w-full min-w-0">
              <div className="mb-6 flex min-w-0 flex-wrap items-end gap-2 border-b-4 border-border pb-px">
                ...
                {feedVisible && tab === 'feed' ? (
                  <button
                    ...
                    className={cn(
                      'inline-flex shrink-0 items-center gap-2 border-2 border-border bg-card px-3 py-2 font-mono text-[10px] font-black uppercase tracking-widest shadow-[3px_3px_0_0_var(--border)] transition-colors',
                  ...
                  >
                    ...
                    <span>Pinned</span>
                    <span className="flex size-6 items-center justify-center rounded-full border-2 border-border bg-muted text-[10px] font-black tabular-nums text-foreground">
                      {pinnedCount}
                    </span>
                  </button>
                ) : null}
              ...
              <TabsContent
                value="resources"
                className="min-h-0 space-y-6 overflow-x-hidden overflow-y-visible border-2 border-border bg-card p-5 shadow-[6px_6px_0_0_var(--border)]"
              >
                <div>
      ...
      <WarningConfirmDialog
        ...
        description="You will need to join again (or use an invite) to come back."
        confirmLabel="Leave"
        confirming={busy}
      ...
      <DeleteConfirmDialog
        ...
        description="This removes the squad, its member list, and squad shares. Blog posts stay published but are unlinked from this squad."
