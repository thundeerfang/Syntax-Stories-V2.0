...
import { SquadsPageContentSkeleton } from '@/components/squads/SquadsPageSkeleton';
...
import { SQUAD_DISCOVER_CARD_GRID_CLASS } from '@/lib/squadDiscoverCardLayout';
...
const BROWSE_TABS: ReadonlyArray<{ id: SquadBrowseTab; label: string }> = [
...
/** Public directory rows plus your memberships, deduped by id (membership wins on clash). */
...
export default function SquadsPage() {
  ...
  const mergedCatalog = useMemo(() => mergeCatalog(publicSquads, mine), [publicSquads, mine]);

  const baseList = useMemo(() => {
    if (browseTab === 'yours') return mine;
    if (browseTab === 'public') return mergedCatalog.filter((s) => s.visibility === 'public');
    if (browseTab === 'private') return mergedCatalog.filter((s) => s.visibility === 'private');
  ...
  const showMembershipAuthGate = !token && (browseTab === 'yours' || browseTab === 'private');
    ...
    <div className={cn(SHELL_CONTENT_RAIL_CLASS, 'flex min-h-0 flex-1 flex-col')}>
        ...
        <ShellPageIntroHeader
          ...
          descriptionEnd={
            <div className="flex w-full flex-wrap items-center justify-center gap-2 sm:w-auto sm:justify-end">
              ...
              >
                <UserPlus className="size-4 shrink-0" strokeWidth={2.5} aria-hidden />
                Create squad
              </Button>
            </div>
          }
                ...
                <RailFeedEmptyState
                  ...
                  title={
                    browseTab === 'yours'
                      ...
                      : browseTab === 'private' && privateCountInMemberships === 0
                  ...
                  {filteredSquads.map((s) => (
                    <li key={s._id}>
                      <SquadDirectoryCard
                        ...
                        onEditSquad={token ? () => onEditSquadClick(s) : undefined}
                      />
                    </li>
                  ))}
                </ul>
      ...
      {token ? (
        <CreateSquadDialog
          ...
          onCreated={(slug) => router.push(`/squads/${encodeURIComponent(slug)}`)}
          onUpdated={() => void load()}
        />
      ) : null}
    </div>
