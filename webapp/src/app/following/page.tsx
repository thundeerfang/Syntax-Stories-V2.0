...
import { BlockShadowButton, RetroMenuItemButton } from "@/components/ui/button";
...
function mergeFollowingPosts(
  batches: PublicFeedPost[][],
  maxTotal: number,
): PublicFeedPost[] {
...
export default function FollowingPage() {
      ...
      try {
        if (!cursor) {
          const [res, chipRes] = await Promise.all([
            ...
            followApi.getFollowing(me, null, 5, {
              order: "asc",
              shuffle: true,
            }),
          ]);
  ...
  const loadPosts = useCallback(async () => {
    ...
    try {
      if (selectedUsername) {
        const res = await blogApi.getUserPublishedPosts(
          selectedUsername,
          36,
          token,
        );
      ...
      const batches = await Promise.all(
        following.map((u) =>
          blogApi
            .getUserPublishedPosts(u.username, perAuthor, token)
            .then((r) => r.posts ?? []),
        ),
      );
          ...
          <>
                  ...
                  {filteredSidebar.length === 0 ? (
                    ...
                    filteredSidebar.map((u) => (
                      <RetroMenuItemButton
                        ...
                        className={cn(
                          "gap-3 px-2 py-2 font-mono text-[11px] normal-case tracking-normal",
                          selectedUsername === u.username &&
                            "bg-primary/[0.08]",
                        )}
            ...
            {followingNext ? (
              <BlockShadowButton
                ...
                className="w-fit normal-case tracking-wide"
              >
